<?php
/**
 * Misc HiveQueen Administration API.
 *
 * @package HiveQueen
...
function save_mod_rewrite_rules() {
	 ...
	 */
	if ((!file_exists($htaccess_file) && is_writable($home_path) && $hq_rewrite->using_mod_rewrite_permalinks()) || is_writable($htaccess_file)) {
		if ( got_mod_rewrite() ) {
			$rules = explode( "\n", $hq_rewrite->mod_rewrite_rules() );
			return insert_with_markers( $htaccess_file, 'HiveQueen', $rules );
		}
	}
...
function iis7_save_url_rewrite_rules(){
	...
	if ( iis7_supports_permalinks() && ( ( ! file_exists($web_config_file) && win_is_writable($home_path) && $hq_rewrite->using_mod_rewrite_permalinks() ) || win_is_writable($web_config_file) ) ) {
		$rule = $hq_rewrite->iis7_url_rewrite_rules(false, '', '');
		if ( ! empty($rule) ) {
 ...
 * @since 0.0.1
 *
 * @param string|HQ_Error $message
 */
function show_message($message) {
	if ( is_hq_error($message) ){
 ...
 * @return array
 */
function hq_doc_link_parse( $content ) {
...
/**
 * Check if rewrite rule for HiveQueen already exists in the IIS 7+ configuration file
 *
 * @since 0.0.1
 *
 * @return bool
...
/**
 * Delete HiveQueen rewrite rule from web.config file if it exists there
 *
 * @since 0.0.1
 *
...
/**
 * Add HiveQueen rewrite rule to the IIS 7+ configuration file.
 *
 * @since 0.0.1
 *
 * @param string $filename The file path to the configuration file
	...
	<fieldset id="color-picker" class="scheme-list">
		...
		foreach ( $_hq_admin_css_colors as $color => $color_info ) :
			...
			<div class="color-option <?php echo ( $color == $current_color ) ? 'selected' : ''; ?>">
				...
				<input type="hidden" class="icon_colors" value="<?php echo esc_attr( hq_json_encode( array( 'icons' => $color_info->icon_colors ) ) ); ?>" />
 ...
 * @since 0.0.1
 */
function hq_check_locked_posts( $response, $data, $screen_id ) {
	$checked = array();

	if ( array_key_exists( 'hq-check-locked-posts', $data ) && is_array( $data['hq-check-locked-posts'] ) ) {
		foreach ( $data['hq-check-locked-posts'] as $key ) {
			if ( ! $post_id = absint( substr( $key, 5 ) ) )
				continue;

			if ( ( $user_id = hq_check_post_lock( $post_id ) ) && ( $user = get_userdata( $user_id ) ) && current_user_can( 'edit_post', $post_id ) ) {
	...
	if ( ! empty( $checked ) )
		$response['hq-check-locked-posts'] = $checked;
 ...
 * @since 0.0.1
 */
function hq_refresh_post_lock( $response, $data, $screen_id ) {
	if ( array_key_exists( 'hq-refresh-post-lock', $data ) ) {
		$received = $data['hq-refresh-post-lock'];
		$send = array();
		...
		if ( ( $user_id = hq_check_post_lock( $post_id ) ) && ( $user = get_userdata( $user_id ) ) ) {
			$error = array(
			...
			if ( $new_lock = hq_set_post_lock( $post_id ) )
				$send['new_lock'] = implode( ':', $new_lock );
		}

		$response['hq-refresh-post-lock'] = $send;
	}
...
function hq_refresh_post_nonces( $response, $data, $screen_id ) {
	if ( array_key_exists( 'hq-refresh-post-nonces', $data ) ) {
		...
		$response['hq-refresh-post-nonces'] = array(
			'replace' => array(
				'getpermalinknonce' => hq_create_nonce('getpermalink'),
				'samplepermalinknonce' => hq_create_nonce('samplepermalink'),
				'closedpostboxesnonce' => hq_create_nonce('closedpostboxes'),
				'_ajax_linking_nonce' => hq_create_nonce( 'internal-linking' ),
				'_hqnonce' => hq_create_nonce( 'update-post_' . $post_id ),
			),
			'heartbeatNonce' => hq_create_nonce( 'heartbeat-nonce' ),
		);
	}
 ...
 */
function hq_heartbeat_set_suspension( $settings ) {
	global $pagenow;
 ...
 * @since 0.0.1
 */
function hq_admin_canonical_url() {
	...
	?>
	<link id="hq-admin-canonical" rel="canonical" href="<?php echo esc_url( $filtered_url ); ?>" />
	<script>
		if ( window.history.replaceState ) {
			window.history.replaceState( null, null, document.getElementById( 'hq-admin-canonical' ).href + window.location.hash );
		}
	</script>
<?php
}
