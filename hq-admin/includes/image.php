 ...
 * @return string|HQ_Error New filepath on success, HQ_Error on failure.
 */
function hq_crop_image( $src, $src_x, $src_y, $src_w, $src_h, $dst_w, $dst_h, $src_abs = false, $dst_file = false ) {
	$src_file = $src;
	...
	$editor = hq_get_image_editor( $src );
	...
	if ( preg_match('!^image/!', get_post_mime_type( $attachment )) && file_is_displayable_image($file) ) {
		...
		foreach ( get_intermediate_image_sizes() as $s ) {
			...
			if ( isset( $_hq_additional_image_sizes[$s]['width'] ) )
				$sizes[$s]['width'] = intval( $_hq_additional_image_sizes[$s]['width'] ); // For theme-added sizes
			else
				$sizes[$s]['width'] = get_option( "{$s}_size_w" ); // For default sizes set in options
			if ( isset( $_hq_additional_image_sizes[$s]['height'] ) )
				$sizes[$s]['height'] = intval( $_hq_additional_image_sizes[$s]['height'] ); // For theme-added sizes
			else
				$sizes[$s]['height'] = get_option( "{$s}_size_h" ); // For default sizes set in options
			if ( isset( $_hq_additional_image_sizes[$s]['crop'] ) )
				$sizes[$s]['crop'] = $_hq_additional_image_sizes[$s]['crop']; // For theme-added sizes
			else
 ...
 */
function hq_read_image_metadata( $file ) {
	if ( ! file_exists( $file ) )
		return false;
	 ...
	 */
	if ( is_callable( 'exif_read_data' ) && in_array( $sourceImageType, apply_filters( 'hq_read_image_metadata_types', array( IMAGETYPE_JPEG, IMAGETYPE_TIFF_II, IMAGETYPE_TIFF_MM ) ) ) ) {
		...
		if ( ! empty( $exif['FNumber'] ) ) {
			$meta['aperture'] = round( hq_exif_frac2dec( $exif['FNumber'] ), 2 );
		}
		if ( ! empty( $exif['Model'] ) ) {
		...
		if ( ! empty( $exif['FocalLength'] ) ) {
			$meta['focal_length'] = (string) hq_exif_frac2dec( $exif['FocalLength'] );
		}
		...
		if ( ! empty( $exif['ExposureTime'] ) ) {
			$meta['shutter_speed'] = (string) hq_exif_frac2dec( $exif['ExposureTime'] );
		}
	 ...
	 */
	return apply_filters( 'hq_read_image_metadata', $meta, $file, $sourceImageType );
