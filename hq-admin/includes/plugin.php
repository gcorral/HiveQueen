 ...
 *     Version: Must be set in the plugin for HiveQueen 2.3+
...
function get_plugin_files($plugin) {
	$plugin_file = HQ_PLUGIN_DIR . '/' . $plugin;
	$dir = dirname($plugin_file);
	$plugin_files = array($plugin);
	if ( is_dir($dir) && $dir != HQ_PLUGIN_DIR ) {
 ...
 *
 * HiveQueen only includes mu-plugin files in the base mu-plugins directory (hq-content/mu-plugins).
 *
 * @since 0.0.1
...
function get_mu_plugins() {
	$hq_plugins = array();
	// Files in hq-content/mu-plugins directory
	$plugin_files = array();

	if ( ! is_dir( HQMU_PLUGIN_DIR ) )
		return $hq_plugins;
	if ( $plugins_dir = @ opendir( HQMU_PLUGIN_DIR ) ) {
		while ( ( $file = readdir( $plugins_dir ) ) !== false ) {
			if ( substr( $file, -4 ) == '.php' )
				$plugin_files[] = $file;
		}
	...
	foreach ( $plugin_files as $plugin_file ) {
		if ( !is_readable( HQMU_PLUGIN_DIR . "/$plugin_file" ) )
			continue;

		$plugin_data = get_plugin_data( HQMU_PLUGIN_DIR . "/$plugin_file", false, false ); //Do not apply markup/translate as it'll be cached.

		if ( empty ( $plugin_data['Name'] ) )
			$plugin_data['Name'] = $plugin_file;

		$hq_plugins[ $plugin_file ] = $plugin_data;
	}

	if ( isset( $hq_plugins['index.php'] ) && filesize( HQMU_PLUGIN_DIR . '/index.php') <= 30 ) // silence is golden
		unset( $hq_plugins['index.php'] );
...
/**
 * Check the hq-content directory and retrieve all drop-ins with any plugin data.
 *
 * @since 0.0.1
...
function get_dropins() {
	...
	// These exist in the hq-content directory
	if ( $plugins_dir = @ opendir( HQ_CONTENT_DIR ) ) {
		while ( ( $file = readdir( $plugins_dir ) ) !== false ) {
	...
	foreach ( $plugin_files as $plugin_file ) {
		if ( !is_readable( HQ_CONTENT_DIR . "/$plugin_file" ) )
			continue;
		$plugin_data = get_plugin_data( HQ_CONTENT_DIR . "/$plugin_file", false, false ); //Do not apply markup/translate as it'll be cached.
		if ( empty( $plugin_data['Name'] ) )
			$plugin_data['Name'] = $plugin_file;
...
/**
 * Returns drop-ins that HiveQueen uses.
 *
...
function _get_dropins() {
	$dropins = array(
		'advanced-cache.php' => array( __( 'Advanced caching plugin.'       ), 'HQ_CACHE' ), // HQ_CACHE
...
function is_network_only_plugin( $plugin ) {
	$plugin_data = get_plugin_data( HQ_PLUGIN_DIR . '/' . $plugin );
	if ( $plugin_data )
...
function activate_plugin( $plugin, $redirect = '', $network_wide = false, $silent = false ) {
	...
	if ( ( $network_wide && ! isset( $current[ $plugin ] ) ) || ( ! $network_wide && ! in_array( $plugin, $current ) ) ) {
		if ( !empty($redirect) )
			hq_redirect(add_query_arg('_error_nonce', hq_create_nonce('plugin-activation-error_' . $plugin), $redirect)); // we'll override this later if the plugin can be included without fatal error
		ob_start();
		hq_register_plugin_realpath( HQ_PLUGIN_DIR . '/' . $plugin );
		$_hq_plugin_file = $plugin;
		include_once( HQ_PLUGIN_DIR . '/' . $plugin );
		$plugin = $_hq_plugin_file; // Avoid stomping of the $plugin variable in a plugin.
 ...
 *
 * When HQ_Error is returned, it does not mean that one of the plugins had
 ...
 * @return bool|HQ_Error True when finished or HQ_Error if there were errors during a plugin activation.
 */
function activate_plugins( $plugins, $redirect = '', $network_wide = false, $silent = false ) {
	...
	if ( !empty($errors) )
		return new HQ_Error('plugins_invalid', __('One of the plugins is invalid.'), $errors);
 ...
 * @return HQ_Error|int 0 on success, HQ_Error on failure.
 */
function validate_plugin($plugin) {
	if ( validate_file($plugin) )
		return new HQ_Error('plugin_invalid', __('Invalid plugin path.'));
	if ( ! file_exists(HQ_PLUGIN_DIR . '/' . $plugin) )
		return new HQ_Error('plugin_not_found', __('Plugin file does not exist.'));

	$installed_plugins = get_plugins();
	if ( ! isset($installed_plugins[$plugin]) )
		return new HQ_Error('no_plugin_header', __('The plugin does not have a valid header.'));
	return 0;
}
...
function is_uninstallable_plugin($plugin) {
	...
	if ( isset( $uninstallable_plugins[$file] ) || file_exists( HQ_PLUGIN_DIR . '/' . dirname($file) . '/uninstall.php' ) )
		return true;
...
function uninstall_plugin($plugin) {
	...
	$uninstallable_plugins = (array) get_option('uninstall_plugins');
	if ( file_exists( HQ_PLUGIN_DIR . '/' . dirname($file) . '/uninstall.php' ) ) {
		...
		define('HQ_UNINSTALL_PLUGIN', $file);
		hq_register_plugin_realpath( HQ_PLUGIN_DIR . '/' . dirname( $file ) );
		include( HQ_PLUGIN_DIR . '/' . dirname($file) . '/uninstall.php' );
	...
	if ( isset( $uninstallable_plugins[$file] ) ) {
		...
		hq_register_plugin_realpath( HQ_PLUGIN_DIR . '/' . $file );
		include( HQ_PLUGIN_DIR . '/' . $file );
 ...
 *     * Pass 'none' to leave div.hq-menu-image empty so an icon can be added via CSS.
 ...
 * that the user has the required capability as well.
 *
 * @global int $_hq_last_object_menu
 *
 * @param string $page_title The text to be displayed in the title tags of the page when the menu is selected
 * @param string $menu_title The text to be used for the menu
 * @param string $capability The capability required for this menu to be displayed to the user.
...
function add_object_page( $page_title, $menu_title, $capability, $menu_slug, $function = '', $icon_url = '') {
	global $_hq_last_object_menu;

	$_hq_last_object_menu++;

	return add_menu_page($page_title, $menu_title, $capability, $menu_slug, $function, $icon_url, $_hq_last_object_menu);
}
 ...
 * that the user has the required capability as well.
 *
 * @global int $_hq_last_utility_menu
 *
 * @param string $page_title The text to be displayed in the title tags of the page when the menu is selected
 * @param string $menu_title The text to be used for the menu
 * @param string $capability The capability required for this menu to be displayed to the user.
...
function add_utility_page( $page_title, $menu_title, $capability, $menu_slug, $function = '', $icon_url = '') {
	global $_hq_last_utility_menu;

	$_hq_last_utility_menu++;

	return add_menu_page($page_title, $menu_title, $capability, $menu_slug, $function, $icon_url, $_hq_last_utility_menu);
}
 ...
 * @global array $menu
 * @global type $_hq_real_parent_file
 * @global bool $_hq_submenu_nopriv
 * @global array $_registered_pages
 * @global array $_parent_pages
 *
 * @param string $parent_slug The slug name for the parent menu (or the file name of a standard HiveQueen admin page)
 * @param string $page_title The text to be displayed in the title tags of the page when the menu is selected
 * @param string $menu_title The text to be used for the menu
 * @param string $capability The capability required for this menu to be displayed to the user.
 * @param string $menu_slug The slug name to refer to this menu by (should be unique for this menu)
...
function add_submenu_page( $parent_slug, $page_title, $menu_title, $capability, $menu_slug, $function = '' ) {
	global $submenu, $menu, $_hq_real_parent_file, $_hq_submenu_nopriv,
		$_registered_pages, $_parent_pages;

	$menu_slug = plugin_basename( $menu_slug );
	$parent_slug = plugin_basename( $parent_slug);

	if ( isset( $_hq_real_parent_file[$parent_slug] ) )
		$parent_slug = $_hq_real_parent_file[$parent_slug];

	if ( !current_user_can( $capability ) ) {
		$_hq_submenu_nopriv[$parent_slug][$menu_slug] = true;
		return false;
	}
	 ...
	 * See hq-admin/admin.php for redirect from edit.php to tools.php
	 */
 ...
 * @global string $_hq_real_parent_file
 * @global array $_hq_menu_nopriv
 * @global array $_hq_submenu_nopriv
 */
function get_admin_page_parent( $parent = '' ) {
	global $parent_file, $menu, $submenu, $pagenow, $typenow,
		$plugin_page, $_hq_real_parent_file, $_hq_menu_nopriv, $_hq_submenu_nopriv;

	if ( !empty ( $parent ) && 'admin.php' != $parent ) {
		if ( isset( $_hq_real_parent_file[$parent] ) )
			$parent = $_hq_real_parent_file[$parent];
		return $parent;
	}
	...
	if ( $pagenow == 'admin.php' && isset( $plugin_page ) ) {
		foreach ( (array)$menu as $parent_menu ) {
			if ( $parent_menu[2] == $plugin_page ) {
				...
				if ( isset( $_hq_real_parent_file[$parent_file] ) )
					$parent_file = $_hq_real_parent_file[$parent_file];
				return $parent_file;
			}
		}
		if ( isset( $_hq_menu_nopriv[$plugin_page] ) ) {
			$parent_file = $plugin_page;
			if ( isset( $_hq_real_parent_file[$parent_file] ) )
					$parent_file = $_hq_real_parent_file[$parent_file];
			return $parent_file;
	...
	if ( isset( $plugin_page ) && isset( $_hq_submenu_nopriv[$pagenow][$plugin_page] ) ) {
		$parent_file = $pagenow;
		if ( isset( $_hq_real_parent_file[$parent_file] ) )
			$parent_file = $_hq_real_parent_file[$parent_file];
		return $parent_file;
...
function user_can_access_admin_page() {
	global $pagenow, $menu, $submenu, $_hq_menu_nopriv, $_hq_submenu_nopriv,
		$plugin_page, $_registered_pages;

	$parent = get_admin_page_parent();

	if ( !isset( $plugin_page ) && isset( $_hq_submenu_nopriv[$parent][$pagenow] ) )
		return false;

	if ( isset( $plugin_page ) ) {
		if ( isset( $_hq_submenu_nopriv[$parent][$plugin_page] ) )
			return false;
	...
	if ( empty( $parent) ) {
		if ( isset( $_hq_menu_nopriv[$pagenow] ) )
			return false;
		if ( isset( $_hq_submenu_nopriv[$pagenow][$pagenow] ) )
			return false;
		if ( isset( $plugin_page ) && isset( $_hq_submenu_nopriv[$pagenow][$plugin_page] ) )
			return false;
		if ( isset( $plugin_page ) && isset( $_hq_menu_nopriv[$plugin_page] ) )
			return false;
		foreach (array_keys( $_hq_submenu_nopriv ) as $key ) {
			if ( isset( $_hq_submenu_nopriv[$key][$pagenow] ) )
				return false;
			if ( isset( $plugin_page ) && isset( $_hq_submenu_nopriv[$key][$plugin_page] ) )
			return false;
		}
		return true;
	}

	if ( isset( $plugin_page ) && ( $plugin_page == $parent ) && isset( $_hq_menu_nopriv[$plugin_page] ) )
		return false;
