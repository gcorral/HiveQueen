...
$hq_file_descriptions = array(
	...
	// Deprecated files
	'hq-layout.css' => __( 'Stylesheet' ),
	'hq-comments.php' => __( 'Comments Template' ),
	'hq-comments-popup.php' => __( 'Popup Comments Template' ),
);
...
/**
 * Get the absolute filesystem path to the root of the HiveQueen installation
 *
 * @since 0.0.1
 *
 * @return string Full filesystem path to the root of the HiveQueen installation
 */
function get_home_path() {
	$home    = set_url_scheme( get_option( 'home' ), 'http' );
	$siteurl = set_url_scheme( get_option( 'siteurl' ), 'http' );
	if ( ! empty( $home ) && 0 !== strcasecmp( $home, $siteurl ) ) {
		$hq_path_rel_to_home = str_ireplace( $home, '', $siteurl ); /* $siteurl - $home */
		$pos = strripos( str_replace( '\\', '/', $_SERVER['SCRIPT_FILENAME'] ), trailingslashit( $hq_path_rel_to_home ) );
 ...
 */
function hq_tempnam( $filename = '', $dir = '' ) {
	if ( empty( $dir ) ) {
		$dir = get_temp_dir();
	}
	...
	if ( ! $temp_filename ) {
		return hq_tempnam( dirname( $filename ), $dir );
	}

	$temp_filename .= '.tmp';
	$temp_filename = $dir . hq_unique_filename( $dir, $temp_filename );
	touch( $temp_filename );
...
function _hq_handle_upload( &$file, $overrides, $time, $action ) {
	 ...
	 */
	return apply_filters( 'hq_handle_upload', array(
		'file' => $new_file,
		'url'  => $url,
		'type' => $type
	), 'hq_handle_sideload' === $action ? 'sideload' : 'upload' ); }

/**
 * Wrapper for _hq_handle_upload(), passes 'hq_handle_upload' action.
 *
 * @since 0.0.1
 *
 * @see _hq_handle_upload()
 *
 ...
 *               $overrides['upload_error_handler'](&$file, $message ) or array( 'error'=>$message ).
 */
function hq_handle_upload( &$file, $overrides = false, $time = null ) {
	/*
	 *  $_POST['action'] must be set and its value must equal $overrides['action']
	 *  or this:
	 */
	$action = 'hq_handle_upload';
	if ( isset( $overrides['action'] ) ) {
		$action = $overrides['action'];
	}

	return _hq_handle_upload( $file, $overrides, $time, $action );
}

/**
 * Wrapper for _hq_handle_upload(), passes 'hq_handle_sideload' action
 *
 * @since 0.0.1
 *
 * @see _hq_handle_upload()
 ...
 *               $overrides['upload_error_handler'](&$file, $message ) or array( 'error'=>$message ).
 */
function hq_handle_sideload( &$file, $overrides = false, $time = null ) {
	/*
	 *  $_POST['action'] must be set and its value must equal $overrides['action']
	 *  or this:
	 */
	$action = 'hq_handle_sideload';
	if ( isset( $overrides['action'] ) ) {
		$action = $overrides['action'];
	}
	return _hq_handle_upload( $file, $overrides, $time, $action );
 ...
 * @return bool|object HQ_Error on failure, true on success, false when the MD5 format is unknown/unexpected
 */
function verify_file_md5( $filename, $expected_md5 ) {
	...
	return new HQ_Error( 'md5_mismatch', sprintf( __( 'The checksum of the file (%1$s) does not match the expected checksum value (%2$s).' ), bin2hex( $file_md5 ), bin2hex( $expected_raw_md5 ) ) );
}

/**
 * Unzips a specified ZIP file to a location on the Filesystem via the HiveQueen Filesystem Abstraction.
 * Assumes that HQ_Filesystem() has already been called and set up. Does not extract a root-level __MACOSX directory, if present.
 *
 ...
 * @since 0.0.1
 *
 * @global HQ_Filesystem_Base $hq_filesystem Subclass
 *
 * @param string $file Full path and filename of zip archive
 * @param string $to Full path on the filesystem to extract archive to
 * @return mixed HQ_Error on failure, True on success
 */
function unzip_file($file, $to) {
	global $hq_filesystem;

	if ( ! $hq_filesystem || !is_object($hq_filesystem) )
		return new HQ_Error('fs_unavailable', __('Could not access filesystem.'));

	// Unzip can use a lot of memory, but not this much hopefully
	/** This filter is documented in hq-admin/admin.php */
	...
	if ( ! $hq_filesystem->is_dir($to) ) { //Only do parents if no children exist
 ...
 * Assumes that HQ_Filesystem() has already been called and set up.
 *
 * @since 0.0.1
 * @see unzip_file
 * @access private
...
function _unzip_file_ziparchive($file, $to, $needed_dirs = array() ) {
	...
	if ( true !== $zopen )
		return new HQ_Error( 'incompatible_archive', __( 'Incompatible Archive.' ), array( 'ziparchive_error' => $zopen ) );

	$uncompressed_size = 0;

	for ( $i = 0; $i < $z->numFiles; $i++ ) {
		if ( ! $info = $z->statIndex($i) )
			return new HQ_Error( 'stat_failed_ziparchive', __( 'Could not retrieve file from archive.' ) );
	...
	if ( defined( 'DOING_CRON' ) && DOING_CRON ) {
		$available_space = @disk_free_space( HQ_CONTENT_DIR );
		if ( $available_space && ( $uncompressed_size * 2.1 ) > $available_space )
			return new HQ_Error( 'disk_full_unzip_file', __( 'Could not copy files. You may have run out of disk space.' ), compact( 'uncompressed_size', 'available_space' ) );
	}
	...
	foreach ( $needed_dirs as $_dir ) {
		...
		if ( ! $hq_filesystem->mkdir( $_dir, FS_CHMOD_DIR ) && ! $hq_filesystem->is_dir( $_dir ) ) {
			return new HQ_Error( 'mkdir_failed_ziparchive', __( 'Could not create directory.' ), substr( $_dir, strlen( $to ) ) );
		}
	}
	unset($needed_dirs);
	...
	for ( $i = 0; $i < $z->numFiles; $i++ ) {
		...
		if ( false === $contents )
			return new HQ_Error( 'extract_failed_ziparchive', __( 'Could not extract file from archive.' ), $info['name'] );

		if ( ! $hq_filesystem->put_contents( $to . $info['name'], $contents, FS_CHMOD_FILE) )
			return new HQ_Error( 'copy_failed_ziparchive', __( 'Could not copy file.' ), $info['name'] );
	}
...
function _unzip_file_pclzip($file, $to, $needed_dirs = array()) {
	...
	require_once(ABSPATH . 'hq-admin/includes/class-pclzip.php');
	...
	if ( !is_array($archive_files) )
		return new HQ_Error('incompatible_archive', __('Incompatible Archive.'), $archive->errorInfo(true));

	if ( 0 == count($archive_files) )
		return new HQ_Error( 'empty_archive_pclzip', __( 'Empty archive.' ) );
	...
	foreach ( $needed_dirs as $_dir ) {
		// Only check to see if the dir exists upon creation failure. Less I/O this way.
		if ( ! $hq_filesystem->mkdir( $_dir, FS_CHMOD_DIR ) && ! $hq_filesystem->is_dir( $_dir ) )
			return new HQ_Error( 'mkdir_failed_pclzip', __( 'Could not create directory.' ), substr( $_dir, strlen( $to ) ) );
	}
	unset($needed_dirs);
	...
	foreach ( $archive_files as $file ) {
		...
		if ( ! $hq_filesystem->put_contents( $to . $file['filename'], $file['content'], FS_CHMOD_FILE) )
			return new HQ_Error( 'copy_failed_pclzip', __( 'Could not copy file.' ), $file['filename'] );
	}
	return true;
}

/**
 * Copies a directory from one location to another via the HiveQueen Filesystem Abstraction.
 * Assumes that HQ_Filesystem() has already been called and setup.
 *
 * @since 0.0.1
 *
 * @global HQ_Filesystem_Base $hq_filesystem Subclass
...
function copy_dir($from, $to, $skip_list = array() ) {
	...
	$dirlist = $hq_filesystem->dirlist($from);
	...
	foreach ( (array) $dirlist as $filename => $fileinfo ) {
		...
		if ( 'f' == $fileinfo['type'] ) {
			if ( ! $hq_filesystem->copy($from . $filename, $to . $filename, true, FS_CHMOD_FILE) ) {
				// If copy failed, chmod file to 0644 and try again.
				$hq_filesystem->chmod( $to . $filename, FS_CHMOD_FILE );
				if ( ! $hq_filesystem->copy($from . $filename, $to . $filename, true, FS_CHMOD_FILE) )
					return new HQ_Error( 'copy_failed_copy_dir', __( 'Could not copy file.' ), $to . $filename );
			}
		} elseif ( 'd' == $fileinfo['type'] ) {
			if ( !$hq_filesystem->is_dir($to . $filename) ) {
				if ( !$hq_filesystem->mkdir($to . $filename, FS_CHMOD_DIR) )
					return new HQ_Error( 'mkdir_failed_copy_dir', __( 'Could not create directory.' ), $to . $filename );
			}
...
/**
 * Initialises and connects the HiveQueen Filesystem Abstraction classes.
 * This function will include the chosen transport and attempt connecting.
 *
 * Plugins may add extra transports, And force HiveQueen to use them by returning
 ...
 *                                                   the `HQ_Filesystem_*()` classes. Default false.
 ...
 */
function HQ_Filesystem( $args = false, $context = false, $allow_relaxed_file_ownership = false ) {
	global $hq_filesystem;

	require_once(ABSPATH . 'hq-admin/includes/class-hq-filesystem-base.php');
	...
	if ( ! class_exists("HQ_Filesystem_$method") ) {
		 ...
		 */
		$abstraction_file = apply_filters( 'filesystem_method_file', ABSPATH . 'hq-admin/includes/class-hq-filesystem-' . $method . '.php', $method );
	...
	$method = "HQ_Filesystem_$method";

	$hq_filesystem = new $method($args);
 ...
 *
 * The return value can be overridden by defining the `FS_METHOD` constant in `hq-config.php`,
 * or filtering via {@see 'filesystem_method'}.
 *
 * Plugins may define a custom transport handler, See HQ_Filesystem().
 *
 * @since 0.0.1
 *
 * @global callback $_hq_filesystem_direct_method
 *
...
function get_filesystem_method( $args = array(), $context = false, $allow_relaxed_file_ownership = false ) {
	...
	if ( ! $context ) {
		$context = HQ_CONTENT_DIR;
	}

	// If the directory doesn't exist (hq-content/languages) then use the parent directory as we'll create it.
	if ( HQ_LANG_DIR == $context && ! is_dir( $context ) ) {
	...
	if ( ! $method ) {
		...
		if ( $temp_handle ) {
			...
			// Attempt to determine the file owner of the HiveQueen files, and that of newly created files
			$hq_file_owner = $temp_file_owner = false;
			if ( function_exists('fileowner') ) {
				$hq_file_owner = @fileowner( __FILE__ );
				$temp_file_owner = @fileowner( $temp_file_name );
			}

			if ( $hq_file_owner !== false && $hq_file_owner === $temp_file_owner ) {
				// HiveQueen is creating files as the same owner as the HiveQueen files,
				// this means it's safe to modify & create new files via PHP.
				$method = 'direct';
				$GLOBALS['_hq_filesystem_direct_method'] = 'file_owner';
			} elseif ( $allow_relaxed_file_ownership ) {
				// The $context directory is writable, and $allow_relaxed_file_ownership is set, this means we can modify files
				// safely in this directory. This mode doesn't create new files, only alter existing ones.
				$method = 'direct';
				$GLOBALS['_hq_filesystem_direct_method'] = 'relaxed_ownership';
			}
 ...
 * @since 0.0.1
 */
function hq_print_request_filesystem_credentials_modal() {
