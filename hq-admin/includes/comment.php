...
function comment_exists($comment_author, $comment_date) {
	...
	return $hqdb->get_var( $hqdb->prepare("SELECT comment_post_ID FROM $hqdb->comments
...
function edit_comment() {
	if ( ! current_user_can( 'edit_comment', (int) $_POST['comment_ID'] ) )
		hq_die ( __( 'You are not allowed to edit comments on this post.' ) );
...
function get_pending_comments_num( $post_id ) {
	...
	$pending = $hqdb->get_results( "SELECT comment_post_ID, COUNT(comment_ID) as num_comments FROM $hqdb->comments WHERE comment_post_ID IN ( $post_id_in ) AND comment_approved = '0' GROUP BY comment_post_ID", ARRAY_A );
