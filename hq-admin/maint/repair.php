 ...
 * @subpackage Database
 */
define('HQ_REPAIRING', true);

require_once( dirname( dirname( dirname( __FILE__ ) ) ) . '/hq-load.php' );
...
<head>
	...
	<title><?php _e( 'HiveQueen &rsaquo; Database Repair' ); ?></title>
	<?php
...
if ( ! defined( 'HQ_ALLOW_REPAIR' ) ) {

	echo '<h1 class="screen-reader-text">' . __( 'Allow automatic database repair' ) . '</h1>';

	echo '<p>' . __( 'To allow use of this page to automatically repair database problems, please add the following line to your <code>hq-config.php</code> file. Once this line is added to your config, reload this page.' ) . "</p><p><code>define('HQ_ALLOW_REPAIR', true);</code></p>";
	...
	foreach ( $tables as $table ) {
		$check = $hqdb->get_row( "CHECK TABLE $table" );
		...
		if ( 'OK' == $check->Msg_text ) {
			...
			printf( __( 'The %1$s table is not okay. It is reporting the following error: %2$s. HiveQueen will attempt to repair this table&hellip;' ) , "<code>$table</code>", "<code>$check->Msg_text</code>" );

			$repair = $hqdb->get_row( "REPAIR TABLE $table" );
		...
		if ( $okay && $optimize ) {
			$check = $hqdb->get_row( "ANALYZE TABLE $table" );
			...
			if ( 'Table is already up to date' == $check->Msg_text )  {
				...
				$check = $hqdb->get_row( "OPTIMIZE TABLE $table" );
	...
	if ( $problems ) {
		...
		echo '<p>' . __( 'Repairs complete. Please remove the following line from hq-config.php to prevent this page from being used by unauthorized users.' ) . "</p><p><code>define('HQ_ALLOW_REPAIR', true);</code></p>";
	}
} else {

	echo '<h1 class="screen-reader-text">' . __( 'HiveQueen database repair' ) . '</h1>';

	if ( isset( $_GET['referrer'] ) && 'is_blog_installed' == $_GET['referrer'] )
		echo '<p>' . __( 'One or more database tables are unavailable. To allow HiveQueen to attempt to repair these tables, press the &#8220;Repair Database&#8221; button. Repairing can take a while, so please be patient.' ) . '</p>';
	else
		echo '<p>' . __( 'HiveQueen can automatically look for some common database problems and repair them. Repairing can take a while, so please be patient.' ) . '</p>';
?>
	<p class="step"><a class="button button-large" href="repair.php?repair=1"><?php _e( 'Repair Database' ); ?></a></p>
	<p><?php _e( 'HiveQueen can also attempt to optimize the database. This improves performance in some situations. Repairing and optimizing the database can take a long time and the database will be locked while optimizing.' ); ?></p>
