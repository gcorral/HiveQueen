...
class Custom_Image_Header {
	...
	public function __construct($admin_header_callback, $admin_image_div_callback = '') {
		...
		add_action( 'hq_ajax_custom-header-crop',   array( $this, 'ajax_header_crop'        ) );
		add_action( 'hq_ajax_custom-header-add',    array( $this, 'ajax_header_add'         ) );
		add_action( 'hq_ajax_custom-header-remove', array( $this, 'ajax_header_remove'      ) );
	}
	...
	public function help() {
		...
		get_current_screen()->add_help_tab( array(
			...
			'content' =>
				...
				'<p>' . __( 'If your theme has more than one default header image, or you have uploaded more than one custom header image, you have the option of having HiveQueen display a randomly different image on each page of your site. Click the &#8220;Random&#8221; radio button next to the Uploaded Images or Default Images section to enable this feature.') . '</p>' .
	...
	public function step() {
		...
		$step = (int) $_GET['step'];
		if ( $step < 1 || 3 < $step ||
			( 2 == $step && ! hq_verify_nonce( $_REQUEST['_hqnonce-custom-header-upload'], 'custom-header-upload' ) ) ||
			( 3 == $step && ! hq_verify_nonce( $_REQUEST['_hqnonce'], 'custom-header-crop-image' ) )
		)
			return 1;
	...
	public function take_action() {
		...
		if ( isset( $_POST['resetheader'] ) ) {
			check_admin_referer( 'custom-header-options', '_hqnonce-custom-header-options' );
	...
	public function process_default_headers() {
		...
		$this->default_headers = $_hq_default_headers;
...
(function($){
	...
	$(document).ready(function() {
		...
		text_color.hqColorPicker({
			change: function( event, ui ) {
				pickColor( text_color.hqColorPicker('color') );
			},
			clear: function() {
				pickColor( '' );
			}
...
<td>
	...
	?></p>
	<form enctype="multipart/form-data" id="upload-form" class="hq-upload-form" method="post" action="<?php echo esc_url( add_query_arg( 'step', 2 ) ) ?>">
	<p>
		<label for="upload"><?php _e( 'Choose an image from your computer:' ); ?></label><br />
		<input type="file" id="upload" name="import" />
		<input type="hidden" name="action" value="save" />
		<?php hq_nonce_field( 'custom-header-upload', '_hqnonce-custom-header-upload' ); ?>
	...
	<?php
		$modal_update_href = esc_url( add_query_arg( array(
			'page' => 'custom-header',
			'step' => 2,
			'_hqnonce-custom-header-upload' => hq_create_nonce('custom-header-upload'),
...
<?php }
	...
	public function step_2() {
			...
			&& $width == get_theme_support( 'custom-header', 'width' ) && $height == get_theme_support( 'custom-header', 'height' ) )
		{
			 ...
			 */
			do_action( 'hq_create_file_in_uploads', $file, $attachment_id ); // For replication
			...
			$image = apply_filters( 'hq_create_file_in_uploads', $image, $attachment_id ); // For replication
...
<form method="post" action="<?php echo esc_url(add_query_arg('step', 3)); ?>">
	...
	<?php } ?>
	<?php hq_nonce_field( 'custom-header-crop-image' ) ?>
	...
	public function step_3() {
		...
		/** This filter is documented in hq-admin/custom-header.php */
		$cropped = apply_filters( 'hq_create_file_in_uploads', $cropped, $attachment_id ); // For replication
	...
	final public function set_header_image( $choice ) {
		if ( is_array( $choice ) || is_object( $choice ) ) {
			...
			update_post_meta( $choice['attachment_id'], '_hq_attachment_is_custom_header', get_stylesheet() );
	...
	public function ajax_header_add() {
		...
		update_post_meta( $attachment_id, '_hq_attachment_is_custom_header', get_stylesheet() );
	...
	/**
	 *
	 * @param HQ_Customize_Manager $hq_customize
	 */
	public function customize_set_last_used( $hq_customize ) {
		$data = $hq_customize->get_setting( 'header_image_data' )->post_value();
	...
	public function get_uploaded_header_images() {
		...
		$timestamp_key = '_hq_attachment_custom_header_last_used_' . get_stylesheet();
		$alt_text_key = '_hq_attachment_image_alt';
