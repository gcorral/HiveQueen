 ...
 *
 * These functions must be used within the HiveQueen Loop.
 *
...
function get_author_posts_url($author_id, $author_nicename = '') {
	...
	$link = $hq_rewrite->get_author_permastruct();
...
function is_multi_author() {
	...
	if ( false === ( $is_multi_author = get_transient( 'is_multi_author' ) ) ) {
		$rows = (array) $hqdb->get_col("SELECT DISTINCT post_author FROM $hqdb->posts WHERE post_type = 'post' AND post_status = 'publish' LIMIT 2");
