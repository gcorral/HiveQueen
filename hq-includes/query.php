 ...
 * HiveQueen Query API
 *
 * The query API attempts to get which part of HiveQueen the user is on. It
...
/**
 * Retrieve variable in the HQ_Query class.
 *
 * @since 0.0.1
 *
...
function get_query_var( $var, $default = '' ) {
        global $hq_query;
        return $hq_query->get( $var, $default );
}

/**
 * Retrieve the currently-queried object. Wrapper for $hq_query->get_queried_object()
 *
 * @since 0.0.1
 * @access public
 *
...
function get_queried_object() {
        global $hq_query;
        return $hq_query->get_queried_object();
}

/**
 * Retrieve ID of the current queried object. Wrapper for $hq_query->get_queried_object_id()
 *
 * @since 0.0.1
 * @access public
 *
...
function get_queried_object_id() {
        global $hq_query;
        return $hq_query->get_queried_object_id();
}
...
function set_query_var( $var, $value ) {
        global $hq_query;
        $hq_query->set( $var, $value );
}

/**
 * Set up The Loop with query parameters.
 *
 * This will override the current HiveHueen Loop and shouldn't be used more than
 * once. This must not be used within the HiveQueen Loop.
 *
 * @since 0.0.1
 *
 * @global HQ_Query $hq_query
...
function query_posts($query) {
        $GLOBALS['hq_query'] = new HQ_Query();
        return $GLOBALS['hq_query']->query($query);
}
 ...
 * @global HQ_Query $hq_query
 */
function hq_reset_postdata() {
        global $hq_query;

        if ( isset( $hq_query ) ) {
                $hq_query->reset_postdata();
        }
}
...
function is_attachment( $attachment = '' ) {
        ...
        return $hq_query->is_attachment( $attachment );
}
...
function is_author( $author = '' ) {
        ...
        return $hq_query->is_author( $author );
}
...
function is_category( $category = '' ) {
        ...
        return $hq_query->is_category( $category );
}
...
function is_tax( $taxonomy = '', $term = '' ) {
        ...
        return $hq_query->is_tax( $taxonomy, $term );
}
...
function is_comments_popup() {
        ...
        return $hq_query->is_comments_popup();
}
...
function is_comment_feed() {
        ...
        return $hq_query->is_comment_feed();
}
       ...
       public function parse_query( $query =  '' ) {
        ...
        protected function parse_search( &$q ) {
                ...
                foreach ( $q['search_terms'] as $term ) {
                        if ( $n ) {
                                $like = '%' . $hqdb->esc_like( $term ) . '%';
                                $q['search_orderby_title'][] = $hqdb->prepare( "$hqdb->posts.post_title LIKE %s", $like );
                        }

                        $like = $n . $hqdb->esc_like( $term ) . $n;
                        $search .= $hqdb->prepare( "{$searchand}(($hqdb->posts.post_title LIKE %s) OR ($hqdb->posts.post_content LIKE %s))", $like, $like );
                        $searchand = ' AND ';
                }
                ...
                if ( ! empty( $search ) ) {
                        ...
                        if ( ! is_user_logged_in() )
                                $search .= " AND ($hqdb->posts.post_password = '') ";
                }
        ...
        protected function get_search_stopwords() {
                 ...
                 */
                $this->stopwords = apply_filters( 'hq_search_stopwords', $stopwords );
                return $this->stopwords;
        }
        ...
        protected function parse_search_order( &$q ) {
                ...
                if ( $q['search_terms_count'] > 1 ) {
                        ...
                        $like = '%' . $hqdb->esc_like( $q['s'] ) . '%';

                        $search_orderby = '(CASE ';
                        // sentence match in 'post_title'
                        $search_orderby .= $hqdb->prepare( "WHEN $hqdb->posts.post_title LIKE %s THEN 1 ", $like );
                        ...
                        $search_orderby .= $hqdb->prepare( "WHEN $hqdb->posts.post_content LIKE %s THEN 4 ", $like );
        ...
        protected function parse_orderby( $orderby ) {
                ...
                switch ( $orderby ) {
                        ...
                        case 'menu_order':
                        case 'comment_count':
                                $orderby_clause = "$hqdb->posts.{$orderby}";
                                break;
                        case 'rand':
                                $orderby_clause = 'RAND()';
                                break;
                        ...
                        default:
                                if ( array_key_exists( $orderby, $meta_clauses ) ) {
                                        ...
                                        $orderby_clause = "$hqdb->posts.post_" . sanitize_key( $orderby );
                                }
        ...
        public function get_posts() {
               ...
               // First let's clear some variables
                ...
                if ( isset( $q['caller_get_posts'] ) ) {
                        _deprecated_argument( 'HQ_Query', '3.1', __( '"caller_get_posts" is deprecated. Use "ignore_sticky_posts" instead.' ) );
                ...
                switch ( $q['fields'] ) {
                        case 'ids':
                                $fields = "$hqdb->posts.ID";
                                break;
                        case 'id=>parent':
                                $fields = "$hqdb->posts.ID, $hqdb->posts.post_parent";
                                break;
                        default:
                                $fields = "$hqdb->posts.*";
                }
                ...
                if ( '' !== $q['menu_order'] )
                        $where .= " AND $hqdb->posts.menu_order = " . $q['menu_order'];

               // The "m" parameter is meant for months but accepts datetimes of varying specificity
                if ( $q['m'] ) {
                        $where .= " AND YEAR($hqdb->posts.post_date)=" . substr($q['m'], 0, 4);
                        if ( strlen($q['m']) > 5 )
                                $where .= " AND MONTH($hqdb->posts.post_date)=" . substr($q['m'], 4, 2);
                        if ( strlen($q['m']) > 7 )
                ...
                if ( $date_parameters ) {
                        $date_query = new HQ_Date_Query( array( $date_parameters ) );
                ...
                if ( ! empty( $q['date_query'] ) ) {
                        $this->date_query = new HQ_Date_Query( $q['date_query'] );
               ...
               // If we've got a post_type AND it's not "any" post_type.
                ...
                if ( '' != $q['name'] ) {
                        ...
                        $where .= " AND $hqdb->posts.post_name = '" . $q['name'] . "'";
                } elseif ( '' != $q['pagename'] ) {
                        ...
                        $q['attachment'] = sanitize_title_for_query( hq_basename( $q['attachment'] ) );
                        $q['name'] = $q['attachment'];
                        $where .= " AND $hqdb->posts.post_name = '" . $q['attachment'] . "'";
                }
                ...
                if ( $q['p'] ) {
                        $where .= " AND {$hqdb->posts}.ID = " . $q['p'];
                } elseif ( $q['post__in'] ) {
                        $post__in = implode(',', array_map( 'absint', $q['post__in'] ));
                        $where .= " AND {$hqdb->posts}.ID IN ($post__in)";
                } elseif ( $q['post__not_in'] ) {
                        $post__not_in = implode(',',  array_map( 'absint', $q['post__not_in'] ));
                        $where .= " AND {$hqdb->posts}.ID NOT IN ($post__not_in)";
                }
                ...
                if ( is_numeric( $q['post_parent'] ) ) {
                        ...
                        $where .= " AND {$hqdb->posts}.post_parent NOT IN ($post_parent__not_in)";
                }

                if ( $q['page_id'] ) {
                        if  ( ('page' != get_option('show_on_front') ) || ( $q['page_id'] != get_option('page_for_posts') ) ) {
                                $q['p'] = $q['page_id'];
                                $where = " AND {$hqdb->posts}.ID = " . $q['page_id'];
                        }
                }
               ...
               /**
                ...
                if ( !$this->is_singular ) {
                        ...
                        $clauses = $this->tax_query->get_sql( $hqdb->posts, 'ID' );
               ...
               // Author/user stuff
                ...
                if ( ! empty( $q['author__not_in'] ) ) {
                        ...
                        $where .= " AND {$hqdb->posts}.post_author NOT IN ($author__not_in) ";
                } elseif ( ! empty( $q['author__in'] ) ) {
                        $author__in = implode( ',', array_map( 'absint', array_unique( (array) $q['author__in'] ) ) );
                        $where .= " AND {$hqdb->posts}.post_author IN ($author__in) ";
                }
                ...
                if ( '' != $q['author_name'] ) {
                        ...
                        $whichauthor .= " AND ($hqdb->posts.post_author = " . absint($q['author']) . ')';
                }

                // MIME-Type stuff for attachment browsing

                if ( isset( $q['post_mime_type'] ) && '' != $q['post_mime_type'] )
                        $whichmimetype = hq_post_mime_type_where( $q['post_mime_type'], $hqdb->posts );
               ...
               // Order by.
                if ( empty( $q['orderby'] ) ) {
                        ...
                        if ( isset( $q['orderby'] ) && ( is_array( $q['orderby'] ) || false === $q['orderby'] ) ) {
                                ...
                                $orderby = "$hqdb->posts.post_date " . $q['order'];
                        }
                } elseif ( 'none' == $q['orderby'] ) {
                        $orderby = '';
                } elseif ( $q['orderby'] == 'post__in' && ! empty( $post__in ) ) {
                        $orderby = "FIELD( {$hqdb->posts}.ID, $post__in )";
                } elseif ( $q['orderby'] == 'post_parent__in' && ! empty( $post_parent__in ) ) {
                        $orderby = "FIELD( {$hqdb->posts}.post_parent, $post_parent__in )";
                } else {
                        $orderby_array = array();
               ...
               // Order search results by relevance only when another "orderby" is not specified in the query.
                if ( ! empty( $q['s'] ) ) {
                         ...
                         * @param HQ_Query $this           The current HQ_Query instance.
                         */
                ...
                if ( isset( $q['post_password'] ) ) {
                        $where .= $hqdb->prepare( " AND $hqdb->posts.post_password = %s", $q['post_password'] );
                        if ( empty( $q['perm'] ) ) {
                                $q['perm'] = 'readable';
                        }
                } elseif ( isset( $q['has_password'] ) ) {
                        $where .= sprintf( " AND $hqdb->posts.post_password %s ''", $q['has_password'] ? '!=' : '=' );
                }
                ...
                if ( 'any' == $post_type ) {
                        ...
                        $where .= " AND $hqdb->posts.post_type = '$post_type'";
                        $post_type_object = get_post_type_object ( $post_type );
                } elseif ( $this->is_attachment ) {
                        $where .= " AND $hqdb->posts.post_type = 'attachment'";
                        $post_type_object = get_post_type_object ( 'attachment' );
                } elseif ( $this->is_page ) {
                        $where .= " AND $hqdb->posts.post_type = 'page'";
                        $post_type_object = get_post_type_object ( 'page' );
                } else {
                        $where .= " AND $hqdb->posts.post_type = 'post'";
                ...
                if ( ! empty( $q['post_status'] ) ) {
                        ...
                        if ( in_array( 'any', $q_status ) ) {
                                foreach ( get_post_stati( array( 'exclude_from_search' => true ) ) as $status ) {
                                        if ( ! in_array( $status, $q_status ) ) {
                                                $e_status[] = "$hqdb->posts.post_status <> '$status'";
                                        }
                                }
                        } else {
                                foreach ( get_post_stati() as $status ) {
                                        if ( in_array( $status, $q_status ) ) {
                                                if ( 'private' == $status )
                                                        $p_status[] = "$hqdb->posts.post_status = '$status'";
                                                else
                                                        $r_status[] = "$hqdb->posts.post_status = '$status'";
                                        }
                                }
                        }
                        ...
                        if ( !empty($r_status) ) {
                                if ( !empty($q['perm'] ) && 'editable' == $q['perm'] && !current_user_can($edit_others_cap) )
                                        $statuswheres[] = "($hqdb->posts.post_author = $user_id " . "AND (" . join( ' OR ', $r_status ) . "))";
                                else
                                        $statuswheres[] = "(" . join( ' OR ', $r_status ) . ")";
                        }
                        if ( !empty($p_status) ) {
                                if ( !empty($q['perm'] ) && 'readable' == $q['perm'] && !current_user_can($read_private_cap) )
                                        $statuswheres[] = "($hqdb->posts.post_author = $user_id " . "AND (" . join( ' OR ', $p_status ) . "))";
                                else
                                        $statuswheres[] = "(" . join( ' OR ', $p_status ) . ")";
                        }

                        if ( $post_status_join ) {
                                $join .= " LEFT JOIN $hqdb->posts AS p2 ON ($hqdb->posts.post_parent = p2.ID) ";
                                foreach ( $statuswheres as $index => $statuswhere )
                                        $statuswheres[$index] = "($statuswhere OR ($hqdb->posts.post_status = 'inherit' AND " . str_replace($hqdb->posts, 'p2', $statuswhere) . "))";
                        }
                        ...
                        $where .= " AND ($hqdb->posts.post_status = 'publish'";
                        ...
                        foreach ( (array) $public_states as $state ) {
                                if ( 'publish' == $state ) // Publish is hard-coded above.
                                        continue;
                                $where .= " OR $hqdb->posts.post_status = '$state'";
                        }
                        ...
                        if ( is_user_logged_in() ) {
                                ...
                                foreach ( (array) $private_states as $state )
                                        $where .= current_user_can( $read_private_cap ) ? " OR $hqdb->posts.post_status = '$state'" : " OR $hqdb->posts.post_author = $user_id AND $hqdb->posts.post_status = '$state'";
                        }
               ...
               /*
                ...
                if ( $this->is_comment_feed && ! $this->is_singular ) {
                        if ( $this->is_archive || $this->is_search ) {
                                $cjoin = "JOIN $hqdb->posts ON ($hqdb->comments.comment_post_ID = $hqdb->posts.ID) $join ";
                                $cwhere = "WHERE comment_approved = '1' $where";
                                $cgroupby = "$hqdb->comments.comment_id";
                        } else { // Other non singular e.g. front
                                $cjoin = "JOIN $hqdb->posts ON ( $hqdb->comments.comment_post_ID = $hqdb->posts.ID )";
                        ...
                        $this->comments = (array) $hqdb->get_results("SELECT $distinct $hqdb->comments.* FROM $hqdb->comments $cjoin $cwhere $cgroupby $corderby $climits");
                        ...
                        $join = '';
                        if ( $post_ids )
                                $where = "AND $hqdb->posts.ID IN ($post_ids) ";
                        else
                                $where = "AND 0";
                }
                ...
                $this->request = $old_request = "SELECT $found_rows $distinct $fields FROM $hqdb->posts $join WHERE 1=1 $where $groupby $orderby $limits";
                ...
                if ( 'ids' == $q['fields'] ) {
                        $this->posts = $hqdb->get_col( $this->request );
                ...
                if ( 'id=>parent' == $q['fields'] ) {
                        $this->posts = $hqdb->get_results( $this->request );
                        $this->post_count = count( $this->posts );
                        $this->set_found_posts( $q, $limits );
                ...
                $split_the_query = ( $old_request == $this->request && "$hqdb->posts.*" == $fields && !empty( $limits ) && $q['posts_per_page'] < 500 );
                /**
                ...
                if ( $split_the_query ) {
                        ...
                        $this->request = "SELECT $found_rows $distinct $hqdb->posts.ID FROM $hqdb->posts $join WHERE 1=1 $where $groupby $orderby $limits";
                ...
                if ( !empty($this->posts) && $this->is_comment_feed && $this->is_singular ) {
                        /** This filter is documented in hq-includes/query.php */
                        ...
                        $comments_request = "SELECT $hqdb->comments.* FROM $hqdb->comments $cjoin $cwhere $cgroupby $corderby $climits";
                        $this->comments = $hqdb->get_results($comments_request);
                        $this->comment_count = count($this->comments);
                }
                ...
                if ( !empty($this->posts) && ($this->is_single || $this->is_page) ) {
                        ...
                        if ( $this->is_preview && $this->posts && current_user_can( $edit_cap, $this->posts[0]->ID ) ) {
                                 ...
                                 * @since 0.0.1
                                 *
                                 * @param HQ_Post  $post_preview  The Post object.
                                 * @param HQ_Query &$this         The HQ_Query instance (passed by reference).
                                 */
