<?php
/**
 * HiveQueen API for media display.
 *
 * @package HiveQueen
 * @subpackage Media
 */
...
function image_constrain_size_for_editor( $width, $height, $size = 'medium', $context = null ) {
	global $content_width, $_hq_additional_image_sizes;
	...
	} elseif ( isset( $_hq_additional_image_sizes ) && count( $_hq_additional_image_sizes ) && in_array( $size, array_keys( $_hq_additional_image_sizes ) ) ) {
		$max_width = intval( $_hq_additional_image_sizes[$size]['width'] );
		$max_height = intval( $_hq_additional_image_sizes[$size]['height'] );
	...
	return hq_constrain_dimensions( $width, $height, $max_width, $max_height );
}
 ...
 * @since 0.0.1
 *
 * @global array $_hq_additional_image_sizes Associative array of additional image sizes.
 *
...
function add_image_size( $name, $width = 0, $height = 0, $crop = false ) {
	...
	$_hq_additional_image_sizes[ $name ] = array(
		'width'  => absint( $width ),
...
function has_image_size( $name ) {
	...
	return isset( $_hq_additional_image_sizes[ $name ] );
}
...
function remove_image_size( $name ) {
	...
	if ( isset( $_hq_additional_image_sizes[ $name ] ) ) {
		unset( $_hq_additional_image_sizes[ $name ] );
		return true;
	}
 ...
 * parameters are: what HiveQueen will use for the class, the Attachment ID,
...
function get_image_tag( $id, $alt, $title, $align, $size = 'medium' ) {
	...
	$class = 'align' . esc_attr($align) .' size-' . esc_attr($size) . ' hq-image-' . $id;
 ...
 */
function hq_constrain_dimensions( $current_width, $current_height, $max_width = 0, $max_height = 0 ) {
	if ( !$max_width && !$max_height )
	 ...
	 */
	return apply_filters( 'hq_constrain_dimensions', array( $w, $h ), $current_width, $current_height, $max_width, $max_height );
}

/**
 * Retrieves calculated resize dimensions for use in HQ_Image_Editor.
 *
...
function image_resize_dimensions($orig_w, $orig_h, $dest_w, $dest_h, $crop = false) {
	...
	if ( $crop ) {
		...
		list( $new_w, $new_h ) = hq_constrain_dimensions( $orig_w, $orig_h, $dest_w, $dest_h );
	}
...
function image_make_intermediate_size( $file, $width, $height, $crop = false ) {
	if ( $width || $height ) {
		...
		if ( is_hq_error( $editor ) || is_hq_error( $editor->resize( $width, $height, $crop ) ) )
			return false;

		$resized_file = $editor->save();

		if ( ! is_hq_error( $resized_file ) && $resized_file ) {
...
function image_get_intermediate_size( $post_id, $size = 'thumbnail' ) {
	if ( !is_array( $imagedata = hq_get_attachment_metadata( $post_id ) ) )
		return false;
	...
	if ( empty($data['path']) && !empty($data['file']) ) {
		$file_url = hq_get_attachment_url($post_id);
...
function get_intermediate_image_sizes() {
	...
	if ( isset( $_hq_additional_image_sizes ) && count( $_hq_additional_image_sizes ) )
		$image_sizes = array_merge( $image_sizes, array_keys( $_hq_additional_image_sizes ) );
 ...
 */
function hq_get_attachment_image_src( $attachment_id, $size = 'thumbnail', $icon = false ) {
	// get a thumbnail or intermediate image if there is one
	$image = image_downsize( $attachment_id, $size );
	if ( ! $image ) {
		$src = false;

		if ( $icon && $src = hq_mime_type_icon( $attachment_id ) ) {
			/** This filter is documented in hq-includes/post.php */
			$icon_dir = apply_filters( 'icon_dir', ABSPATH . HQINC . '/images/media' );

			$src_file = $icon_dir . '/' . hq_basename( $src );
	 ...
	 */
	return apply_filters( 'hq_get_attachment_image_src', $image, $attachment_id, $size, $icon );
}
 ...
 */
function hq_get_attachment_image($attachment_id, $size = 'thumbnail', $icon = false, $attr = '') {
	$html = '';
	$image = hq_get_attachment_image_src($attachment_id, $size, $icon);
	if ( $image ) {
		...
		$default_attr = array(
			...
			'alt'	=> trim(strip_tags( get_post_meta($attachment_id, '_hq_attachment_image_alt', true) )), // Use Alt field first
		);
		...
		$attr = hq_parse_args($attr, $default_attr);
		 ...
		 * @param HQ_Post      $attachment Image attachment post.
		 * @param string|array $size       Requested size.
		 */
		$attr = apply_filters( 'hq_get_attachment_image_attributes', $attr, $attachment, $size );
...
/**
 * Adds a 'hq-post-image' class to post thumbnails. Internal use only.
 *
 ...
 * @return array Modified array of attributes including the new 'hq-post-image' class.
 */
function _hq_post_thumbnail_class_filter( $attr ) {
	$attr['class'] .= ' hq-post-image';
	return $attr;
}

/**
 * Adds '_hq_post_thumbnail_class_filter' callback to the 'hq_get_attachment_image_attributes'
 ...
 * @param array $attr Thumbnail attributes including src, class, alt, title.
 */
function _hq_post_thumbnail_class_filter_add( $attr ) {
	add_filter( 'hq_get_attachment_image_attributes', '_hq_post_thumbnail_class_filter' );
}

/**
 * Removes the '_hq_post_thumbnail_class_filter' callback from the 'hq_get_attachment_image_attributes'
 * filter hook. Internal use only.
 *
 * @ignore
 * @since 0.0.1
 ...
 * @param array $attr Thumbnail attributes including src, class, alt, title.
 */
function _hq_post_thumbnail_class_filter_remove( $attr ) {
	remove_filter( 'hq_get_attachment_image_attributes', '_hq_post_thumbnail_class_filter' );
}

//TODO: !!!
//add_shortcode('hq_caption', 'img_caption_shortcode');
...
function img_caption_shortcode( $attr, $content = null ) {
	...
	if ( current_theme_supports( 'html5', 'caption' ) ) {
		...
		. do_shortcode( $content ) . '<figcaption class="hq-caption-text">' . $atts['caption'] . '</figcaption></figure>';
	}
	...
	. do_shortcode( $content ) . '<p class="hq-caption-text">' . $atts['caption'] . '</p></div>';
}
...
function gallery_shortcode( $attr ) {
	...
	if ( is_feed() ) {
		...
		foreach ( $attachments as $att_id => $attachment ) {
			$output .= hq_get_attachment_link( $att_id, $atts['size'], true ) . "\n";
		}
		return $output;
	}
	...
	$valid_tags = hq_kses_allowed_html( 'post' );
 ...
 * @since 0.0.1
 */
function hq_underscore_playlist_templates() {
?>
<script type="text/html" id="tmpl-hq-playlist-current-item">
	<# if ( data.image ) { #>
	<img src="{{ data.thumb.src }}"/>
	<# } #>
	<div class="hq-playlist-caption">
		<span class="hq-playlist-item-meta hq-playlist-item-title">&#8220;{{ data.title }}&#8221;</span>
		<# if ( data.meta.album ) { #><span class="hq-playlist-item-meta hq-playlist-item-album">{{ data.meta.album }}</span><# } #>
		<# if ( data.meta.artist ) { #><span class="hq-playlist-item-meta hq-playlist-item-artist">{{ data.meta.artist }}</span><# } #>
	</div>
</script>
<script type="text/html" id="tmpl-hq-playlist-item">
	<div class="hq-playlist-item">
		<a class="hq-playlist-caption" href="{{ data.src }}">
			{{ data.index ? ( data.index + '. ' ) : '' }}
			<# if ( data.caption ) { #>
				{{ data.caption }}
			<# } else { #>
				<span class="hq-playlist-item-title">&#8220;{{{ data.title }}}&#8221;</span>
				<# if ( data.artists && data.meta.artist ) { #>
				<span class="hq-playlist-item-artist"> &mdash; {{ data.meta.artist }}</span>
				<# } #>
			<# } #>
		</a>
		<# if ( data.meta.length_formatted ) { #>
		<div class="hq-playlist-item-length">{{ data.meta.length_formatted }}</div>
		<# } #>
	</div>
</script>
<?php
 ...
 */
function hq_playlist_scripts( $type ) {
	hq_enqueue_style( 'hq-mediaelement' );
	hq_enqueue_script( 'hq-playlist' );
?>
<!--[if lt IE 9]><script>document.createElement('<?php echo esc_js( $type ) ?>');</script><![endif]-->
<?php
	add_action( 'hq_footer', 'hq_underscore_playlist_templates', 0 );
	add_action( 'admin_footer', 'hq_underscore_playlist_templates', 0 );
}
 ...
 * a collection of HiveQueen audio or video files in a post.
 *
 * @since 0.0.1
 *
 * @global int $content_width
 ...
 */
function hq_playlist_shortcode( $attr ) {
	...
	if ( is_feed() ) {
		$output = "\n";
		foreach ( $attachments as $att_id => $attachment ) {
			$output .= hq_get_attachment_link( $att_id ) . "\n";
		}
		return $output;
	}
	...
	$data = array(
		...
		'tracklist' => hq_validate_boolean( $atts['tracklist'] ),
		'tracknumbers' => hq_validate_boolean( $atts['tracknumbers'] ),
		'images' => hq_validate_boolean( $atts['images'] ),
		'artists' => hq_validate_boolean( $atts['artists'] ),
	);

	$tracks = array();
	foreach ( $attachments as $attachment ) {
		$url = hq_get_attachment_url( $attachment->ID );
		$ftype = hq_check_filetype( $url, hq_get_mime_types() );
		$track = array(
			'src' => $url,
			'type' => $ftype['type'],
		...
		if ( ! empty( $meta ) ) {
			...
			foreach ( hq_get_attachment_id3_keys( $attachment ) as $key => $label ) {
				if ( ! empty( $meta[ $key ] ) ) {
		...
		if ( $atts['images'] ) {
			...
			if ( ! empty( $thumb_id ) ) {
				list( $src, $width, $height ) = hq_get_attachment_image_src( $thumb_id, 'full' );
				$track['image'] = compact( 'src', 'width', 'height' );
				list( $src, $width, $height ) = hq_get_attachment_image_src( $thumb_id, 'thumbnail' );
	...
	if ( 1 === $instance ) {
		 ...
		 */
		do_action( 'hq_playlist_scripts', $atts['type'], $atts['style'] );
	} ?>
<div class="hq-playlist hq-<?php echo $safe_type ?>-playlist hq-playlist-<?php echo $safe_style ?>">
	<?php if ( 'audio' === $atts['type'] ): ?>
	<div class="hq-playlist-current-item"></div>
	<?php endif ?>
	...
	<div class="hq-playlist-next"></div>
	<div class="hq-playlist-prev"></div>
	<noscript>
	<ol><?php
	foreach ( $attachments as $att_id => $attachment ) {
		printf( '<li>%s</li>', hq_get_attachment_link( $att_id ) );
	}
	?></ol>
	</noscript>
	<script type="application/json" class="hq-playlist-script"><?php echo hq_json_encode( $data ) ?></script>
</div>
	<?php
	return ob_get_clean();
}
...
//TODO: !!!
//add_shortcode( 'playlist', 'hq_playlist_shortcode' );
 ...
 */
function hq_mediaelement_fallback( $url ) {
	/**
	 ...
	 */
	return apply_filters( 'hq_mediaelement_fallback', sprintf( '<a href="%1$s">%1$s</a>', esc_url( $url ) ), $url );
}

/**
 * Returns a filtered list of HQ-supported audio formats.
 *
 * @since 0.0.1
 *
 * @return array Supported audio formats.
 */
function hq_get_audio_extensions() {
	/**
	 ...
	 *                          'mp3', 'ogg', 'wma', 'm4a', 'wav'.
	 */
	return apply_filters( 'hq_audio_extensions', array( 'mp3', 'ogg', 'wma', 'm4a', 'wav' ) );
}
 ...
 * @since 0.0.1
 *
 * @param HQ_Post $attachment The current attachment, provided for context.
 * @param string  $context    Optional. The context. Accepts 'edit', 'display'. Default 'display'.
 * @return array Key/value pairs of field keys to labels.
 */
function hq_get_attachment_id3_keys( $attachment, $context = 'display' ) {
	$fields = array(
		'artist' => __( 'Artist' ),
		'album' => __( 'Album' ),
	);
	 ...
	 */
	return apply_filters( 'hq_get_attachment_id3_keys', $fields, $attachment, $context );
}
/**
 * Builds the Audio shortcode output.
 *
 * This implements the functionality of the Audio Shortcode for displaying
 * HiveQueen mp3s in a post.
 *
 * @since 0.0.1
 *
 * @staticvar int $instance
 ...
 *     @type string $class    The 'class' attribute for the `<audio>` element. Default 'hq-audio-shortcode'.
 *     @type string $style    The 'style' attribute for the `<audio>` element. Default 'width: 100%'.
 * }
 * @param string $content Shortcode content.
 * @return string|void HTML content to display audio.
 */
function hq_audio_shortcode( $attr, $content = '' ) {
	 ...
	 * @param array  $attr     Attributes of the shortcode. @see hq_audio_shortcode()
	 * @param string $content  Shortcode content.
	 * @param int    $instance Unique numeric ID of this audio shortcode instance.
	 */
	$override = apply_filters( 'hq_audio_shortcode_override', '', $attr, $content, $instance );
	if ( '' !== $override ) {
		return $override;
	}
	...
	$default_types = hq_get_audio_extensions();
	$defaults_atts = array(
		'src'      => '',
		'loop'     => '',
		'autoplay' => '',
	...
	if ( ! empty( $atts['src'] ) ) {
		...
		if ( ! in_array( strtolower( $type['ext'] ), $default_types ) ) {
			return sprintf( '<a class="hq-embedded-audio" href="%s">%s</a>', esc_url( $atts['src'] ), esc_html( $atts['src'] ) );
		}
		$primary = true;
	...
	if ( ! $primary ) {
		...
		$audio = reset( $audios );
		$atts['src'] = hq_get_attachment_url( $audio->ID );
		if ( empty( $atts['src'] ) ) {
			return;
		}
	 ...
	 */
	$library = apply_filters( 'hq_audio_shortcode_library', 'mediaelement' );
	 ...
	 */
	$html_atts = array(
		'class'    => apply_filters( 'hq_audio_shortcode_class', 'hq-audio-shortcode' ),
		'id'       => sprintf( 'audio-%d-%d', $post_id, $instance ),
		'loop'     => hq_validate_boolean( $atts['loop'] ),
		'autoplay' => hq_validate_boolean( $atts['autoplay'] ),
	 ...
	 * @param string $library Media library used for the audio shortcode.
	 */
	return apply_filters( 'hq_audio_shortcode', $html, $atts, $audio, $post_id, $library );
}

//TODO: !!!
//add_shortcode( 'audio', 'hq_audio_shortcode' );

/**
 * Returns a filtered list of HQ-supported video formats.
 *
 * @since 0.0.1
 *
 * @return array List of supported video formats.
 */
function hq_get_video_extensions() {
	/**
	 ...
	 */
	return apply_filters( 'hq_video_extensions', array( 'mp4', 'm4v', 'webm', 'ogv', 'wmv', 'flv' ) );
}
...
function hq_video_shortcode( $attr, $content = '' ) {
	 ...
	 */
	$html_atts = array(
		'class'    => apply_filters( 'hq_video_shortcode_class', 'hq-video-shortcode' ),
...
//TODO: !!!
//add_shortcode( 'video', 'hq_video_shortcode' );
...
function adjacent_image_link( $prev = true, $size = 'thumbnail', $text = false ) {
	...
	if ( $attachments ) {
		...
		if ( isset( $attachments[ $k ] ) ) {
			...
			$output = hq_get_attachment_link( $attachment_id, $size, true, false, $text );
		}
	}
 ...
 */
function hq_imagecreatetruecolor($width, $height) {
 ...
 */
function hq_embed_register_handler( $id, $regex, $callback, $priority = 10 ) {
	global $hq_embed;
	$hq_embed->register_handler( $id, $regex, $callback, $priority );
}
 ...
 */
function hq_embed_unregister_handler( $id, $priority = 10 ) {
	global $hq_embed;
	$hq_embed->unregister_handler( $id, $priority );
}
 ...
 */
function hq_embed_defaults( $url = '' ) {
 ...
 */
function hq_expand_dimensions( $example_width, $example_height, $max_width, $max_height ) {
	$example_width  = (int) $example_width;
	$example_height = (int) $example_height;
	$max_width      = (int) $max_width;
	$max_height     = (int) $max_height;

	return hq_constrain_dimensions( $example_width * 1000000, $example_height * 1000000, $max_width, $max_height );
}
 ...
 */
function hq_oembed_get( $url, $args = '' ) {
	require_once( ABSPATH . HQINC . '/class-oembed.php' );
	$oembed = _hq_oembed_get_object();
 ...
 */
function hq_oembed_add_provider( $format, $provider, $regex = false ) {
	require_once( ABSPATH . HQINC . '/class-oembed.php' );
	...
	if ( did_action( 'plugins_loaded' ) ) {
		...
		HQ_oEmbed::_add_provider_early( $format, $provider, $regex );
	}
}
 ...
 */
function hq_oembed_remove_provider( $format ) {
	require_once( ABSPATH . HQINC . '/class-oembed.php' );
	...
	if ( did_action( 'plugins_loaded' ) ) {
		...
		HQ_oEmbed::_remove_provider_early( $format );
	}
 ...
 * @since 0.0.1
 *
 * @see hq_embed_register_handler()
 */
function hq_maybe_load_embeds() {
	/**
	...
	hq_embed_register_handler( 'youtube_embed_url', '#https?://(www.)?youtube\.com/(?:v|embed)/([^/]+)#i', 'hq_embed_handler_youtube' );

	hq_embed_register_handler( 'googlevideo', '#http://video\.google\.([A-Za-z.]{2,5})/videoplay\?docid=([\d-]+)(.*?)#i', 'hq_embed_handler_googlevideo' );
	 ...
	 */
	hq_embed_register_handler( 'audio', '#^https?://.+?\.(' . join( '|', hq_get_audio_extensions() ) . ')$#i', apply_filters( 'hq_audio_embed_handler', 'hq_embed_handler_audio' ), 9999 );
	 ...
	 */
	hq_embed_register_handler( 'video', '#^https?://.+?\.(' . join( '|', hq_get_video_extensions() ) . ')$#i', apply_filters( 'hq_video_embed_handler', 'hq_embed_handler_video' ), 9999 );
}
 ...
 *
 * @see HQ_Embed::register_handler()
 * @see HQ_Embed::shortcode()
 *
 * @param array  $matches The RegEx matches from the provided regex when calling hq_embed_register_handler().
 * @param array  $attr    Embed attributes.
 * @param string $url     The original URL that was matched by the regex.
 * @param array  $rawattr The original unmodified attributes.
 * @return string The embed HTML.
 */
function hq_embed_handler_googlevideo( $matches, $attr, $url, $rawattr ) {
	// If the user supplied a fixed width AND height, use it
	if ( !empty($rawattr['width']) && !empty($rawattr['height']) ) {
		$width  = (int) $rawattr['width'];
		$height = (int) $rawattr['height'];
	} else {
		list( $width, $height ) = hq_expand_dimensions( 425, 344, $attr['width'], $attr['height'] );
	}
 ...
 *                        hq_embed_register_handler().
 * @param array  $attr    Embed attributes.
 * @param string $url     The original URL that was matched by the regex.
 * @param array  $rawattr The original unmodified attributes.
 * @return string The embed HTML.
 */
function hq_embed_handler_youtube( $matches, $attr, $url, $rawattr ) {
	global $hq_embed;
	$embed = $hq_embed->autoembed( "https://youtube.com/watch?v={$matches[2]}" );
	 ...
	 * @param array  $rawattr The original unmodified attributes.
	 */
	return apply_filters( 'hq_embed_handler_youtube', $embed, $attr, $url, $rawattr );
}
 ...
 * @return string The embed HTML.
 */
function hq_embed_handler_audio( $matches, $attr, $url, $rawattr ) {
	 ...
	 * @param array  $rawattr The original unmodified attributes.
	 */
	return apply_filters( 'hq_embed_handler_audio', $audio, $attr, $url, $rawattr );
}
 ...
 * @return string The embed HTML.
 */
function hq_embed_handler_video( $matches, $attr, $url, $rawattr ) {
	$dimensions = '';
	if ( ! empty( $rawattr['width'] ) && ! empty( $rawattr['height'] ) ) {
	 ...
	 * @param array  $rawattr The original unmodified attributes.
	 */
	return apply_filters( 'hq_embed_handler_video', $video, $attr, $url, $rawattr );
}
 ...
 */
function hq_convert_hr_to_bytes( $size ) {
 ...
 */
function hq_max_upload_size() {
	$u_bytes = hq_convert_hr_to_bytes( ini_get( 'upload_max_filesize' ) );
	$p_bytes = hq_convert_hr_to_bytes( ini_get( 'post_max_size' ) );
 ...
 */
function hq_image_editor_supports( $args = array() ) {
	return (bool) _hq_image_editor_choose( $args );
}
 ...
 */
function _hq_image_editor_choose( $args = array() ) {
	require_once ABSPATH . HQINC . '/class-hq-image-editor.php';
	require_once ABSPATH . HQINC . '/class-hq-image-editor-gd.php';
	require_once ABSPATH . HQINC . '/class-hq-image-editor-imagick.php';
	 ...
	 *                             'HQ_Image_Editor_Imagick', 'HQ_Image_Editor_GD'.
	 */
	$implementations = apply_filters( 'hq_image_editors', array( 'HQ_Image_Editor_Imagick', 'HQ_Image_Editor_GD' ) );
...
function hq_plupload_default_settings() {
	...
	if ( hq_is_mobile() && strpos( $_SERVER['HTTP_USER_AGENT'], 'OS 7_' ) !== false &&
...
function hq_prepare_attachment_for_js( $attachment ) {
	...
	$response = array(
		'id'          => $attachment->ID,
		'title'       => $attachment->post_title,
		'filename'    => hq_basename( get_attached_file( $attachment->ID ) ),
		'url'         => $attachment_url,
		'link'        => get_attachment_link( $attachment->ID ),
		'alt'         => get_post_meta( $attachment->ID, '_hq_attachment_image_alt', true ),
		...
		'type'        => $type,
		'subtype'     => $subtype,
		'icon'        => hq_mime_type_icon( $attachment->ID ),
...
function hq_enqueue_media( $args = array() ) {
	...
	$settings = array(
		...
		'mimeTypes' => hq_list_pluck( get_post_mime_types(), 0 ),
		/** This filter is documented in hq-admin/includes/media.php */
		'captions'  => ! apply_filters( 'disable_captions', '' ),
		'nonce'     => array(
			'sendToEditor' => hq_create_nonce( 'media-send-to-editor' ),
		),
		'post'    => array(
			'id' => 0,
		),
	...
	if ( isset( $args['post'] ) ) {
		...
		$settings['post'] = array(
			'id' => $post->ID,
			'nonce' => hq_create_nonce( 'update-post_' . $post->ID ),
		);
 ...
 * @param int|HQ_Post $post Optional. Post ID or HQ_Post object. Default is global $post.
 ...
 * @since 0.0.1
 *
 * @param int|HQ_Post $post Post ID or object.
...
function get_post_galleries_images( $post = 0 ) {
	...
	return hq_list_pluck( $galleries, 'src' );
}
...
function attachment_url_to_postid( $url ) {
	...
	$sql = $hqdb->prepare(
		"SELECT post_id FROM $hqdb->postmeta WHERE meta_key = '_hq_attached_file' AND meta_value = %s",
		$path
	);
 ...
 */
function hqview_media_sandbox_styles() {
 	$version = 'ver=' . $GLOBALS['hq_version'];
 	$mediaelement = includes_url( "js/mediaelement/mediaelementplayer.min.css?$version" );
 	$hqmediaelement = includes_url( "js/mediaelement/hq-mediaelement.css?$version" );

	return array( $mediaelement, $hqmediaelement );
}
