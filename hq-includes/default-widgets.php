 ...
 * @since 0.0.1
 */
class HQ_Widget_Pages extends HQ_Widget {
	...
	public function widget( $args, $instance ) {
		 ...
		 */
		$out = hq_list_pages( apply_filters( 'widget_pages_args', array(
			'title_li'    => '',
			'echo'        => 0,
			'sort_column' => $sortby,
			'exclude'     => $exclude
	...
	public function form( $instance ) {
		//Defaults
		$instance = hq_parse_args( (array) $instance, array( 'sortby' => 'post_title', 'title' => '', 'exclude' => '') );
 ...
 * @since 0.0.1
 */
class HQ_Widget_Links extends HQ_Widget {
	...
	public function widget( $args, $instance ) {
		 ...
		 * @since 0.0.1
		 *
		 * @see hq_list_bookmarks()
		 *
		 * @param array $args An array of arguments to retrieve the links list.
		 */
		hq_list_bookmarks( apply_filters( 'widget_links_args', array(
	...
	public function form( $instance ) {
		...
		//Defaults
		$instance = hq_parse_args( (array) $instance, array( 'images' => true, 'name' => true, 'description' => false, 'rating' => false, 'category' => false, 'orderby' => 'name', 'limit' => -1 ) );
 ...
 * @since 0.0.1
 */
class HQ_Widget_Search extends HQ_Widget {
...
<?php
	...
	public function update( $new_instance, $old_instance ) {
		$instance = $old_instance;
		$new_instance = hq_parse_args((array) $new_instance, array( 'title' => ''));
		$instance['title'] = strip_tags($new_instance['title']);
		return $instance;
	}
 ...
 * @since 0.0.1
 */
class HQ_Widget_Archives extends HQ_Widget {
...
?>
		...
		<select id="<?php echo esc_attr( $dropdown_id ); ?>" name="archive-dropdown" onchange='document.location.href=this.options[this.selectedIndex].value;'>
			...
			<?php hq_get_archives( $dropdown_args ); ?>
...
<?php
		 ...
		 */
		hq_get_archives( apply_filters( 'widget_archives_args', array(
			'type'            => 'monthly',
			'show_post_count' => $c
		) ) );
?>
...
<?php
	...
	public function update( $new_instance, $old_instance ) {
		$instance = $old_instance;
		$new_instance = hq_parse_args( (array) $new_instance, array( 'title' => '', 'count' => 0, 'dropdown' => '') );
		$instance['title'] = strip_tags($new_instance['title']);
	 ...
	 */
	public function form( $instance ) {
		$instance = hq_parse_args( (array) $instance, array( 'title' => '', 'count' => 0, 'dropdown' => '') );
 ...
 * @since 0.0.1
 */
class HQ_Widget_Meta extends HQ_Widget {

	public function __construct() {
		$widget_ops = array('classname' => 'widget_meta', 'description' => __( "Login, RSS, &amp; HiveQueen.org links.") );
...
<?php
			/**
			 * Filter the "Powered by HiveQueen" text in the Meta widget.
			 *
			 * @since 0.0.1
			 *
			 * @param string $title_text Default title text for the HiveQueen.org link.
			 */
			echo apply_filters( 'widget_meta_poweredby', sprintf( '<li><a href="%s" title="%s">%s</a></li>',
				esc_url( __( 'https://wordpress.org/' ) ),
				esc_attr__( 'Powered by HiveQueen, state-of-the-art semantic personal publishing platform.' ),
				_x( 'HiveQueen.org', 'meta widget link text' )
			) );
 ...
 * @since 0.0.1
 */
class HQ_Widget_Calendar extends HQ_Widget {
 ...
 * @since 0.0.1
 */
class HQ_Widget_Text extends HQ_Widget {
	...
	public function widget( $args, $instance ) {
		 ...
		 * @param HQ_Widget $instance    HQ_Widget instance.
		 */
		...
		if ( ! empty( $title ) ) {
			...
			<div class="textwidget"><?php echo !empty( $instance['filter'] ) ? hqautop( $text ) : $text; ?></div>
		<?php
		echo $args['after_widget'];
	}
	...
	public function update( $new_instance, $old_instance ) {
		...
		if ( current_user_can('unfiltered_html') )
			...
			$instance['text'] = stripslashes( hq_filter_post_kses( addslashes($new_instance['text']) ) ); // hq_filter_post_kses() expects slashed
	 ...
	 */
	public function form( $instance ) {
		$instance = hq_parse_args( (array) $instance, array( 'title' => '', 'text' => '' ) );
		$title = strip_tags($instance['title']);
 ...
 * @since 0.0.1
 */
class HQ_Widget_Categories extends HQ_Widget {
	...
	public function widget( $args, $instance ) {
		...
		if ( $d ) {
			 ...
			 * @since 0.0.1
			 *
			 * @see hq_dropdown_categories()
			 *
			 * @param array $cat_args An array of Categories widget drop-down arguments.
			 */
			hq_dropdown_categories( apply_filters( 'widget_categories_dropdown_args', $cat_args ) );
?>
...
<?php
		 ...
		 */
		hq_list_categories( apply_filters( 'widget_categories_args', $cat_args ) );
?>
		</ul>
<?php
		}
 ...
 * @since 0.0.1
 */
class HQ_Widget_Recent_Posts extends HQ_Widget {
	...
	public function widget( $args, $instance ) {
		 ...
		 */
		$r = new HQ_Query( apply_filters( 'widget_posts_args', array(
...
<?php
	...
	public function flush_widget_cache() {
		hq_cache_delete('widget_recent_posts', 'widget');
	}
 ...
 * @since 0.0.1
 */
class HQ_Widget_Recent_Comments extends HQ_Widget {
	...
	public function __construct() {
		...
		if ( is_active_widget(false, false, $this->id_base) )
			add_action( 'hq_head', array($this, 'recent_comments_style') );
 ...
 * @since 0.0.1
 */
class HQ_Widget_RSS extends HQ_Widget {
	...
	public function widget( $args, $instance ) {
		...
		hq_widget_rss_output( $rss, $instance );
		echo $args['after_widget'];
	...
	public function update( $new_instance, $old_instance ) {
		...
		return hq_widget_rss_process( $new_instance, $testurl );
	}
 ...
 */
function hq_widget_rss_output( $rss, $args = array() ) {
	if ( is_string( $rss ) ) {
		$rss = fetch_feed($rss);
	...
	foreach ( $rss->get_items( 0, $items ) as $item ) {
		...
		$desc = esc_attr( hq_trim_words( $desc, 55, ' [&hellip;]' ) );
 ...
 */
function hq_widget_rss_form( $args, $inputs = null ) {
	$default_inputs = array( 'url' => true, 'title' => true, 'items' => true, 'show_summary' => true, 'show_author' => true, 'show_date' => true );
	$inputs = hq_parse_args( $inputs, $default_inputs );
 ...
 * @return array
 */
function hq_widget_rss_process( $widget_rss, $check_feed = true ) {
 ...
 * @since 0.0.1
 */
class HQ_Widget_Tag_Cloud extends HQ_Widget {
	...
	public function widget( $args, $instance ) {
		 ...
		 */
		hq_tag_cloud( apply_filters( 'widget_tag_cloud_args', array(
 ...
 * @since 0.0.1
 */
 class HQ_Nav_Menu_Widget extends HQ_Widget {
	...
	public function form( $instance ) {
		...
		<p class="nav-menu-widget-no-menus-message" <?php if ( ! empty( $menus ) ) { echo ' style="display:none" '; } ?>>
			<?php
			if ( isset( $GLOBALS['hq_customize'] ) && $GLOBALS['hq_customize'] instanceof HQ_Customize_Manager ) {
				$url = 'javascript: hq.customize.panel( "nav_menus" ).focus();';
			} else {
...
/**
 * Register all of the default HiveQueen widgets on startup.
 *
 * Calls 'widgets_init' action after all of the HiveQueen widgets have been
 * registered.
 *
 * @since 0.0.1
 */
function hq_widgets_init() {
	...
	register_widget('HQ_Widget_Pages');

	register_widget('HQ_Widget_Calendar');

	register_widget('HQ_Widget_Archives');

	if ( get_option( 'link_manager_enabled' ) )
		register_widget('HQ_Widget_Links');

	register_widget('HQ_Widget_Meta');

	register_widget('HQ_Widget_Search');

	register_widget('HQ_Widget_Text');

	register_widget('HQ_Widget_Categories');

	register_widget('HQ_Widget_Recent_Posts');

	register_widget('HQ_Widget_Recent_Comments');

	register_widget('HQ_Widget_RSS');

	register_widget('HQ_Widget_Tag_Cloud');

	register_widget('HQ_Nav_Menu_Widget');

	/**
	 * Fires after all default HiveQueen widgets have been registered.
	 *
	 * @since 0.0.1
	 */
