<?php
/**
 * HiveQueen Post Thumbnail Template Functions.
 *
 ...
 *
 * @param HQ_Query $hq_query Optional. A HQ_Query instance. Defaults to the $hq_query global.
 */
function update_post_thumbnail_cache( $hq_query = null ) {
	if ( ! $hq_query )
		$hq_query = $GLOBALS['hq_query'];

	if ( $hq_query->thumbnails_cached )
		return;

	$thumb_ids = array();
	foreach ( $hq_query->posts as $post ) {
	...
	$hq_query->thumbnails_cached = true;
}
