 ...
 * @since 0.0.1
 *
 * @global array $hq_cockneyreplace Array of formatted entities for certain common phrases
 ...
 */
function hqtexturize( $text, $reset = false ) {
	global $hq_cockneyreplace, $shortcode_tags;
	...
	if ( $reset || ! isset( $static_characters ) ) {
		/**
		 * Filter whether to skip running hqtexturize().
		 *
		 * Passing false to the filter will effectively short-circuit hqtexturize().
		 * returning the original text passed to the function instead.
		 *
		 * The filter runs only once, the first time hqtexturize() is called.
		 *
		 * @since 0.0.1
		 *
		 * @see hqtexturize()
		 *
		 * @param bool $run_texturize Whether to short-circuit hqtexturize().
		 */
		$run_texturize = apply_filters( 'run_hqtexturize', $run_texturize );
		...
		if ( isset($hq_cockneyreplace) ) {
			$cockney = array_keys( $hq_cockneyreplace );
			$cockneyreplace = array_values( $hq_cockneyreplace );
		} else {
	...
	foreach ( $textarr as &$curl ) {
		// Only call _hqtexturize_pushpop_element if $curl is a delimiter.
		$first = $curl[0];
		if ( '<' === $first && '<!--' === substr( $curl, 0, 4 ) ) {
			...
			_hqtexturize_pushpop_element( $curl, $no_texturize_tags_stack, $no_texturize_tags );
			...
			if ( '[[' !== substr( $curl, 0, 2 ) && ']]' !== substr( $curl, -2 ) ) {
				...
				_hqtexturize_pushpop_element( $curl, $no_texturize_shortcodes_stack, $no_texturize_shortcodes );
			} else {
			...
			if ( false !== strpos( $curl, "'" ) ) {
				...
				$curl = hqtexturize_primes( $curl, "'", $prime, $open_sq_flag, $closing_single_quote );
				$curl = str_replace( $apos_flag, $apos, $curl );
				$curl = str_replace( $open_sq_flag, $opening_single_quote, $curl );
			}
			if ( false !== strpos( $curl, '"' ) ) {
				$curl = preg_replace( $dynamic_characters['quote'], $dynamic_replacements['quote'], $curl );
				$curl = hqtexturize_primes( $curl, '"', $double_prime, $open_q_flag, $closing_quote );
 ...
 */
function hqtexturize_primes( $haystack, $needle, $prime, $open_quote, $close_quote ) {
	$spaces = hq_spaces_regexp();
	$flag = '<!--hq-prime-or-quote-->';
 ...
 */
function _hqtexturize_pushpop_element( $text, &$stack, $disabled_elements ) {
...
function hqautop( $pee, $br = true ) {
	...
	$pee = hq_replace_in_html_tags( $pee, array( "\n" => " <!-- hqnl --> " ) );
	...
	if ( $br ) {
		...
		$pee = str_replace('<HQPreserveNewline />', "\n", $pee);
	}
	...
	if ( false !== strpos( $pee, '<!-- hqnl -->' ) ) {
		$pee = str_replace( array( ' <!-- hqnl --> ', '<!-- hqnl -->' ), "\n", $pee );
	}
 ...
 */
function hq_replace_in_html_tags( $haystack, $replace_pairs ) {
	// Find all elements.
	$textarr = hq_html_split( $haystack );
	$changed = false;
...
/**
 * Newline preservation help function for hqautop
 *
 * @since 0.0.1
 * @access private
 *
 ...
 *                                Also compatible with old _hq_specialchars() values;
 *                                converting single quotes if set to 'single',
 *                                double if set to 'double' or both if otherwise set.
 *                                Default is ENT_NOQUOTES.
 * @return string The decoded text without HTML entities.
 */
function hq_specialchars_decode( $string, $quote_style = ENT_NOQUOTES ) {
	$string = (string) $string;
	...
	// Match the previous behaviour of _hq_specialchars() when the $quote_style is not an accepted value
	if ( empty( $quote_style ) ) {
		$quote_style = ENT_NOQUOTES;
	} elseif ( !in_array( $quote_style, array( 0, 2, 3, 'single', 'double' ), true ) ) {
		$quote_style = ENT_QUOTES;
 ...
 */
function hq_check_invalid_utf8( $string, $strip = false ) {
	$string = (string) $string;
...
function sanitize_file_name( $filename ) {
	...
	$filename .= '.' . $extension;
	/** This filter is documented in hq-includes/formatting.php */
	return apply_filters('sanitize_file_name', $filename, $filename_raw);
}
...
function sanitize_user( $username, $strict = false ) {
	...
	$username = hq_strip_all_tags( $username );
...
function convert_invalid_entities( $content ) {
	...
	if ( strpos( $content, '&#1' ) !== false ) {
		$content = strtr( $content, $hq_htmltranswinuni );
	}
...
function force_balance_tags( $text ) {
	...
	// HQ bug fix for comments - in case you REALLY meant to type '< !--'
	$text = str_replace('< !--', '<    !--', $text);
	// HQ bug fix for LOVE <3 (and other situations with '<' before a number)
	...
	// HQ fix for the bug with HTML comments
 ...
 */
function hq_rel_nofollow( $text ) {
	// This is a pre save filter, so text is already escaped.
	$text = stripslashes($text);
	$text = preg_replace_callback('|<a (.+?)>|i', 'hq_rel_nofollow_callback', $text);
	return hq_slash( $text );
}
 ...
 */
function hq_rel_nofollow_callback( $matches ) {
	$text = $matches[1];
 ...
 * Looks up one smiley code in the $hqsmiliestrans global array and returns an
 * `<img>` string for that smiley.
 *
 * @since 0.0.1
 *
 * @global array $hqsmiliestrans
 *
 * @param array $matches Single match. Smiley code to convert to image.
 * @return string Image string for smiley.
 */
function translate_smiley( $matches ) {
	global $hqsmiliestrans;

	if ( count( $matches ) == 0 )
		return '';

	$smiley = trim( reset( $matches ) );
	$img = $hqsmiliestrans[ $smiley ];
	...
	return sprintf( '<img src="%s" alt="%s" class="hq-smiley" style="height: 1em; max-height: 1em;" />', esc_url( $src_url ), esc_attr( $smiley ) );
}
 ...
 * @since 0.0.1
 *
 * @global string|array $hq_smiliessearch
 *
 * @param string $text Content to convert smilies from text.
 * @return string Converted content with text smilies replaced with images.
 */
function convert_smilies( $text ) {
	global $hq_smiliessearch;
	$output = '';
	if ( get_option( 'use_smilies' ) && ! empty( $hq_smiliessearch ) ) {
		...
		for ( $i = 0; $i < $stop; $i++ ) {
			...
			if ( '' ==  $ignore_block_element && strlen( $content ) > 0 && '<' != $content[0] ) {
				$content = preg_replace_callback( $hq_smiliessearch, 'translate_smiley', $content );
			}
 ...
 */
function hq_iso_descrambler( $string ) {
	/* this may only work with iso-8859-1, I'm afraid */
	if (!preg_match('#\=\?(.+)\?Q\?(.+)\?\=#i', $string, $matches)) {
		return $string;
	} else {
		$subject = str_replace('_', ' ', $matches[2]);
		return preg_replace_callback( '#\=([0-9a-f]{2})#i', '_hq_iso_convert', $subject );
	}
}
 ...
 */
function _hq_iso_convert( $match ) {
...
function hq_trim_excerpt( $text = '' ) {
	...
	if ( '' == $text ) {
		...
		$text = hq_trim_words( $text, $excerpt_length, $excerpt_more );
	}
	/**
	 ...
	 */
	return apply_filters( 'hq_trim_excerpt', $text, $raw_excerpt );
}
 ...
 */
function hq_trim_words( $text, $num_words = 55, $more = null ) {
	if ( null === $more ) {
		$more = __( '&hellip;' );
	}

	$original_text = $text;
	$text = hq_strip_all_tags( $text );
	 ...
	 */
	return apply_filters( 'hq_trim_words', $text, $num_words, $more, $original_text );
}
 ...
 *
 * Usually you should prepare queries using hqdb::prepare().
...
function esc_js( $text ) {
	$safe_text = hq_check_invalid_utf8( $text );
	$safe_text = _hq_specialchars( $safe_text, ENT_COMPAT );
...
function esc_html( $text ) {
	$safe_text = hq_check_invalid_utf8( $text );
	$safe_text = _hq_specialchars( $safe_text, ENT_QUOTES );
	/**
 ...
 */
function hq_make_link_relative( $link ) {
...
function sanitize_option( $option, $value ) {
	...
	switch ( $option ) {
		...
		case 'new_admin_email' :
			$value = $hqdb->strip_invalid_text_for_column( $hqdb->options, 'option_value', $value );
			if ( is_hq_error( $value ) ) {
		...
		case 'siteurl':
			...
			if ( is_hq_error( $value ) ) {
				...
				if ( preg_match( '#http(s?)://(.+)#i', $value ) ) {
					...
					$error = __( 'The HiveQueen address you entered did not appear to be a valid URL. Please enter a valid URL.' );
				}
			}
			break;
 ...
 */
function hq_parse_str( $string, &$array ) {
	 ...
	 */
	$array = apply_filters( 'hq_parse_str', $array );
}
 ...
 * @return string Converted text.
 */
function hq_pre_kses_less_than( $text ) {
	return preg_replace_callback('%<[^>]*?((?=<)|>|$)%', 'hq_pre_kses_less_than_callback', $text);
}
 ...
 */
function hq_pre_kses_less_than_callback( $matches ) {
...
/**
 * HiveQueen implementation of PHP sprintf() with filters.
 *
 * @since 0.0.1
...
function hq_sprintf( $pattern ) {
	...
	while ( $len > $start ) {
		...
		if ( $pattern[$start] == '%' ) {
			...
			/**
			 * Filter a fragment from the pattern passed to hq_sprintf().
			 *
			 ...
			 */
			$_fragment = apply_filters( 'hq_sprintf', $fragment, $arg );
 ...
 */
function hq_sprintf_l( $pattern, $args ) {
	// Not a match
	...
	/**
	 * Filter the translated delimiters used by hq_sprintf_l().
	 ...
	 */
	$l = apply_filters( 'hq_sprintf_l', array(
 ...
 * @return string The excerpt.
 */
function hq_html_excerpt( $str, $count, $more = null ) {
	if ( null === $more )
		$more = '';
	$str = hq_strip_all_tags( $str, true );
...
function _links_add_base( $m ) {
	...
	return $m[1] . '=' . $m[2] .
		( preg_match( '#^(\w{1,20}):#', $m[3], $protocol ) && in_array( $protocol[1], hq_allowed_protocols() ) ?
			$m[3] :
			HQ_HTTP::make_absolute_url( $m[3], $_links_add_base )
		)
		. $m[2];
}
 ...
 * will return 'something'. hq_strip_all_tags will return ''
 *
 * @since 0.0.1
 *
 ...
 */
function hq_strip_all_tags($string, $remove_breaks = false) {
...
function sanitize_text_field( $str ) {
	$filtered = hq_check_invalid_utf8( $str );

	if ( strpos($filtered, '<') !== false ) {
		$filtered = hq_pre_kses_less_than( $filtered );
		// This will strip extra whitespace for us.
		$filtered = hq_strip_all_tags( $filtered, true );
	} else {
 ...
 * @return string
 */
function hq_basename( $path, $suffix = '' ) {
	return urldecode( basename( str_replace( array( '%2F', '%5C' ), '/', urlencode( $path ) ), $suffix ) );
}

/**
 * Forever eliminate "Hivequeen" from the planet (or at least the little bit we can influence).
 *
...
function capital_P_dangit( $text ) {
	...
	if ( 'the_title' === $current_filter || 'hq_title' === $current_filter )
		return str_replace( 'Hivequeen', 'HiveQueen', $text );
	...
	return str_replace(
		array( ' Hivequeen', '&#8216;Hivequeen', $dblq . 'Hivequeen', '>Hivequeen', '(Hivequeen' ),
		array( ' HiveQueen', '&#8216;HiveQueen', $dblq . 'HiveQueen', '>HiveQueen', '(HiveQueen' ),
	$text );
}
...
function hq_spaces_regexp() {
	...
	if ( empty( $spaces ) ) {
		 ...
		 */
		$spaces = apply_filters( 'hq_spaces_regexp', '[\r\n\t ]|\xC2\xA0|&nbsp;' );
	}
...
function print_emoji_detection_script() {
	...
	if ( SCRIPT_DEBUG ) {
		$settings['source'] = array(
			...
			'hqemoji' => apply_filters( 'script_loader_src', includes_url( "js/hq-emoji.js?$version" ), 'hqemoji' ),
			/** This filter is documented in hq-includes/class.hq-scripts.php */
		...
		$settings['source'] = array(
			/** This filter is documented in hq-includes/class.hq-scripts.php */
			'concatemoji' => apply_filters( 'script_loader_src', includes_url( "js/hq-emoji-release.min.js?$version" ), 'concatemoji' ),
		);
 ...
 */
function hq_encode_emoji( $content ) {
 ...
 * @return string The encoded content.
 */
function hq_staticize_emoji( $text ) {
	$text = hq_encode_emoji( $text );
			...
			if ( preg_match_all( '/(&#x1f1(e[6-9a-f]|f[0-9a-f]);){2}/', $content, $matches ) ) {
				if ( ! empty( $matches[0] ) ) {
					foreach ( $matches[0] as $flag ) {
						...
						$entity = sprintf( '<img src="%s" alt="%s" class="hq-smiley" style="height: 1em; max-height: 1em;" />', $cdn_url . $char1 . '-' . $char2 . $ext, html_entity_decode( $flag ) );
			...
			if ( preg_match_all( $regex, $content, $matches ) ) {
				if ( ! empty( $matches[1] ) ) {
					foreach ( $matches[1] as $emoji ) {
						...
						$entity = sprintf( '<img src="%s" alt="%s" class="hq-smiley" style="height: 1em; max-height: 1em;" />', $cdn_url . $char . $ext, html_entity_decode( $emoji ) );
 ...
 */
function hq_staticize_emoji_for_email( $mail ) {
	 ...
	 * in hq_mail() - get the Content-Type from the headers, if there is one,
	 * then pass it through the hq_mail_content_type filter, in case a plugin
	...
	if ( 'text/html' === $content_type ) {
		$mail['message'] = hq_staticize_emoji( $mail['message'] );
	}
