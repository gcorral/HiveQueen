...
function _post_format_link( $link, $term, $taxonomy ) {
	...
	if ( $hq_rewrite->get_extra_permastruct( $taxonomy ) ) {
...
/**
 * Remove the post format prefix from the name property of the term objects created by hq_get_object_terms().
 *
 * @access private
 * @since 0.0.1
 *
 ...
 * @return array
 */
function _post_format_hq_get_object_terms( $terms ) {
	foreach ( (array) $terms as $order => $term ) {
		if ( isset( $term->taxonomy ) && 'post_format' == $term->taxonomy ) {
			$terms[$order]->name = get_post_format_string( str_replace( 'post-format-', '', $term->slug ) );
		}
