 ...
 */
function hq_get_nav_menu_object( $menu ) {
	$menu_obj = false;
	...
	if ( ! $menu_obj || is_hq_error( $menu_obj ) ) {
		$menu_obj = false;
	}

	/**
	 * Filter the nav_menu term retrieved for hq_get_nav_menu_object().
	 *
	 * @since 0.0.1
	 *
	 * @param object|false $menu_obj Term from nav_menu taxonomy, or false if nothing had been found.
	 * @param string       $menu     The menu ID, slug, or name passed to hq_get_nav_menu_object().
	 */
	return apply_filters( 'hq_get_nav_menu_object', $menu_obj, $menu );
}
...
function is_nav_menu( $menu ) {
	...
	$menu_obj = hq_get_nav_menu_object( $menu );

	if (
		$menu_obj &&
		! is_hq_error( $menu_obj ) &&
 ...
 * @since 0.0.1
 *
 * @global array $_hq_registered_nav_menus
 *
 * @param array $locations Associative array of menu location identifiers (like a slug) and descriptive text.
 */
function register_nav_menus( $locations = array() ) {
	global $_hq_registered_nav_menus;

	add_theme_support( 'menus' );

	$_hq_registered_nav_menus = array_merge( (array) $_hq_registered_nav_menus, $locations );
}
...
function unregister_nav_menu( $location ) {
	...
	if ( is_array( $_hq_registered_nav_menus ) && isset( $_hq_registered_nav_menus[$location] ) ) {
		unset( $_hq_registered_nav_menus[$location] );
		if ( empty( $_hq_registered_nav_menus ) ) {
...
function get_registered_nav_menus() {
	global $_hq_registered_nav_menus;
	if ( isset( $_hq_registered_nav_menus ) )
		return $_hq_registered_nav_menus;
	return array();
}
...
function is_nav_menu_item( $menu_item_id = 0 ) {
	return ( ! is_hq_error( $menu_item_id ) && ( 'nav_menu_item' == get_post_type( $menu_item_id ) ) );
}
 ...
 */
function hq_create_nav_menu( $menu_name ) {
	return hq_update_nav_menu_object( 0, array( 'menu-name' => $menu_name ) );
}
 ...
 * @return int|WP_Error Menu ID on success, WP_Error object on failure.
 */
function hq_update_nav_menu_object( $menu_id = 0, $menu_data = array() ) {
	$menu_id = (int) $menu_id;

	$_menu = hq_get_nav_menu_object( $menu_id );
	...
	if (
		$_possible_existing &&
		! is_hq_error( $_possible_existing ) &&
	...
	if ( ! $_menu || is_hq_error( $_menu ) ) {
		$menu_exists = get_term_by( 'name', $menu_data['menu-name'], 'nav_menu' );

		if ( $menu_exists )
			return new WP_Error( 'menu_exists', sprintf( __('The menu name <strong>%s</strong> conflicts with another menu name. Please try another.'), esc_html( $menu_data['menu-name'] ) ) );

		$_menu = hq_insert_term( $menu_data['menu-name'], 'nav_menu', $args );

		if ( is_hq_error( $_menu ) )
			return $_menu;
		 ...
		 */
		do_action( 'hq_create_nav_menu', $_menu['term_id'], $menu_data );
	...
	$update_response = hq_update_term( $menu_id, 'nav_menu', $args );

	if ( is_hq_error( $update_response ) )
	 ...
	 * @param array $menu_data An array of menu data.
	 */
	do_action( 'hq_update_nav_menu', $menu_id, $menu_data );
	return $menu_id;
}
 ...
 */
function hq_get_nav_menus( $args = array() ) {
	 ...
	 */
	return apply_filters( 'hq_get_nav_menus', get_terms( 'nav_menu',  $args), $args );
}
...
function hq_setup_nav_menu_item( $menu_item ) {
	if ( isset( $menu_item->post_type ) ) {
		if ( 'nav_menu_item' == $menu_item->post_type ) {
			...
			if ( 'post_type' == $menu_item->type ) {
				...
				$menu_item->url = !is_hq_error( $term_url ) ? $term_url : '';

				$original_title = get_term_field( 'name', $menu_item->object_id, $menu_item->object, 'raw' );
				if ( is_hq_error( $original_title ) )
			...
			if ( ! isset( $menu_item->description ) ) {
				 ...
				 */
				$menu_item->description = apply_filters( 'nav_menu_description', hq_trim_words( $menu_item->post_content, 200 ) );
			}
			...
			/** This filter is documented in hq-includes/nav-menu.php */
	 ...
	 */
	return apply_filters( 'hq_setup_nav_menu_item', $menu_item );
}
 ...
 */
function hq_get_associated_nav_menu_items( $object_id = 0, $object_type = 'post_type', $taxonomy = '' ) {
 ...
 *
 */
function _hq_delete_post_menu_item( $object_id = 0 ) {
	$object_id = (int) $object_id;

	$menu_item_ids = hq_get_associated_nav_menu_items( $object_id, 'post_type' );
 ...
 */
function _hq_auto_add_pages_to_menu( $new_status, $old_status, $post ) {
	...
	foreach ( $auto_add as $menu_id ) {
		$items = hq_get_nav_menu_items( $menu_id, array( 'post_status' => 'publish,draft' ) );
		if ( ! is_array( $items ) )
			continue;
		foreach ( $items as $item ) {
		...
		hq_update_nav_menu_item( $menu_id, 0, $args );
	}
}
