...
function add_rewrite_rule($regex, $redirect, $after = 'bottom') {
	global $hq_rewrite;
	$hq_rewrite->add_rule($regex, $redirect, $after);
}
...
function add_rewrite_tag( $tag, $regex, $query = '' ) {
	...
	$hq_rewrite->add_rewrite_tag( $tag, $regex, $query );
}
 ...
 *                       see {@link HQ_Rewrite::add_permastruct()} for full details.
 */
function add_permastruct( $name, $struct, $args = array() ) {
	...
	$hq_rewrite->add_permastruct( $name, $struct, $args );
}
...
function add_feed($feedname, $function) {
	global $hq_rewrite;
	if ( ! in_array($feedname, $hq_rewrite->feeds) ) //override the file if it is
		$hq_rewrite->feeds[] = $feedname;
...
function add_rewrite_endpoint( $name, $places, $query_var = true ) {
	global $hq_rewrite;
	$hq_rewrite->add_endpoint( $name, $places, $query_var );
}
 ...
 * @return string
 */
function _hq_filter_taxonomy_base( $base ) {
	if ( !empty( $base ) ) {
 ...
 *
 * Permalinks of posts with numeric slugs can sometimes look to HQ_Query::parse_query()
 ...
 * post permalink.
 *
 * Note that, since 4.3.0, hq_unique_post_slug() prevents the creation of post slugs
 ...
 *                          HQ::parse_request(). Default empty array.
 * @return array Returns the original array of query vars, with date/post conflicts resolved.
 */
function hq_resolve_numeric_slug_conflicts( $query_vars = array() ) {
...
/**
 * HiveQueen Rewrite Component.
 *
 * The HiveQueen Rewrite class writes the rewrite module rules to the .htaccess
 * file. It also handles parsing the request to get the correct setup for the
 * HiveQueen Query class.
 *
 * The Rewrite along with HQ class function as a front controller for HiveQueen.
...
class HQ_Rewrite {
	 ...
	 * {@link HQ_Rewrite::$index} with a trailing slash appended. Otherwise
	 * the root will be empty.
	 *
	 * @see HQ_Rewrite::init()
	 * @see HQ_Rewrite::using_index_permalinks()
	 * @since 0.0.1
	 * @var string
	 */
	public $root = '';
	...
	/**
	 * Rules that don't redirect to HiveQueen' index.php.
	 *
	...
	/**
	 * Whether to write every mod_rewrite rule for HiveQueen into the .htaccess file.
	 *
	 * This is off by default, turning it on might print a lot of rewrite rules
	 * to the .htaccess file.
	 *
	 * @see HQ_Rewrite::mod_rewrite_rules()
	 * @since 0.0.1
	 * @access public
	 * @var bool
	 */
	 ...
	 * set to true. Prior to HiveQueen 3.3 this flag indicated that every page
	 * would have a set of rules added to the top of the rewrite rules array.
	 * Now it tells {@link HQ::parse_request()} to check if a URL matching the
	 ...
	 * {@link HQ_Rewrite::$rewritereplace} and are rewritten to the
	 * query variables listed in {@link HQ_Rewrite::$queryreplace}.
	 *
	 ...
	 * of rewrite tags, see {@link HQ_Rewrite::$rewritecode}.
	 *
	 * @since 0.0.1
	 * @access private
	 * @var array
	...
	/**
	 * Query variables that rewrite tags map to, see {@link HQ_Rewrite::$rewritecode}.
	 *
	 * @since 0.0.1
	 * @access private
	 * @var array
	...
	public function page_uri_index() {
		...
		$pages = $hqdb->get_results("SELECT ID, post_name, post_parent FROM $hqdb->posts WHERE post_type = 'page' AND post_status != 'auto-draft'");
		...
		foreach ( $posts as $id => $post ) {
			// URL => page name
			$uri = get_page_uri($id);
			$attachments = $hqdb->get_results( $hqdb->prepare( "SELECT ID, post_name, post_parent FROM $hqdb->posts WHERE post_type = 'attachment' AND post_parent = %d", $id ));
	 ...
	 *
	 * @see HQ_Rewrite::$rewritecode
	 * @see HQ_Rewrite::$rewritereplace
	 * @see HQ_Rewrite::$queryreplace
	 * @since 0.0.1
	 * @access public
	 *
	 ...
	 *
	 * The main HQ_Rewrite function for building the rewrite rule list. The
	 ...
	 *
	 * Shorten version of {@link HQ_Rewrite::generate_rewrite_rules()} that
	 * allows for shorter list of parameters. See the method for longer
	 * description of what generating rewrite rules does.
	 *
	 * @uses HQ_Rewrite::generate_rewrite_rules() See for long description and rest of parameters.
	 * @since 0.0.1
	 * @access public
	 *
	 ...
	 * reference to the current HQ_Rewrite instance to further manipulate the
	...
	public function rewrite_rules() {
		...
		$deprecated_files = array(
			'.*hq-(atom|rdf|rss|rss2|feed|commentsrss2)\.php$' => $this->index . '?feed=old',
			'.*hq-app\.php(/.*)?$' => $this->index . '?error=403',
		);
		...
		if ( false && is_main_site() ) {
			$registration_pages['.*hq-signup.php$'] = $this->index . '?signup=true';
			$registration_pages['.*hq-activate.php$'] = $this->index . '?activate=true';
		}
		$registration_pages['.*hq-register.php$'] = $this->index . '?register=true'; // Deprecated
		 ...
		 * @since 0.0.1
		 *
		 * @param HQ_Rewrite $this Current HQ_Rewrite instance, passed by reference.
		 */
	 ...
	 * HQ_Rewrite::rewrite_rules()} is that this method stores the rewrite rules
	 ...
	 */
	public function hq_rewrite_rules() {
	 ...
	 *
	 * Will add the non_hq_rules property rules to the .htaccess file before
	 * the HiveQueen rewrite rules one.
	 *
	 * @since 0.0.1
	 * @access public
	 *
	...
	public function mod_rewrite_rules() {
		...
		//add in the rules that don't redirect to HQ's index.php (and thus shouldn't be handled by HQ at all)
		foreach ( (array) $this->non_hq_rules as $match => $query) {
	...
	public function add_rule($regex, $redirect, $after = 'bottom') {
		...
		if ( $front != $this->index ) { //it doesn't redirect to HQ's index.php
	...
	public function add_external_rule($regex, $redirect) {
		$this->non_hq_rules[$regex] = $redirect;
	}
	...
	public function init() {
		$this->extra_rules = $this->non_hq_rules = $this->endpoints = array();
	 ...
	 * {@link HQ_Rewrite::init()} after the option is updated.
	 *
