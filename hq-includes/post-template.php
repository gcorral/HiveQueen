<?php
/**
 * HiveQueen Post Template Functions.
 *
...
/**
 * Display the ID of the current item in the HiveQueen Loop.
 *
 * @since 0.0.1
 */
function the_ID() {
...
/**
 * Retrieve the ID of the current item in the HiveQueen Loop.
 *
 * @since 0.0.1
 *
 * @return int|false The ID of the current item in the HiveQueen Loop. False if $post is not set.
 */
function get_the_ID() {
	$post = get_post();
 ...
 *     @type HQ_Post $post   Current post object to retrieve the title for.
 * }
...
function get_the_title( $post = 0 ) {
	...
	if ( ! is_admin() ) {
		if ( ! empty( $post->post_password ) ) {
			 ...
			 *                         Default 'Protected: %s'.
			 * @param HQ_Post $post    Current post object.
			 */
 ...
 * @since 0.0.1
 *
 * @param int|HQ_Post $id Optional. Post ID or post object.
 */
...
function get_the_content( $more_link_text = null, $strip_teaser = false ) {
	...
	if ( preg_match( '/<!--more(.*?)?-->/', $content, $matches ) ) {
		...
		if ( ! empty( $matches[1] ) && ! empty( $more_link_text ) )
			$more_link_text = strip_tags( hq_kses_no_null( trim( $matches[1] ) ) );
 ...
 * @param int|HQ_Post $post_id Optional. Post ID or post object.
 */
...
function get_post_class( $class = '', $post_id = null ) {
	...
	if ( post_type_supports( $post->post_type, 'post-formats' ) ) {
		...
		if ( $post_format && !is_hq_error($post_format) )
...
function get_body_class( $class = '' ) {
	...
	if ( is_search() ) {
		$classes[] = 'search';
		$classes[] = $hq_query->posts ? 'search-results' : 'search-no-results';
	}
	if ( is_paged() )
		$classes[] = 'paged';
	if ( is_attachment() )
	...
	if ( is_single() ) {
		$post_id = $hq_query->get_queried_object_id();
		$post = $hq_query->get_queried_object();
		...
		if ( is_post_type_archive() ) {
			...
			$author = $hq_query->get_queried_object();
			$classes[] = 'author';
			...
			$cat = $hq_query->get_queried_object();
			$classes[] = 'category';
			...
			$tag = $hq_query->get_queried_object();
			$classes[] = 'tag';
			...
			$term = $hq_query->get_queried_object();
		...
		$page_id = $hq_query->get_queried_object_id();
	...
	$page = $hq_query->get( 'page' );

	if ( ! $page || $page < 2 )
		$page = $hq_query->get( 'paged' );
 ...
 * @since 0.0.1
 *
 * @param int|HQ_Post|null $post An optional post. Global $post used if not provided.
...
function post_password_required( $post = null ) {
	...
	if ( ! isset( $_COOKIE['hq-postpass_' . COOKIEHASH] ) )
		return true;

	require_once ABSPATH . HQINC . '/class-phpass.php';
	$hasher = new PasswordHash( 8, true );

	$hash = hq_unslash( $_COOKIE[ 'hq-postpass_' . COOKIEHASH ] );
...
function hq_link_pages( $args = '' ) {
	...
	$params = hq_parse_args( $args, $defaults );
	 ...
	 */
	$r = apply_filters( 'hq_link_pages_args', $params );
	...
	if ( $multipage ) {
		if ( 'number' == $r['next_or_number'] ) {
			...
			for ( $i = 1; $i <= $numpages; $i++ ) {
				 ...
				 */
				$link = apply_filters( 'hq_link_pages_link', $link, $i );
			...
			$prev = $page - 1;
			if ( $prev ) {
				$link = _hq_link_page( $prev ) . $r['link_before'] . $r['previouspagelink'] . $r['link_after'] . '</a>';

				/** This filter is documented in hq-includes/post-template.php */
				$output .= apply_filters( 'hq_link_pages_link', $link, $prev );
			}
			$next = $page + 1;
			if ( $next <= $numpages ) {
				if ( $prev ) {
				...
				$link = _hq_link_page( $next ) . $r['link_before'] . $r['nextpagelink'] . $r['link_after'] . '</a>';

				/** This filter is documented in hq-includes/post-template.php */
				$output .= apply_filters( 'hq_link_pages_link', $link, $next );
			}
			$output .= $r['after'];
		}
	}
	 ...
	 */
	$html = apply_filters( 'hq_link_pages', $output, $args );
...
/**
 * Helper function for hq_link_pages().
 *
 * @since 0.0.1
 * @access private
 *
...
function _hq_link_page( $i ) {
	...
	if ( 1 == $i ) {
		...
		elseif ( 'page' == get_option('show_on_front') && get_option('page_on_front') == $post->ID )
			$url = trailingslashit(get_permalink()) . user_trailingslashit("$hq_rewrite->pagination_base/" . $i, 'single_paged');
		else
	...
	if ( is_preview() ) {
		...
		if ( ( 'draft' !== $post->post_status ) && isset( $_GET['preview_id'], $_GET['preview_nonce'] ) ) {
			$url = add_query_arg( array(
				'preview_id'    => hq_unslash( $_GET['preview_id'] ),
				'preview_nonce' => hq_unslash( $_GET['preview_nonce'] )
			), $url );
		}
	}
 ...
 */
function hq_dropdown_pages( $args = '' ) {
	$defaults = array(
		'depth' => 0, 'child_of' => 0,
		'selected' => 0, 'echo' => 1,
		'name' => 'page_id', 'id' => '',
	 ...
	 */
	$html = apply_filters( 'hq_dropdown_pages', $output );
...
function hq_list_pages( $args = '' ) {
	 ...
	 */
	$r['exclude'] = implode( ',', apply_filters( 'hq_list_pages_excludes', $exclude_array ) );
	...
	if ( ! empty( $pages ) ) {
		...
		global $hq_query;
		if ( is_page() || is_attachment() || $hq_query->is_posts_page ) {
	 ...
	 */
	$html = apply_filters( 'hq_list_pages', $output, $r );
 ...
 * hq_list_pages()} function. Check that function for more info on those
 * arguments.
 *
 * @since 0.0.1
 *
 * @param array|string $args {
 *     Optional. Arguments to generate a page menu. See hq_list_pages() for additional arguments.
 *
 ...
 */
function hq_page_menu( $args = array() ) {
	 ...
	 */
	$args = apply_filters( 'hq_page_menu_args', $args );
	 ...
	 * @param array  $args An array of arguments.
	 */
	$menu = apply_filters( 'hq_page_menu', $menu, $args );
	if ( $args['echo'] )
		echo $menu;
	else
		return $menu;
...
class Walker_Page extends Walker {
	...
	public function start_el( &$output, $page, $depth = 0, $args = array(), $current_page = 0 ) {
		 ...
		 *                             to each list item.
		 * @param HQ_Post $page         Page data object.
		 * @param int     $depth        Depth of page, used for padding.
		 * @param array   $args         An array of arguments.
...
class Walker_PageDropdown extends Walker {
	 ...
	 *                       element. Uses 'value_field' argument to fill "value" attribute. See {@see hq_dropdown_pages()}.
	 * @param int    $id
	 */
...
function the_attachment_link( $id = 0, $fullsize = false, $deprecated = false, $permalink = false ) {
	...
	if ( $fullsize )
		echo hq_get_attachment_link($id, 'full', $permalink);
	else
		echo hq_get_attachment_link($id, 'thumbnail', $permalink);
}
 ...
 */
function hq_get_attachment_link( $id = 0, $size = 'thumbnail', $permalink = false, $icon = false, $text = false, $attr = '' ) {
	$id = intval( $id );
	$_post = get_post( $id );

	if ( empty( $_post ) || ( 'attachment' != $_post->post_type ) || ! $url = hq_get_attachment_url( $_post->ID ) )
	...
	if ( $text ) {
		...
		$link_text = hq_get_attachment_image( $id, $size, $icon, $attr );
	} else {
		$link_text = '';
	}
	 ...
	 */
	return apply_filters( 'hq_get_attachment_link', "<a href='$url'>$link_text</a>", $id, $size, $permalink, $icon, $text );
}
...
function prepend_attachment($content) {
	...
	if ( hq_attachment_is( 'video', $post ) ) {
		$meta = hq_get_attachment_metadata( get_the_ID() );
		$atts = array( 'src' => hq_get_attachment_url() );
		if ( ! empty( $meta['width'] ) && ! empty( $meta['height'] ) ) {
			$atts['width'] = (int) $meta['width'];
			$atts['height'] = (int) $meta['height'];
		}
		if ( has_post_thumbnail() ) {
			$atts['poster'] = hq_get_attachment_url( get_post_thumbnail_id() );
		}
		$p = hq_video_shortcode( $atts );
	} elseif ( hq_attachment_is( 'audio', $post ) ) {
		$p = hq_audio_shortcode( array( 'src' => hq_get_attachment_url() ) );
	} else {
		$p = '<p class="attachment">';
		// show the medium sized image representation of the attachment if available, and link to the raw file
		$p .= hq_get_attachment_link(0, 'medium', false);
		$p .= '</p>';
	}
...
function get_the_password_form( $post = 0 ) {
	...
	$output = '<form action="' . esc_url( site_url( 'hq-login.php?action=postpass', 'login_post' ) ) . '" class="post-password-form" method="post">
...
function get_page_template_slug( $post_id = null ) {
	...
	if ( ! $post || 'page' != $post->post_type )
		return false;
	$template = get_post_meta( $post->ID, '_hq_page_template', true );
 ...
 */
function hq_post_revision_title( $revision, $link = true ) {
	...
	if ( !hq_is_post_revision( $revision ) )
		$date = sprintf( $currentf, $date );
	elseif ( hq_is_post_autosave( $revision ) )
 ...
 */
function hq_post_revision_title_expanded( $revision, $link = true ) {
	if ( !$revision = get_post( $revision ) )
		return $revision;
 ...
 * @since 0.0.1
 *
 * @param int|HQ_Post $post_id Optional. Post ID or HQ_Post object. Default is global $post.
 * @param string      $type    'all' (default), 'revision' or 'autosave'
 */
function hq_list_post_revisions( $post_id = 0, $type = 'all' ) {
	...
	if ( ! $revisions = hq_get_post_revisions( $post->ID ) )
		return;
	...
	foreach ( $revisions as $revision ) {
		...
		$is_autosave = hq_is_post_autosave( $revision );
		if ( ( 'revision' === $type && $is_autosave ) || ( 'autosave' === $type && ! $is_autosave ) )
			continue;

		$rows .= "\t<li>" . hq_post_revision_title_expanded( $revision ) . "</li>\n";
	}
