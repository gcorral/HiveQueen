...
class Walker_Nav_Menu extends Walker {
	 ...
	 * @param array  $args   An array of arguments. @see hq_nav_menu()
	 */
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		 ...
		 * @param array  $args    An array of {@see hq_nav_menu()} arguments.
		 * @param int    $depth   Depth of menu item. Used for padding.
		 */
...
function hq_nav_menu( $args = array() ) {
	...
	'echo' => true, 'fallback_cb' => 'hq_page_menu', 'before' => '', 'after' => '', 'link_before' => '', 'link_after' => '', 'items_wrap' => '<ul id="%1$s" class="%2$s">%3$s</ul>',
	 ...
	 *  - Otherwise, bail.
	 */
	if ( ( !$menu || is_hq_error($menu) || ( isset($menu_items) && empty($menu_items) && !$args->theme_location ) )
...
function _hq_menu_item_classes_by_context( &$menu_items ) {
	...
	foreach ( (array) $menu_items as $key => $menu_item ) {
		...
		// if the menu item corresponds to the currently-queried post or taxonomy object
			...
			(
				( ! empty( $home_page_id ) && 'post_type' == $menu_item->type && $hq_query->is_home && $home_page_id == $menu_item->object_id ) ||
				( 'post_type' == $menu_item->type && $hq_query->is_singular ) ||
				( 'taxonomy' == $menu_item->type && ( $hq_query->is_category || $hq_query->is_tag || $hq_query->is_tax ) && $queried_object->taxonomy == $menu_item->object )
			)
		) {
