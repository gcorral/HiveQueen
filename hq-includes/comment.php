...
function check_comment($author, $email, $url, $comment, $user_ip, $user_agent, $comment_type) {
	...
	/** This filter is documented in hq-includes/comment-template.php */
	...
	if ( 1 == get_option('comment_whitelist')) {
		if ( 'trackback' != $comment_type && 'pingback' != $comment_type && $author != '' && $email != '' ) {
			...
			$ok_to_comment = $hqdb->get_var("SELECT comment_approved FROM $hqdb->comments WHERE comment_author = '$author' AND comment_author_email = '$email' and comment_approved = '1' LIMIT 1");
			if ( ( 1 == $ok_to_comment ) &&
 ...
 * @param  array $args    Optional. See {@see HQ_Comment_Query::query()} for information
...
function get_comment(&$comment, $output = OBJECT) {
	...
	if ( empty($comment) ) {
		...
		hq_cache_add($comment->comment_ID, $comment, 'comment');
		$_comment = $comment;
	} else {
		if ( isset($GLOBALS['comment']) && ($GLOBALS['comment']->comment_ID == $comment) ) {
			$_comment = & $GLOBALS['comment'];
		} elseif ( ! $_comment = hq_cache_get($comment, 'comment') ) {
			$_comment = $hqdb->get_row($hqdb->prepare("SELECT * FROM $hqdb->comments WHERE comment_ID = %d LIMIT 1", $comment));
			if ( ! $_comment )
				return null;
			hq_cache_add($_comment->comment_ID, $_comment, 'comment');
		}
	}
 ...
 * @since 0.0.1
 *
 * @param string|array $args Optional. Array or string of arguments. See {@see HQ_Comment_Query::parse_query()}
...
/**
 * HiveQueen Comment Query class.
 *
 * See HQ_Comment_Query::__construct() for accepted arguments.
 *
 * @since 0.0.1
 */
class HQ_Comment_Query {
	/**
	 ...
	 *     @type array        $date_query          Date query clauses to limit comments by. See HQ_Date_Query.
	 *                                             Default null.
	 ...
	 *                                             See HQ_Meta_Query. Default empty.
	...
	public function __construct( $query = '' ) {
		$this->query_var_defaults = array(
			...
			'meta_value' => '',
			'meta_query' => '',
			'date_query' => null, // See HQ_Date_Query
		);
	 ...
	 * @access public
	 *
	 * @param string|array $query HQ_Comment_Query arguments. See HQ_Comment_Query::__construct()
	 */
	public function parse_query( $query = '' ) {
		...
		$this->query_vars = hq_parse_args( $query, $this->query_var_defaults );
	...
	protected function parse_orderby( $orderby ) {
		...
		if ( $orderby == $this->query_vars['meta_key'] || $orderby == 'meta_value' ) {
			$parsed = "$hqdb->commentmeta.meta_value";
		} elseif ( $orderby == 'meta_value_num' ) {
			$parsed = "$hqdb->commentmeta.meta_value+0";
...
/**
 * Retrieve all of the HiveQueen supported comment statuses.
 *
...
function get_lastcommentmodified($timezone = 'server') {
	...
	switch ( strtolower($timezone)) {
		case 'gmt':
			$lastcommentmodified = $hqdb->get_var("SELECT comment_date_gmt FROM $hqdb->comments WHERE comment_approved = '1' ORDER BY comment_date_gmt DESC LIMIT 1");
			break;
		case 'blog':
			$lastcommentmodified = $hqdb->get_var("SELECT comment_date FROM $hqdb->comments WHERE comment_approved = '1' ORDER BY comment_date_gmt DESC LIMIT 1");
			break;
		case 'server':
			$lastcommentmodified = $hqdb->get_var($hqdb->prepare("SELECT DATE_ADD(comment_date_gmt, INTERVAL %s SECOND) FROM $hqdb->comments WHERE comment_approved = '1' ORDER BY comment_date_gmt DESC LIMIT 1", $add_seconds_server));
			break;
	}
 ...
 *
 * A lot like {@link hq_count_comments()}, in that they both return comment
 * stats (albeit with different types). The {@link hq_count_comments()} actual
...
function get_comment_count( $post_id = 0 ) {
	...
	$where = '';
	if ( $post_id > 0 ) {
		$where = $hqdb->prepare("WHERE comment_post_ID = %d", $post_id);
	}

	$totals = (array) $hqdb->get_results("
 ...
 * @since 0.0.1
 */
function hq_set_comment_cookies($comment, $user) {
	if ( $user->exists() )
		return;
...
function sanitize_comment_cookies() {
	if ( isset( $_COOKIE['comment_author_' . COOKIEHASH] ) ) {
		 ...
		 *
		 * When this filter hook is evaluated in hq_filter_comment(),
		...
		$comment_author = hq_unslash($comment_author);
	...
	if ( isset( $_COOKIE['comment_author_email_' . COOKIEHASH] ) ) {
		...
		$comment_author_email = hq_unslash($comment_author_email);
	...
	if ( isset( $_COOKIE['comment_author_url_' . COOKIEHASH] ) ) {
		...
		$comment_author_url = hq_unslash($comment_author_url);
 ...
 */
function hq_allow_comment( $commentdata ) {
	global $hqdb;

	// Simple duplicate check
	// expected_slashed ($comment_post_ID, $comment_author, $comment_author_email, $comment_content)
	$dupe = $hqdb->prepare(
		"SELECT comment_ID FROM $hqdb->comments WHERE comment_post_ID = %d AND comment_parent = %s AND comment_approved != 'trash' AND ( comment_author = %s ",
		hq_unslash( $commentdata['comment_post_ID'] ),
		hq_unslash( $commentdata['comment_parent'] ),
		hq_unslash( $commentdata['comment_author'] )
	);
	if ( $commentdata['comment_author_email'] ) {
		$dupe .= $hqdb->prepare(
			"OR comment_author_email = %s ",
			hq_unslash( $commentdata['comment_author_email'] )
		);
	}
	$dupe .= $hqdb->prepare(
		") AND comment_content = %s LIMIT 1",
		hq_unslash( $commentdata['comment_content'] )
	);
	if ( $hqdb->get_var( $dupe ) ) {
		/**
		...
		hq_die( __( 'Duplicate comment detected; it looks as though you&#8217;ve already said that!' ), 409 );
	}
	...
	if ( ! empty( $commentdata['user_id'] ) ) {
		...
		$post_author = $hqdb->get_var( $hqdb->prepare(
			"SELECT post_author FROM $hqdb->posts WHERE ID = %d LIMIT 1",
 ...
 *
 * @param array $comments Optional array of comment objects. Defaults to $hq_query->comments
...
function get_comment_pages_count( $comments = null, $per_page = null, $threaded = null ) {
	...
	if ( null === $comments && null === $per_page && null === $threaded && !empty($hq_query->max_num_comment_pages) )
		return $hq_query->max_num_comment_pages;

	if ( ( ! $comments || ! is_array( $comments ) ) && ! empty( $hq_query->comments )  )
		$comments = $hq_query->comments;
 ...
 */
function hq_blacklist_check($author, $email, $url, $comment, $user_ip, $user_agent) {
	/**
	 ...
	 */
	do_action( 'hq_blacklist_check', $author, $email, $url, $comment, $user_ip, $user_agent );
 ...
 */
function hq_count_comments( $post_id = 0 ) {
	global $hqdb;
	 ...
	 */
	$stats = apply_filters( 'hq_count_comments', array(), $post_id );
	if ( !empty($stats) )
		return $stats;

	$count = hq_cache_get("comments-{$post_id}", 'counts');
	...
	$count = $hqdb->get_results( "SELECT comment_approved, COUNT( * ) AS num_comments FROM {$hqdb->comments} {$where} GROUP BY comment_approved", ARRAY_A );
	...
	$stats = (object) $stats;
	hq_cache_set("comments-{$post_id}", $stats, 'counts');
 ...
 * @return bool True on success, false on failure.
 */
function hq_delete_comment($comment_id, $force_delete = false) {
	global $hqdb;
	if (!$comment = get_comment($comment_id))
		return false;

	if ( !$force_delete && EMPTY_TRASH_DAYS && !in_array( hq_get_comment_status($comment_id), array( 'trash', 'spam' ) ) )
		return hq_trash_comment($comment_id);
	...
	$children = $hqdb->get_col( $hqdb->prepare("SELECT comment_ID FROM $hqdb->comments WHERE comment_parent = %d", $comment_id) );
	if ( !empty($children) ) {
		$hqdb->update($hqdb->comments, array('comment_parent' => $comment->comment_parent), array('comment_parent' => $comment_id));
		clean_comment_cache($children);
	}

	// Delete metadata
	$meta_ids = $hqdb->get_col( $hqdb->prepare( "SELECT meta_id FROM $hqdb->commentmeta WHERE comment_id = %d", $comment_id ) );
	foreach ( $meta_ids as $mid )
		delete_metadata_by_mid( 'comment', $mid );

	if ( ! $hqdb->delete( $hqdb->comments, array( 'comment_ID' => $comment_id ) ) )
		return false;
	...
	if ( $post_id && $comment->comment_approved == 1 )
		hq_update_comment_count($post_id);

	clean_comment_cache($comment_id);

	/** This action is documented in hq-includes/comment.php */
	do_action( 'hq_set_comment_status', $comment_id, 'delete' );

	hq_transition_comment_status('delete', $comment->comment_approved, $comment);
	return true;
}
 ...
 * @return bool True on success, false on failure.
 */
function hq_untrash_comment($comment_id) {
	if ( ! (int)$comment_id )
		return false;
	...
	if ( hq_set_comment_status($comment_id, $status) ) {
		delete_comment_meta($comment_id, '_hq_trash_meta_time');
 ...
 * @return bool True on success, false on failure.
 */
function hq_spam_comment($comment_id) {
	if ( !$comment = get_comment($comment_id) )
		return false;
	...
	if ( hq_set_comment_status($comment_id, 'spam') ) {
		add_comment_meta($comment_id, '_hq_trash_meta_status', $comment->comment_approved);
		/**
 ...
 */
function hq_get_comment_status($comment_id) {
 ...
 */
function hq_transition_comment_status($new_status, $old_status, $comment) {
	/*
	...
	$comment_statuses = array(
		0         => 'unapproved',
		'hold'    => 'unapproved', // hq_set_comment_status() uses "hold"
		1         => 'approved',
		'approve' => 'approved', // hq_set_comment_status() uses "approve"
	);
 ...
 */
function hq_get_current_commenter() {
	 ...
	 */
	return apply_filters( 'hq_get_current_commenter', compact('comment_author', 'comment_author_email', 'comment_author_url') );
}
 ...
 */
function hq_insert_comment( $commentdata ) {
	global $hqdb;
	$data = hq_unslash( $commentdata );
	...
	if ( ! $hqdb->insert( $hqdb->comments, $compacted ) ) {
		return false;
	}

	$id = (int) $hqdb->insert_id;
	 ...
	 * @param obj $comment Comment object.
	 */
	do_action( 'hq_insert_comment', $id, $comment );
 ...
 */
function hq_filter_comment($commentdata) {
	if ( isset( $commentdata['user_ID'] ) ) {
		...
		/** This filter is documented in hq-includes/comment.php */
		$commentdata['user_id'] = apply_filters( 'pre_user_id', $commentdata['user_id'] );
	}
 ...
 */
function hq_throttle_comment_flood($block, $time_lastcomment, $time_newcomment) {
 ...
 * and whether comment is approved by HiveQueen. Also has 'preprocess_comment'
 * filter for processing the comment data before the function handles it.
 *
 * We use REMOTE_ADDR here directly. If you are behind a proxy, you should ensure
 * that it is properly set, such as in hq-config.php, for your environment.
 * See {@link https://core.trac.wordpress.org/ticket/9235}
 *
 * @since 0.0.1
 * @since 0.0.1
 *
 * @see hq_insert_comment()
 *
 * @global hqdb $hqdb
 *
 * @param array $commentdata Contains information on the comment. See hq_insert_comment()
 *                           for information on accepted arguments.
 * @return int|false The ID of the comment on success, false on failure.
 */
function hq_new_comment( $commentdata ) {
	global $hqdb;
	...
	$parent_status = ( 0 < $commentdata['comment_parent'] ) ? hq_get_comment_status($commentdata['comment_parent']) : '';
	...
	$commentdata = hq_filter_comment($commentdata);

	$commentdata['comment_approved'] = hq_allow_comment($commentdata);

	$comment_ID = hq_insert_comment($commentdata);
	if ( ! $comment_ID ) {
		$fields = array( 'comment_author', 'comment_author_email', 'comment_author_url', 'comment_content' );

		foreach( $fields as $field ) {
			if ( isset( $commentdata[ $field ] ) ) {
				$commentdata[ $field ] = $hqdb->strip_invalid_text_for_column( $hqdb->comments, $field, $commentdata[ $field ] );
			}
		}
	...
	if ( 'spam' !== $commentdata['comment_approved'] ) { // If it's spam save it silently for later crunching
		if ( '0' == $commentdata['comment_approved'] ) {
			hq_notify_moderator( $comment_ID );
		}

		// hq_notify_postauthor() checks if notifying the author of their own comment.
		// By default, it won't, but filters can override this.
		if ( get_option( 'comments_notify' ) && $commentdata['comment_approved'] ) {
			hq_notify_postauthor( $comment_ID );
		}
	}
 ...
 *
 * The 'hq_set_comment_status' action is called after the comment is handled.
 ...
 * @param bool $hq_error Whether to return a HQ_Error object if there is a failure. Default is false.
 * @return bool|HQ_Error True on success, false or HQ_Error on failure.
 */
function hq_set_comment_status($comment_id, $comment_status, $hq_error = false) {
	global $hqdb;
	...
	if ( !$hqdb->update( $hqdb->comments, array('comment_approved' => $status), array('comment_ID' => $comment_id) ) ) {
		if ( $hq_error )
			return new HQ_Error('db_update_error', __('Could not update comment status'), $hqdb->last_error);
		else
			return false;
	}
	 ...
	 */
	do_action( 'hq_set_comment_status', $comment_id, $comment_status );

	hq_transition_comment_status($comment_status, $comment_old->comment_approved, $comment);

	hq_update_comment_count($comment->comment_post_ID);
 ...
 * updated without having to call hq_update_comment_count() after.
 *
 * @since 0.0.1
 ...
 * @return bool
 */
function hq_defer_comment_counting($defer=null) {
	static $_defer = false;
	...
	if ( is_bool($defer) ) {
		...
		if ( !$defer )
			hq_update_comment_count( null, true );
	}
 ...
 *
 * @since 0.0.1
 * @see hq_update_comment_count_now() For what could cause a false return value
 *
 ...
 */
function hq_update_comment_count($post_id, $do_deferred=false) {
	static $_deferred = array();

	if ( $do_deferred ) {
		$_deferred = array_unique($_deferred);
		foreach ( $_deferred as $i => $_post_id ) {
			hq_update_comment_count_now($_post_id);
			unset( $_deferred[$i] ); /** @todo Move this outside of the foreach and reset $_deferred to an array instead */
		}
	}

	if ( hq_defer_comment_counting() ) {
		$_deferred[] = $post_id;
		return true;
	}
	elseif ( $post_id ) {
		return hq_update_comment_count_now($post_id);
	}
 ...
 */
function hq_update_comment_count_now($post_id) {
	global $hqdb;
	$post_id = (int) $post_id;
	if ( !$post_id )
		return false;
	...
	$new = (int) $hqdb->get_var( $hqdb->prepare("SELECT COUNT(*) FROM $hqdb->comments WHERE comment_post_ID = %d AND comment_approved = '1'", $post_id) );
	$hqdb->update( $hqdb->posts, array('comment_count' => $new), array('ID' => $post_id) );
	 ...
	 */
	do_action( 'hq_update_comment_count', $post_id, $new, $old );
...
function do_all_pings() {
	...
	// Do pingbacks
	while ($ping = $hqdb->get_row("SELECT ID, post_content, meta_id FROM {$hqdb->posts}, {$hqdb->postmeta} WHERE {$hqdb->posts}.ID = {$hqdb->postmeta}.post_id AND {$hqdb->postmeta}.meta_key = '_pingme' LIMIT 1")) {
		delete_metadata_by_mid( 'post', $ping->meta_id );
		pingback( $ping->post_content, $ping->ID );
	}

	// Do Enclosures
	while ($enclosure = $hqdb->get_row("SELECT ID, post_content, meta_id FROM {$hqdb->posts}, {$hqdb->postmeta} WHERE {$hqdb->posts}.ID = {$hqdb->postmeta}.post_id AND {$hqdb->postmeta}.meta_key = '_encloseme' LIMIT 1")) {
		delete_metadata_by_mid( 'post', $enclosure->meta_id );
		do_enclose( $enclosure->post_content, $enclosure->ID );
	}

	// Do Trackbacks
	$trackbacks = $hqdb->get_col("SELECT ID FROM $hqdb->posts WHERE to_ping <> '' AND post_status = 'publish'");
...
function do_trackbacks($post_id) {
	...
	if ( empty($to_ping) ) {
		$hqdb->update($hqdb->posts, array('to_ping' => ''), array('ID' => $post_id) );
		return;
	}
	...
	$excerpt = hq_html_excerpt($excerpt, 252, '&#8230;');
	...
	if ( $to_ping ) {
		foreach ( (array) $to_ping as $tb_ping ) {
			...
			if ( !in_array($tb_ping, $pinged) ) {
				...
				$hqdb->query( $hqdb->prepare("UPDATE $hqdb->posts SET to_ping = TRIM(REPLACE(to_ping, %s, '')) WHERE ID = %d", $tb_ping, $post_id) );
			}
		}
	}
}
...
function weblog_ping($server = '', $path = '') {
	...
	// using a timeout of 3 seconds should be enough to cover slow servers
	$client = new HQ_HTTP_IXR_Client($server, ((!strlen(trim($path)) || ('/' == $path)) ? false : $path));
	$client->timeout = 3;
	$client->useragent .= ' -- HiveQueen/'.$hq_version;
 ...
 *
 * @since 0.0.1
 * @see hq_http_validate_url()
 *
 * @param string $source_uri
 * @return string
 */
function pingback_ping_source_uri( $source_uri ) {
	return (string) hq_http_validate_url( $source_uri );
}
