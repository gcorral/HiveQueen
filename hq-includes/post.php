 ...
 * '_hq_attached_file' and returning it. This is a convenience function to
...
function get_attached_file( $attachment_id, $unfiltered = false ) {
	$file = get_post_meta( $attachment_id, '_hq_attached_file', true );
	// If the file is relative, prepend upload dir.
	if ( $file && 0 !== strpos($file, '/') && !preg_match('|^.:\\\|', $file) && ( ($uploads = hq_upload_dir()) && false === $uploads['error'] ) )
 ...
 * '_hq_attached_file' to store the path of the attachment.
 *
 * @since 0.0.1
 *
 * @param int    $attachment_id Attachment ID.
...
function update_attached_file( $attachment_id, $file ) {
	...
	if ( $file = _hq_relative_upload_path( $file ) )
		return update_post_meta( $attachment_id, '_hq_attached_file', $file );
	else
		return delete_post_meta( $attachment_id, '_hq_attached_file' );
}
 ...
 */
function _hq_relative_upload_path( $path ) {
	$new_path = $path;

	$uploads = hq_upload_dir();
	 ...
	 * @param string $path     Full path to the file.
	 */
	return apply_filters( '_hq_relative_upload_path', $new_path, $path );
}
 ...
 * context of the inner workings of HiveQueen, pages, posts, and attachments
 ...
 * criteria of posts to retrieve. The 'post_type' can be anything, but HiveQueen
 ...
 * @global HQ_Post $post
 *
 * @param int|HQ_Post|null $post   Optional. Post ID or post object. Defaults to global $post.
 * @param string           $output Optional, default is Object. Accepts OBJECT, ARRAY_A, or ARRAY_N.
 *                                 Default OBJECT.
 * @param string           $filter Optional. Type of filter to apply. Accepts 'raw', 'edit', 'db',
 *                                 or 'display'. Default 'raw'.
 * @return HQ_Post|array|null Type corresponding to $output on success or null on failure.
 *                            When $output is OBJECT, a `HQ_Post` instance is returned.
 */
function get_post( $post = null, $output = OBJECT, $filter = 'raw' ) {
	if ( empty( $post ) && isset( $GLOBALS['post'] ) )
		$post = $GLOBALS['post'];

	if ( $post instanceof HQ_Post ) {
		$_post = $post;
	} elseif ( is_object( $post ) ) {
		if ( empty( $post->filter ) ) {
			$_post = sanitize_post( $post, 'raw' );
			$_post = new HQ_Post( $_post );
		} elseif ( 'raw' == $post->filter ) {
			$_post = new HQ_Post( $post );
		} else {
			$_post = HQ_Post::get_instance( $post->ID );
		}
	} else {
		$_post = HQ_Post::get_instance( $post );
	}
...
final class HQ_Post {
	 ...
	 * @return HQ_Post|false Post object, false otherwise.
	 */
	public static function get_instance( $post_id ) {
		...
		return new HQ_Post( $_post );
	}

	/**
	 * Constructor.
	 *
	 * @param HQ_Post|object $post Post object.
	 */
	...
	public function __get( $key ) {
		if ( 'page_template' == $key && $this->__isset( $key ) ) {
			return get_post_meta( $this->ID, '_hq_page_template', true );
		}
		...
		if ( 'post_category' == $key ) {
			...
			return hq_list_pluck( $terms, 'term_id' );
		}
		...
		if ( 'tags_input' == $key ) {
			...
			return hq_list_pluck( $terms, 'name' );
		}
	 ...
	 * @return self|array|bool|object|HQ_Post
	 */
 ...
 * @since 0.0.1
 *
 * @param int|HQ_Post $post Post ID or post object.
 ...
 * @since 0.0.1
 *
 * @param int|HQ_Post $ID Optional. Post ID or post object. Default empty.
	...
	if ( 'attachment' == $post->post_type ) {
		...
		if ( $post->post_parent && ( $post->ID != $post->post_parent ) ) {
			...
			if ( 'trash' == $parent_post_status ) {
				return get_post_meta( $post->post_parent, '_hq_trash_meta_status', true );
			} else {
...
/**
 * Retrieve all of the HiveQueen supported post statuses.
 *
...
/**
 * Retrieve all of the HiveQueen support page statuses.
 *
 ...
 *
 * @since 0.0.1
 * @global array $hq_post_statuses Inserts new post status object into the list
 *
...
function register_post_status( $post_status, $args = array() ) {
	global $hq_post_statuses;

	if (!is_array($hq_post_statuses))
		$hq_post_statuses = array();
	...
	$hq_post_statuses[$post_status] = $args;
 ...
 * @since 0.0.1
 *
 * @global array $hq_post_statuses List of post statuses.
 *
...
function get_post_status_object( $post_status ) {
	...
	if ( empty($hq_post_statuses[$post_status]) )
		return null;

	return $hq_post_statuses[$post_status];
}
 ...
 *                               properties of the global `$hq_post_statuses objects`. Default empty array.
...
function get_post_stati( $args = array(), $output = 'names', $operator = 'and' ) {
	...
	return hq_filter_object_list($hq_post_statuses, $args, $operator, $field);
}
 ...
 * @since 0.0.1
 *
 * @param int|HQ_Post|null $post Optional. Post ID or post object. Default is global $post.
 ...
 * @since 0.0.1
 *
 * @global array $hq_post_types List of post types.
 *
...
function get_post_type_object( $post_type ) {
	global $hq_post_types;

	if ( empty($hq_post_types[$post_type]) )
		return null;

	return $hq_post_types[$post_type];
}
...
function get_post_types( $args = array(), $output = 'names', $operator = 'and' ) {
	...
	return hq_filter_object_list($hq_post_types, $args, $operator, $field);
}
 ...
 *
 * @global array      $hq_post_types List of post types.
 * @global HQ_Rewrite $hq_rewrite    Used for default feeds.
 * @global HQ         $hq            Used to add query vars.
 *
 ...
 *                                             'dashicons-chart-pie'. Pass 'none' to leave div.hq-menu-image empty
 ...
 *         @type bool   $with_front Whether the permastruct should be prepended with HQ_Rewrite::$front.
 *                                  Default true.
 ...
 * }
 * @return object|HQ_Error The registered post type object, or an error object.
 */
function register_post_type( $post_type, $args = array() ) {
	global $hq_post_types, $hq_rewrite, $hq;

	if ( ! is_array( $hq_post_types ) )
		$hq_post_types = array();
	...
	if ( empty( $post_type ) || strlen( $post_type ) > 20 ) {
		...
		return new HQ_Error( 'post_type_length_invalid', __( 'Post type names must be between 1 and 20 characters in length.' ) );
	}
	...
	if ( false !== $args->query_var && ! empty( $hq ) ) {
		if ( true === $args->query_var )
			$args->query_var = $post_type;
		else
			$args->query_var = sanitize_title_with_dashes( $args->query_var );
		$hq->add_query_var( $args->query_var );
	}
	...
	if ( false !== $args->rewrite && ( is_admin() || '' != get_option( 'permalink_structure' ) ) ) {
		...
		if ( $args->has_archive ) {
			...
			if ( $args->rewrite['with_front'] )
				$archive_slug = substr( $hq_rewrite->front, 1 ) . $archive_slug;
			else
				$archive_slug = $hq_rewrite->root . $archive_slug;

			add_rewrite_rule( "{$archive_slug}/?$", "index.php?post_type=$post_type", 'top' );
			if ( $args->rewrite['feeds'] && $hq_rewrite->feeds ) {
				$feeds = '(' . trim( implode( '|', $hq_rewrite->feeds ) ) . ')';
				add_rewrite_rule( "{$archive_slug}/feed/$feeds/?$", "index.php?post_type=$post_type" . '&feed=$matches[1]', 'top' );
				add_rewrite_rule( "{$archive_slug}/$feeds/?$", "index.php?post_type=$post_type" . '&feed=$matches[1]', 'top' );
			}
			if ( $args->rewrite['pages'] )
				add_rewrite_rule( "{$archive_slug}/{$hq_rewrite->pagination_base}/([0-9]{1,})/?$", "index.php?post_type=$post_type" . '&paged=$matches[1]', 'top' );
		}
 ...
 *             {@link https://codex.wordpress.org/I18n_for_HiveQueen_Developers#Disambiguation_by_context}
 ...
 * @since 0.0.1
 *
 * @global array $_hq_post_type_features
 *
...
function add_post_type_support( $post_type, $feature ) {
	global $_hq_post_type_features;

	$features = (array) $feature;
	foreach ($features as $feature) {
		if ( func_num_args() == 2 )
			$_hq_post_type_features[$post_type][$feature] = true;
		else
			$_hq_post_type_features[$post_type][$feature] = array_slice( func_get_args(), 2 );
	}
}
...
function remove_post_type_support( $post_type, $feature ) {
	...
	unset( $_hq_post_type_features[ $post_type ][ $feature ] );
}
...
function get_all_post_type_supports( $post_type ) {
	...
	if ( isset( $_hq_post_type_features[$post_type] ) )
		return $_hq_post_type_features[$post_type];
...
function post_type_supports( $post_type, $feature ) {
	...
	return ( isset( $_hq_post_type_features[$post_type][$feature] ) );
}
...
function set_post_type( $post_id = 0, $post_type = 'post' ) {
	...
	$return = $hqdb->update( $hqdb->posts, array('post_type' => $post_type), array('ID' => $post_id) );
...
function add_post_meta( $post_id, $meta_key, $meta_value, $unique = false ) {
	...
	if ( $the_post = hq_is_post_revision($post_id) )
		$post_id = $the_post;
 ...
 * @see sanitize_post_field()
 *
 * @param object|HQ_Post|array $post    The Post Object or Array
 * @param string               $context Optional. How to sanitize post fields.
 *                                      Accepts 'raw', 'edit', 'db', or 'display'.
 *                                      Default 'display'.
 * @return object|HQ_Post|array The now sanitized Post Object or Array (will be the
 *                              same type as $post).
 */
...
/**
 * Return the cache key for hq_count_posts() based on the passed arguments.
 *
 * @since 0.0.1
 *
 ...
 */
function hq_count_posts( $type = 'post', $perm = '' ) {
	global $hqdb;
	...
	$counts = hq_cache_get( $cache_key, 'counts' );
	if ( false !== $counts ) {
		/** This filter is documented in hq-includes/post.php */
		return apply_filters( 'hq_count_posts', $counts, $type, $perm );
	}

	$query = "SELECT post_status, COUNT( * ) AS num_posts FROM {$hqdb->posts} WHERE post_type = %s";
	if ( 'readable' == $perm && is_user_logged_in() ) {
		$post_type_object = get_post_type_object($type);
		if ( ! current_user_can( $post_type_object->cap->read_private_posts ) ) {
			$query .= $hqdb->prepare( " AND (post_status != 'private' OR ( post_author = %d AND post_status = 'private' ))",
				get_current_user_id()
			);
		}
	}
	...
	$results = (array) $hqdb->get_results( $hqdb->prepare( $query, $type ), ARRAY_A );
	...
	hq_cache_set( $cache_key, $counts, 'counts' );
 ...
 */
function hq_count_attachments( $mime_type = '' ) {
	global $hqdb;

	$and = hq_post_mime_type_where( $mime_type );
	$count = $hqdb->get_results( "SELECT post_mime_type, COUNT( * ) AS num_posts FROM $hqdb->posts WHERE post_type = 'attachment' AND post_status != 'trash' $and GROUP BY post_mime_type", ARRAY_A );

	$counts = array();
	foreach( (array) $count as $row ) {
		$counts[ $row['post_mime_type'] ] = $row['num_posts'];
	}
	$counts['trash'] = $hqdb->get_var( "SELECT COUNT( * ) FROM $hqdb->posts WHERE post_type = 'attachment' AND post_status = 'trash' $and");
	 ...
	 *                          counted.
	 */
	return apply_filters( 'hq_count_attachments', (object) $counts, $mime_type );
}
 ...
 */
function hq_match_mime_types( $wildcard_mime_types, $real_mime_types ) {
	$matches = array();
 ...
 */
function hq_post_mime_type_where( $post_mime_types, $table_alias = '' ) {
	$where = '';
	$wildcards = array('', '%', '%/%');
 ...
 * @return false|array|HQ_Post|null Post data array, otherwise false.
 */
function hq_trash_post( $post_id = 0 ) {
	if ( !EMPTY_TRASH_DAYS )
		return hq_delete_post($post_id, true);
	 ...
	 * @param int $post_id Post ID.
	 */
	do_action( 'hq_trash_post', $post_id );

	add_post_meta($post_id,'_hq_trash_meta_status', $post['post_status']);
	add_post_meta($post_id,'_hq_trash_meta_time', time());

	$post['post_status'] = 'trash';
	hq_insert_post($post);

	hq_trash_post_comments($post_id);
 ...
 *
 * @param int $post_id Optional. Post ID. Default is ID of the global $post.
 * @return HQ_Post|false HQ_Post object. False on failure.
 */
function hq_untrash_post( $post_id = 0 ) {
	if ( !$post = get_post($post_id, ARRAY_A) )
		return $post;
	...
	$post_status = get_post_meta($post_id, '_hq_trash_meta_status', true);
	...
	hq_untrash_post_comments($post_id);
 ...
 */
function hq_trash_post_comments( $post = null ) {
	global $hqdb;
	...
	$comments = $hqdb->get_results( $hqdb->prepare("SELECT comment_ID, comment_approved FROM $hqdb->comments WHERE comment_post_ID = %d", $post_id) );
	if ( empty($comments) )
		return;
	...
	add_post_meta($post_id, '_hq_trash_meta_comments_status', $statuses);

	// Set status for all comments to post-trashed.
	$result = $hqdb->update($hqdb->comments, array('comment_approved' => 'post-trashed'), array('comment_post_ID' => $post_id));
 ...
 * @return true|void
 */
function hq_untrash_post_comments( $post = null ) {
	global $hqdb;
	...
	$statuses = get_post_meta($post_id, '_hq_trash_meta_comments_status', true);
	...
	foreach ( $group_by_status as $status => $comments ) {
		...
		$hqdb->query( $hqdb->prepare( "UPDATE $hqdb->comments SET comment_approved = %s WHERE comment_ID IN ($comments_in)", $status ) );
	}

	clean_comment_cache( array_keys($statuses) );

	delete_post_meta($post_id, '_hq_trash_meta_comments_status');
 ...
 */
function hq_get_post_categories( $post_id = 0, $args = array() ) {
	$post_id = (int) $post_id;

	$defaults = array('fields' => 'ids');
	$args = hq_parse_args( $args, $defaults );

	$cats = hq_get_object_terms($post_id, 'category', $args);
	return $cats;
}
 ...
 */
function hq_get_post_tags( $post_id = 0, $args = array() ) {
	return hq_get_post_terms( $post_id, 'post_tag', $args);
}
 ...
 * @param array  $args     Optional. {@link hq_get_object_terms()} arguments. Default empty array.
 * @return array|HQ_Error  List of post terms or empty array if no terms were found. HQ_Error object
 *                         if `$taxonomy` doesn't exist.
 */
function hq_get_post_terms( $post_id = 0, $taxonomy = 'post_tag', $args = array() ) {
	$post_id = (int) $post_id;

	$defaults = array('fields' => 'all');
	$args = hq_parse_args( $args, $defaults );

	$tags = hq_get_object_terms($post_id, $taxonomy, $args);
 ...
 */
function hq_get_recent_posts( $args = array(), $output = ARRAY_A ) {
...
function hq_publish_post( $post ) {
	...
	$hqdb->update( $hqdb->posts, array( 'post_status' => 'publish' ), array( 'ID' => $post->ID ) );

	clean_post_cache( $post->ID );

	$old_status = $post->post_status;
	$post->post_status = 'publish';
	hq_transition_post_status( 'publish', $old_status, $post );
	...
	/** This action is documented in hq-includes/post.php */
	do_action( 'hq_insert_post', $post->ID, $post, true );
}
 ...
 * @since 0.0.1
 *
 * @param int|HQ_Post $post_id Post ID or post object.
 */
function check_and_publish_future_post( $post_id ) {
	...
	if ( $time > time() ) {
		hq_clear_scheduled_hook( 'publish_future_post', array( $post_id ) ); // clear anything else in the system
		hq_schedule_single_event( $time, 'publish_future_post', array( $post_id ) );
		return;
	}

	// hq_publish_post(_ returns no meaninful value
	hq_publish_post( $post_id );
}
 ...
 */
function hq_unique_post_slug( $slug, $post_ID, $post_status, $post_type, $post_parent ) {
	if ( in_array( $post_status, array( 'draft', 'pending', 'auto-draft' ) ) || ( 'inherit' == $post_status && 'revision' == $post_type ) )
		return $slug;

	global $hqdb, $hq_rewrite;

	$original_slug = $slug;

	$feeds = $hq_rewrite->feeds;
	if ( ! is_array( $feeds ) )
		$feeds = array();

	if ( 'attachment' == $post_type ) {
		// Attachment slugs must be unique across all types.
		$check_sql = "SELECT post_name FROM $hqdb->posts WHERE post_name = %s AND ID != %d LIMIT 1";
		$post_name_check = $hqdb->get_var( $hqdb->prepare( $check_sql, $slug, $post_ID ) );
		 ...
		 */
		if ( $post_name_check || in_array( $slug, $feeds ) || apply_filters( 'hq_unique_post_slug_is_bad_attachment_slug', false, $slug ) ) {
			$suffix = 2;
			do {
				$alt_post_name = _truncate_post_slug( $slug, 200 - ( strlen( $suffix ) + 1 ) ) . "-$suffix";
				$post_name_check = $hqdb->get_var( $hqdb->prepare( $check_sql, $alt_post_name, $post_ID ) );
				$suffix++;
		 ...
		 */
		$check_sql = "SELECT post_name FROM $hqdb->posts WHERE post_name = %s AND post_type IN ( %s, 'attachment' ) AND ID != %d AND post_parent = %d LIMIT 1";
		$post_name_check = $hqdb->get_var( $hqdb->prepare( $check_sql, $slug, $post_type, $post_ID, $post_parent ) );
		 ...
		 * @param int    $post_parent Post parent ID.
		 */
		if ( $post_name_check || in_array( $slug, $feeds ) || preg_match( "@^($hq_rewrite->pagination_base)?\d+$@", $slug )  || apply_filters( 'hq_unique_post_slug_is_bad_hierarchical_slug', false, $slug, $post_type, $post_parent ) ) {
			$suffix = 2;
			do {
				$alt_post_name = _truncate_post_slug( $slug, 200 - ( strlen( $suffix ) + 1 ) ) . "-$suffix";
				$post_name_check = $hqdb->get_var( $hqdb->prepare( $check_sql, $alt_post_name, $post_type, $post_ID, $post_parent ) );
				$suffix++;
			} while ( $post_name_check );
			$slug = $alt_post_name;
		}
		...
		$check_sql = "SELECT post_name FROM $hqdb->posts WHERE post_name = %s AND post_type = %s AND ID != %d LIMIT 1";
		$post_name_check = $hqdb->get_var( $hqdb->prepare( $check_sql, $slug, $post_type, $post_ID ) );
		 ...
		 * @param string $post_type Post type.
		 */
		if ( $post_name_check || in_array( $slug, $feeds ) || $conflicts_with_date_archive || apply_filters( 'hq_unique_post_slug_is_bad_flat_slug', false, $slug, $post_type ) ) {
			$suffix = 2;
			do {
				$alt_post_name = _truncate_post_slug( $slug, 200 - ( strlen( $suffix ) + 1 ) ) . "-$suffix";
				$post_name_check = $hqdb->get_var( $hqdb->prepare( $check_sql, $alt_post_name, $post_type, $post_ID ) );
				$suffix++;
			} while ( $post_name_check );
			$slug = $alt_post_name;
		}
	 ...
	 */
	return apply_filters( 'hq_unique_post_slug', $slug, $post_ID, $post_status, $post_type, $post_parent, $original_slug );
}
 ...
 * @return array|false|HQ_Error Will return false if $post_id is not an integer or is 0.
 */
function hq_add_post_tags( $post_id = 0, $tags = '' ) {
	return hq_set_post_tags($post_id, $tags, true);
}
 ...
 */
function hq_set_post_tags( $post_id = 0, $tags = '', $append = false ) {
	return hq_set_post_terms( $post_id, $tags, 'post_tag', $append);
}
 ...
 */
function hq_transition_post_status( $new_status, $old_status, $post ) {
	/**
...
function add_ping( $post_id, $uri ) {
	global $hqdb;
	$pung = $hqdb->get_var( $hqdb->prepare( "SELECT pinged FROM $hqdb->posts WHERE ID = %d", $post_id ));
	$pung = trim($pung);
	...
	$new = hq_unslash($new);
	return $hqdb->update( $hqdb->posts, array( 'pinged' => $new ), array( 'ID' => $post_id ) );
}
...
function get_to_ping( $post_id ) {
	global $hqdb;
	$to_ping = $hqdb->get_var( $hqdb->prepare( "SELECT to_ping FROM $hqdb->posts WHERE ID = %d", $post_id ));
...
function trackback_url_list( $tb_list, $post_id ) {
	if ( ! empty( $tb_list ) ) {
		...
		foreach( (array) $trackback_urls as $tb_url ) {
			$tb_url = trim( $tb_url );
			trackback( $tb_url, hq_unslash( $postdata['post_title'] ), $excerpt, $post_id );
		}
	}
}
...
function get_all_page_ids() {
	...
	$page_ids = hq_cache_get('all_page_ids', 'posts');
	if ( ! is_array( $page_ids ) ) {
		$page_ids = $hqdb->get_col("SELECT ID FROM $hqdb->posts WHERE post_type = 'page'");
		hq_cache_add('all_page_ids', $page_ids, 'posts');
	}
 ...
 * @return HQ_Post|array|null HQ_Post on success or null on failure.
 */
 ...
 * @return HQ_Post|array|void HQ_Post on success.
 */
function get_page_by_path( $page_path, $output = OBJECT, $post_type = 'page' ) {
	...
	$pages = $hqdb->get_results( $sql, OBJECT_K );
 ...
 *                                 Default OBJECT.
 * @param string|array $post_type  Optional. Post type or array of post types. Default 'page'.
 * @return HQ_Post|array|void HQ_Post on success or null on failure
 */
 ...
 * @since 0.0.1
 *
 * @param HQ_Post|object|int $page Page object or page ID.
 ...
 */
function hq_get_attachment_metadata( $post_id = 0, $unfiltered = false ) {
	$post_id = (int) $post_id;
	if ( !$post = get_post( $post_id ) )
		return false;

	$data = get_post_meta( $post->ID, '_hq_attachment_metadata', true );
	 ...
	 * @param int        $post_id Attachment ID.
	 */
	return apply_filters( 'hq_get_attachment_metadata', $data, $post->ID );
}
 ...
 */
function hq_update_attachment_metadata( $post_id, $data ) {
	$post_id = (int) $post_id;
	if ( !$post = get_post( $post_id ) )
		return false;
	 ...
	 * @param int   $post_id Attachment ID.
	 */
	if ( $data = apply_filters( 'hq_update_attachment_metadata', $data, $post->ID ) )
		return update_post_meta( $post->ID, '_hq_attachment_metadata', $data );
	else
		return delete_post_meta( $post->ID, '_hq_attachment_metadata' );
}
 ...
 * @since 0.0.1
 * @since 0.0.1
 *              allowed HQ_Post object to be passed.
 *
 * @param int|HQ_Post $post Optional. Attachment ID. Default 0.
 * @return bool Whether the attachment is an image.
 */
function hq_attachment_is_image( $post = 0 ) {
	return hq_attachment_is( 'image', $post );
}
 ...
 * added to the post meta field ('_hq_old_slug') for storing old slugs for that
 * post.
 *
 ...
 *
 * @param int     $post_id     Post ID.
 * @param HQ_Post $post        The Post Object
 * @param HQ_Post $post_before The Previous Post Object
 */
function hq_check_for_changed_slugs( $post_id, $post, $post_before ) {
	...
	$old_slugs = (array) get_post_meta($post_id, '_hq_old_slug');

	// If we haven't added this old slug before, add it now.
	if ( !empty( $post_before->post_name ) && !in_array($post_before->post_name, $old_slugs) )
		add_post_meta($post_id, '_hq_old_slug', $post_before->post_name);

	// If the new slug was used previously, delete it from the list.
	if ( in_array($post->post_name, $old_slugs) )
		delete_post_meta($post_id, '_hq_old_slug', $post->post_name);
}
...
function get_posts_by_author_sql( $post_type, $full = true, $post_author = null, $public_only = false ) {
	...
	if ( null !== $post_author ) {
		$sql .= $hqdb->prepare( ' AND post_author = %d', $post_author );
	}
...
function _get_last_post_time( $timezone, $field ) {
	...
	$date = hq_cache_get( $key, 'timeinfo' );

	if ( !$date ) {
		$add_seconds_server = date('Z');

		$post_types = get_post_types( array( 'public' => true ) );
		array_walk( $post_types, array( &$hqdb, 'escape_by_ref' ) );
		$post_types = "'" . implode( "', '", $post_types ) . "'";

		switch ( $timezone ) {
			case 'gmt':
				$date = $hqdb->get_var("SELECT post_{$field}_gmt FROM $hqdb->posts WHERE post_status = 'publish' AND post_type IN ({$post_types}) ORDER BY post_{$field}_gmt DESC LIMIT 1");
				break;
			case 'blog':
				$date = $hqdb->get_var("SELECT post_{$field} FROM $hqdb->posts WHERE post_status = 'publish' AND post_type IN ({$post_types}) ORDER BY post_{$field}_gmt DESC LIMIT 1");
				break;
			case 'server':
				$date = $hqdb->get_var("SELECT DATE_ADD(post_{$field}_gmt, INTERVAL '$add_seconds_server' SECOND) FROM $hqdb->posts WHERE post_status = 'publish' AND post_type IN ({$post_types}) ORDER BY post_{$field}_gmt DESC LIMIT 1");
				break;
		}

		if ( $date )
			hq_cache_set( $key, $date, 'timeinfo' );
	}
 ...
 *
 * This function will not run if $_hq_suspend_cache_invalidation is not empty.
 *
 * @since 0.0.1
 *
 * @global bool $_hq_suspend_cache_invalidation
...
function clean_attachment_cache( $id, $clean_terms = false ) {
	global $_hq_suspend_cache_invalidation;
	...
	hq_cache_delete($id, 'posts');
	hq_cache_delete($id, 'post_meta');
 ...
 * to the 'hq_insert_post_parent' filter.
 *
 * @since 0.0.1
 *
 * @see hq_find_hierarchy_loop()
 *
 * @param int $post_parent ID of the parent for the post we're checking.
 * @param int $post_ID     ID of the post we're checking.
 * @return int The new post_parent for the post, 0 otherwise.
 */
function hq_check_post_hierarchy_for_loops( $post_parent, $post_ID ) {
	...
	if ( !$loop = hq_find_hierarchy_loop( 'hq_get_post_parent_id', $post_ID, $post_parent ) )
	...
	foreach ( array_keys( $loop ) as $loop_member )
		hq_update_post( array( 'ID' => $loop_member, 'post_parent' => 0 ) );
 ...
 * @since 0.0.1
 *
 * @param int|HQ_Post $post Post ID or post object where thumbnail should be removed from.
 * @return bool True on success, false on failure.
 */
