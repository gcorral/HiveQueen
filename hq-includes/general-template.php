...
function get_header( $name = null ) {
	...
	if ('' == locate_template($templates, true))
		load_template( ABSPATH . HQINC . '/theme-compat/header.php');
}
...
function get_footer( $name = null ) {
	...
	// Backward compat code will be removed in a future release
	if ('' == locate_template($templates, true))
		load_template( ABSPATH . HQINC . '/theme-compat/footer.php');
}
...
function get_sidebar( $name = null ) {
	...
	// Backward compat code will be removed in a future release
	if ('' == locate_template($templates, true))
		load_template( ABSPATH . HQINC . '/theme-compat/sidebar.php');
}
 ...
 * form into the sidebar and also by the search widget in HiveQueen.
 *
 ...
 */
function hq_login_url($redirect = '', $force_reauth = false) {
	$login_url = site_url('hq-login.php', 'login');
...
function hq_registration_url() {
	 ...
	 */
	return apply_filters( 'register_url', site_url( 'hq-login.php?action=register', 'login' ) );
}

/**
 * Provides a simple login form for use anywhere within HiveQueen. By default, it echoes
 ...
 * @return string|void String when retrieving.
 */
function hq_login_form( $args = array() ) {
	$defaults = array(
		'echo' => true,
	 ...
	 */
	$args = hq_parse_args( $args, apply_filters( 'login_form_defaults', $defaults ) );
	...
	$form = '
		<form name="' . $args['form_id'] . '" id="' . $args['form_id'] . '" action="' . esc_url( site_url( 'hq-login.php', 'login_post' ) ) . '" method="post">
			' . $login_form_top . '
			...
			<p class="login-submit">
				<input type="submit" name="hq-submit" id="' . esc_attr( $args['id_submit'] ) . '" class="button-primary" value="' . esc_attr( $args['label_log_in'] ) . '" />
 ...
 */
function hq_lostpassword_url( $redirect = '' ) {
	$args = array( 'action' => 'lostpassword' );
	if ( !empty($redirect) ) {
		$args['redirect_to'] = $redirect;
	}

	$lostpassword_url = add_query_arg( $args, network_site_url('hq-login.php', 'login') );
...
/**
 * Theme container function for the 'hq_meta' action.
 *
 * The 'hq_meta' action can have several purposes, depending on how you use it,
 * but one purpose might have been to allow for theme switching.
 *
 * @since 0.0.1
 *
 * @link https://core.trac.wordpress.org/ticket/1458 Explanation of 'hq_meta' action.
 */
function hq_meta() {
	/**
 ...
 * 2. hqurl - Blog URI path to HiveQueen.
 ...
 */
function hq_title( $sep = '&raquo;', $display = true, $seplocation = '' ) {
	global $hq_locale, $page, $paged;
	...
	$t_sep = '%HQ_TITILE_SEP%'; // Temporary separator, for accurate flipping, if necessary
	...
	if ( is_archive() && !empty($year) ) {
		$title = $year;
		if ( !empty($monthnum) )
			$title .= $t_sep . $hq_locale->get_month($monthnum);
		if ( !empty($day) )
	 ...
	 */
	$title_array = apply_filters( 'hq_title_parts', explode( $t_sep, $title ) );
	 ...
	 */
	$title = apply_filters( 'hq_title', $title, $sep, $seplocation );
 ...
 * for category page title and with less overhead than {@link hq_title()}.
 *
 * It does not support placing the separator after the title, but by leaving the
 * prefix parameter empty, you can set the title separator manually. The prefix
 * does not automatically place a space between the prefix, so if there should
 ...
 * overhead than {@link hq_title()}, because of its limited implementation.
 *
 * It does not support placing the separator after the title, but by leaving the
 * prefix parameter empty, you can set the title separator manually. The prefix
 * does not automatically place a space between the prefix, so if there should
 ...
 * It has less overhead than {@link hq_title()}, because of its limited implementation.
 *
 * It does not support placing the separator after the title, but by leaving the
 * prefix parameter empty, you can set the title separator manually. The prefix
 * does not automatically place a space between the prefix, so if there should
 ...
 * is needed, should be better than {@link hq_title()}.
 *
 * It does not support placing the separator after the title, but by leaving the
 * prefix parameter empty, you can set the title separator manually. The prefix
 * does not automatically place a space between the prefix, so if there should
...
function single_month_title($prefix = '', $display = true ) {
	...
	if ( !empty($monthnum) && !empty($year) ) {
		$my_year = $year;
		$my_month = $hq_locale->get_month($monthnum);
	} elseif ( !empty($m) ) {
		$my_year = substr($m, 0, 4);
		$my_month = $hq_locale->get_month(substr($m, 4, 2));
	}
 ...
 * @return string|void String when retrieving.
 */
function hq_get_archives( $args = '' ) {
	global $hqdb, $hq_locale;
	...
	if ( 'monthly' == $r['type'] ) {
		$query = "SELECT YEAR(post_date) AS `year`, MONTH(post_date) AS `month`, count(ID) as posts FROM $hqdb->posts $join $where GROUP BY YEAR(post_date), MONTH(post_date) ORDER BY post_date $order $limit";
		$key = md5( $query );
		$key = "hq_get_archives:$key:$last_changed";
		if ( ! $results = hq_cache_get( $key, 'posts' ) ) {
			$results = $hqdb->get_results( $query );
			hq_cache_set( $key, $results, 'posts' );
		}
		if ( $results ) {
			$after = $r['after'];
			foreach ( (array) $results as $result ) {
				...
				$text = sprintf( __( '%1$s %2$d' ), $hq_locale->get_month( $result->month ), $result->year );
		...
		$query = "SELECT YEAR(post_date) AS `year`, count(ID) as posts FROM $hqdb->posts $join $where GROUP BY YEAR(post_date) ORDER BY post_date $order $limit";
		$key = md5( $query );
		$key = "hq_get_archives:$key:$last_changed";
		if ( ! $results = hq_cache_get( $key, 'posts' ) ) {
			$results = $hqdb->get_results( $query );
		...
		$query = "SELECT YEAR(post_date) AS `year`, MONTH(post_date) AS `month`, DAYOFMONTH(post_date) AS `dayofmonth`, count(ID) as posts FROM $hqdb->posts $join $where GROUP BY YEAR(post_date), MONTH(post_date), DAYOFMONTH(post_date) ORDER BY post_date $order $limit";
		$key = md5( $query );
		$key = "hq_get_archives:$key:$last_changed";
		if ( ! $results = hq_cache_get( $key, 'posts' ) ) {
			$results = $hqdb->get_results( $query );
		...
		$week = _hq_mysql_week( '`post_date`' );
		$query = "SELECT DISTINCT $week AS `week`, YEAR( `post_date` ) AS `yr`, DATE_FORMAT( `post_date`, '%Y-%m-%d' ) AS `yyyymmdd`, count( `ID` ) AS `posts` FROM `$hqdb->posts` $join $where GROUP BY $week, YEAR( `post_date` ) ORDER BY `post_date` $order $limit";
		$key = md5( $query );
		$key = "hq_get_archives:$key:$last_changed";
		if ( ! $results = hq_cache_get( $key, 'posts' ) ) {
			$results = $hqdb->get_results( $query );
		...
		$query = "SELECT * FROM $hqdb->posts $join $where ORDER BY $orderby $limit";
		$key = md5( $query );
		$key = "hq_get_archives:$key:$last_changed";
		if ( ! $results = hq_cache_get( $key, 'posts' ) ) {
			$results = $hqdb->get_results( $query );
...
function get_calendar($initial = true, $echo = true) {
	...
	$ak_post_titles = $hqdb->get_results("SELECT ID, post_title, DAYOFMONTH(post_date) as dom "
		."FROM $hqdb->posts "
 ...
 *
 * @param  string      $d    Optional. PHP date format defaults to the date_format option if not specified.
 * @param  int|HQ_Post $post Optional. Post ID or HQ_Post object. Default current post.
...
function get_the_date( $d = '', $post = null ) {
	 ...
	 * @param string      $d        PHP date format. Defaults to 'date_format' option
	 *                              if not specified.
	 * @param int|HQ_Post $post     The post object or ID.
	 */
 ...
 * @param int|HQ_Post $post HQ_Post object or ID. Default is global $post object.
...
function get_the_time( $d = '', $post = null ) {
	 ...
	 * @param int|HQ_Post $post     HQ_Post object or ID.
	 */
...
function the_weekday() {
	global $hq_locale;
	$the_weekday = $hq_locale->get_weekday( mysql2date( 'w', get_post()->post_date, false ) );
...
function the_weekday_date($before='',$after='') {
	global $hq_locale, $currentday, $previousweekday;
	$the_weekday_date = '';
	if ( $currentday != $previousweekday ) {
		$the_weekday_date .= $before;
		$the_weekday_date .= $hq_locale->get_weekday( mysql2date( 'w', get_post()->post_date, false ) );
 ...
 */
function rsd_link() {
	echo '<link rel="EditURI" type="application/rsd+xml" title="RSD" href="' . get_bloginfo('hqurl') . "/xmlrpc.php?rsd\" />\n";
}
 ...
 * output to tell web robots not to index the page content. Add this to the hq_head action.
 * Typical usage is as a hq_head callback. add_action( 'hq_head', 'noindex' );
 *
 * @see hq_no_robots
 *
 * @since 0.0.1
 ...
 * Typical usage is as a hq_head callback. add_action( 'hq_head', 'hq_no_robots' );
 *
 * @since 0.0.1
 */
function hq_no_robots() {
...
function hq_default_editor() {
	...
	if ( hq_get_current_user() ) { // look for cookie
 ...
 * _HQ_Editors should not be used directly. See https://core.trac.wordpress.org/ticket/17144.
 *
 * NOTE: Once initialized the TinyMCE editor cannot be safely moved in the DOM. For that reason
 * running hq_editor() inside of a metabox is not a good idea unless only Quicktags is used.
 * On the post edit screen several actions can be used to include additional editors
 * containing TinyMCE: 'edit_page_form', 'edit_form_advanced' and 'dbx_post_sidebar'.
 * See https://core.trac.wordpress.org/ticket/19173 for more information.
 *
 * @see hq-includes/class-hq-editor.php
 * @since 0.0.1
 *
 * @param string $content   Initial content for the editor.
 * @param string $editor_id HTML ID attribute value for the textarea and TinyMCE. Can only be /[a-z]+/.
 * @param array  $settings  See _HQ_Editors::editor().
 */
function hq_editor( $content, $editor_id, $settings = array() ) {
	if ( ! class_exists( '_HQ_Editors' ) )
		require( ABSPATH . HQINC . '/class-hq-editor.php' );

	_HQ_Editors::editor($content, $editor_id, $settings);
}

/**
 * Retrieve the contents of the search HiveQueen query variable.
 *
 ...
 *
 *     hq_admin_css_color( 'classic', __( 'Classic' ), admin_url( "css/colors-classic.css" ), array(
 ...
 */
function hq_admin_css_color( $key, $name, $url, $colors = array(), $icons = array() ) {
	global $_hq_admin_css_colors;

	if ( !isset($_hq_admin_css_colors) )
		$_hq_admin_css_colors = array();

	$_hq_admin_css_colors[$key] = (object) array(
		'name' => $name,
		'url' => $url,
		'colors' => $colors,
		'icon_colors' => $icons,
...
function register_admin_color_schemes() {
	...
	hq_admin_css_color( 'fresh', _x( 'Default', 'admin color scheme' ),
		false,
	...
	if ( false !== strpos( $GLOBALS['hq_version'], '-src' ) )
		return;

	hq_admin_css_color( 'light', _x( 'Light', 'admin color scheme' ),
		admin_url( "css/colors/light/colors$suffix.css" ),
		array( '#e5e5e5', '#999', '#d64e07', '#04a4cc' ),
		array( 'base' => '#999', 'focus' => '#ccc', 'current' => '#ccc' )
	);

	hq_admin_css_color( 'blue', _x( 'Blue', 'admin color scheme' ),
		admin_url( "css/colors/blue/colors$suffix.css" ),
		array( '#096484', '#4796b3', '#52accc', '#74B6CE' ),
		array( 'base' => '#e5f8ff', 'focus' => '#fff', 'current' => '#fff' )
	);

	hq_admin_css_color( 'midnight', _x( 'Midnight', 'admin color scheme' ),
		admin_url( "css/colors/midnight/colors$suffix.css" ),
		array( '#25282b', '#363b3f', '#69a8bb', '#e14d43' ),
		array( 'base' => '#f1f2f3', 'focus' => '#fff', 'current' => '#fff' )
	);

	hq_admin_css_color( 'sunrise', _x( 'Sunrise', 'admin color scheme' ),
		admin_url( "css/colors/sunrise/colors$suffix.css" ),
		array( '#b43c38', '#cf4944', '#dd823b', '#ccaf0b' ),
		array( 'base' => '#f3f1f1', 'focus' => '#fff', 'current' => '#fff' )
	);

	hq_admin_css_color( 'ectoplasm', _x( 'Ectoplasm', 'admin color scheme' ),
		admin_url( "css/colors/ectoplasm/colors$suffix.css" ),
		array( '#413256', '#523f6d', '#a3b745', '#d46f15' ),
		array( 'base' => '#ece6f6', 'focus' => '#fff', 'current' => '#fff' )
	);

	hq_admin_css_color( 'ocean', _x( 'Ocean', 'admin color scheme' ),
		admin_url( "css/colors/ocean/colors$suffix.css" ),
		array( '#627c83', '#738e96', '#9ebaa0', '#aa9d88' ),
		array( 'base' => '#f2fcff', 'focus' => '#fff', 'current' => '#fff' )
	);

	hq_admin_css_color( 'coffee', _x( 'Coffee', 'admin color scheme' ),
...
/**
 * Display the URL of a HiveQueen admin CSS file.
 *
 * @see HQ_Styles::_css_href and its style_loader_src filter.
 *
 * @since 0.0.1
 *
 * @param string $file file relative to hq-admin/ without its ".css" extension.
 * @return string
 */
function hq_admin_css_uri( $file = 'hq-admin' ) {
	...
	/**
	 * Filter the URI of a HiveQueen admin CSS file.
	 *
	 * @since 0.0.1
	 *
	 ...
	 */
	return apply_filters( 'hq_admin_css_uri', $_file, $file );
}
...
function hq_admin_css( $file = 'hq-admin', $force_echo = false ) {
	...
	if ( hq_styles()->query( $handle ) ) {
		if ( $force_echo || did_action( 'hq_print_styles' ) ) // we already printed the style queue. Print this one immediately
...
/**
 * Display the XHTML generator that is generated on the hq_head hook.
 *
 * @since 0.0.1
 */
function hq_generator() {
	/**
	 ...
	 */
	the_generator( apply_filters( 'hq_generator_type', 'xhtml' ) );
}
...
function get_the_generator( $type = '' ) {
	...
	switch ( $type ) {
		case 'html':
			$gen = '<meta name="generator" content="HiveQueen ' . get_bloginfo( 'version' ) . '">';
			break;
		case 'xhtml':
			$gen = '<meta name="generator" content="HiveQueen ' . get_bloginfo( 'version' ) . '" />';
			break;
		case 'atom':
			$gen = '<generator uri="http://wordpress.org/" version="' . get_bloginfo_rss( 'version' ) . '">HiveQueen</generator>';
			break;
		case 'rss2':
		...
		case 'rdf':
			...
			break;
		case 'comment':
			$gen = '<!-- generator="HiveQueen/' . get_bloginfo( 'version' ) . '" -->';
			break;
		case 'export':
			$gen = '<!-- generator="HiveQueen/' . get_bloginfo_rss('version') . '" created="'. date('Y-m-d H:i') . '" -->';
			break;
	}
	 ...
	 * @since 0.0.1
	 *
	 * @param string $gen  The HTML markup output to {@see hq_head()}.
