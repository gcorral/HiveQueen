 ...
 *
 * Standardizes the HTTP requests for HiveQueen. Handles cookies, gzip encoding and decoding, chunk
...
/**
 * HiveQueen HTTP Class for managing HTTP Transports and making HTTP requests.
 *
...
class HQ_Http {
	 ...
	 *                                             Default HiveQueen/' . $hq_version . '; ' . get_bloginfo( 'url' ).
	 *     @type bool         $reject_unsafe_urls  Whether to pass URLs through {@see hq_http_validate_url()}.
	 *                                             Default false.
	 ...
	 *                                             Default ABSPATH . HQINC . '/certificates/ca-bundle.crt'.
	 *     @type bool         $stream              Whether to stream to a file. If set to true and no filename was
	 *                                             given, it will be droped it in the HQ temp dir and its name will
	 ...
	 *
	 * }
	 * @return array|HQ_Error Array containing 'headers', 'body', 'response', 'cookies', 'filename'.
	 *                        A HQ_Error instance upon error.
	 */
	public function request( $url, $args = array() ) {
		...
		$defaults = array(
			 ...
			 * @since 0.0.1
			 *
			 * @param string $user_agent HiveQueen user agent string.
			 */
			'user-agent' => apply_filters( 'http_headers_useragent', 'HiveQueen/' . $hq_version . '; ' . get_bloginfo( 'url' ) ),
			/**
			 * Filter whether to pass URLs through hq_http_validate_url() in an HTTP request.
			 *
			 * @since 0.0.1
			 *
			 * @param bool $pass_url Whether to pass URLs through hq_http_validate_url().
			 *                       Default false.
			 */
			...
			'decompress' => true,
			'sslverify' => true,
			'sslcertificates' => ABSPATH . HQINC . '/certificates/ca-bundle.crt',
			'stream' => false,
			'filename' => null,
		...
		if ( function_exists( 'hq_kses_bad_protocol' ) ) {
			if ( $r['reject_unsafe_urls'] )
				$url = hq_http_validate_url( $url );
			if ( $url ) {
				$url = hq_kses_bad_protocol( $url, array( 'http', 'https', 'ssl' ) );
			}
		}
		...
		if ( empty( $url ) || empty( $arrURL['scheme'] ) )
			return new HQ_Error('http_request_failed', __('A valid URL was not provided.'));

		if ( $this->block_request( $url ) )
			return new HQ_Error( 'http_request_failed', __( 'User has blocked requests through HTTP.' ) );
		...
		// Determine if this request is to OUR install of HiveQueen.
		$homeURL = parse_url( get_bloginfo( 'url' ) );
		$r['local'] = 'localhost' == $arrURL['host'] || ( isset( $homeURL['host'] ) && $homeURL['host'] == $arrURL['host'] );
		unset( $homeURL );

		/*
		 * If we are streaming to a file but no filename was given drop it in the HQ temp dir
		 * and pick its name using the basename of the $url.
		 */
		if ( $r['stream']  && empty( $r['filename'] ) ) {
			$r['filename'] = get_temp_dir() . hq_unique_filename( get_temp_dir(), basename( $url ) );
		}
		...
		if ( $r['stream'] ) {
			$r['blocking'] = true;
			if ( ! hq_is_writable( dirname( $r['filename'] ) ) )
				return new HQ_Error( 'http_request_failed', __( 'Destination directory for file streaming does not exist or is not writable.' ) );
		}
		...
		if ( ! isset( $r['headers']['Accept-Encoding'] ) ) {
			if ( $encoding = HQ_Http_Encoding::accept_encoding( $url, $r ) )
		...
		if ( ! empty( $r['cookies'] ) ) {
			$cookies_set = hq_list_pluck( $response['cookies'], 'name' );
	 ...
	 * @param string $url URL to Request
	 * @param array $args Request arguments
	 * @return array|HQ_Error Array containing 'headers', 'body', 'response', 'cookies', 'filename'. A HQ_Error instance upon error
	 */
	private function _dispatch_request( $url, $args ) {
		...
		if ( !$class )
			return new HQ_Error( 'http_failure', __( 'There are no HTTP transports available which can complete the requested request.' ) );
		 ...
		 * @since 0.0.1
		 *
		 * @param array|HQ_Error $response HTTP response or HQ_Error object.
	...
	public static function processHeaders( $headers, $url = '' ) {
		...
		foreach ( (array) $headers as $tempheader ) {
			...
			if ( 'set-cookie' == $key )
				$cookies[] = new HQ_Http_Cookie( $value, $url );
		}
	 ...
	 *
	 * If it's found, then it upgrades any basic name => value pairs to HQ_Http_Cookie instances,
	...
	public static function buildCookieHeader( &$r ) {
		if ( ! empty($r['cookies']) ) {
			// Upgrade any name => value cookie pairs to HQ_HTTP_Cookie instances.
			foreach ( $r['cookies'] as $name => $value ) {
				if ( ! is_object( $value ) )
					$r['cookies'][ $name ] = new HQ_HTTP_Cookie( array( 'name' => $name, 'value' => $value ) );
			}
	 ...
	 *
	 * You block external URL requests by defining HQ_HTTP_BLOCK_EXTERNAL as true in your hq-config.php
	 * file and this will only allow localhost and your blog to make requests. The constant
	 * HQ_ACCESSIBLE_HOSTS will allow additional hosts to go through for requests. The format of the
	 * HQ_ACCESSIBLE_HOSTS constant is a comma separated list of hostnames to allow, wildcard domains
	 *
	 * @since 0.0.1
	 *
	...
	public function block_request($uri) {
		...
		if ( ! defined( 'HQ_HTTP_BLOCK_EXTERNAL' ) || ! HQ_HTTP_BLOCK_EXTERNAL )
			return false;
		...
		if ( !defined('HQ_ACCESSIBLE_HOSTS') )
			return true;

		static $accessible_hosts = null;
		static $wildcard_regex = array();
		if ( null === $accessible_hosts ) {
			$accessible_hosts = preg_split('|,\s*|', HQ_ACCESSIBLE_HOSTS);

			if ( false !== strpos(HQ_ACCESSIBLE_HOSTS, '*') ) {
	...
	public static function make_absolute_url( $maybe_relative_path, $url ) {
		...
		if ( ! $url_parts = HQ_HTTP::parse_url( $url ) ) {
			return $maybe_relative_path;
		}

		if ( ! $relative_url_parts = HQ_HTTP::parse_url( $maybe_relative_path ) ) {
			return $maybe_relative_path;
		}
	 ...
	 * @return false|object False if no redirect is present, a HQ_HTTP or HQ_Error result otherwise.
	 */
	public static function handle_redirects( $url, $args, $response ) {
		...
		if ( $args['redirection']-- <= 0 )
			return new HQ_Error( 'http_request_failed', __('Too many redirects.') );
		...
		$redirect_location = HQ_HTTP::make_absolute_url( $redirect_location, $url );
		...
		return hq_remote_request( $redirect_location, $args );
	}
 ...
 * @since 0.0.1
 */
class HQ_Http_Streams {
	/**
	 * Send a HTTP request to a URI using PHP Streams.
	 *
	 * @see HQ_Http::request For default options descriptions.
	 *
	 * @since 0.0.1
	 * @since 0.0.1
	 *
	...
	public function request($url, $args = array()) {
		...
		if ( false === $handle ) {
			...
			if ( $secure_transport && 0 === $connection_error && '' === $connection_error_str )
				return new HQ_Error( 'http_request_failed', __( 'The SSL certificate for the host could not be verified.' ) );

			return new HQ_Error('http_request_failed', $connection_error . ': ' . $connection_error_str );
		}
		...
		if ( $r['stream'] ) {
			if ( ! HQ_DEBUG )
				...
				$stream_handle = fopen( $r['filename'], 'w+' );
			if ( ! $stream_handle )
				return new HQ_Error( 'http_request_failed', sprintf( __( 'Could not open handle for fopen() to %s' ), $r['filename'] ) );
			...
			while ( ! feof($handle) && $keep_reading ) {
				...
				if ( ! $bodyStarted ) {
					...
					if ( strpos( $strResponse, "\r\n\r\n" ) ) {
						$process = HQ_Http::processResponse( $strResponse );
						$bodyStarted = true;
						$block = $process['body'];
						unset( $strResponse );
						$process['body'] = '';
				...
				if ( $bytes_written_to_file != $this_block_size ) {
					...
					return new HQ_Error( 'http_request_failed', __( 'Failed to write request to temporary file.' ) );
				}
		...
		$arrHeaders = HQ_Http::processHeaders( $process['headers'], $url );
		...
		// Handle redirects.
		if ( false !== ( $redirect_response = HQ_HTTP::handle_redirects( $url, $r, $response ) ) )
			return $redirect_response;

		// If the body was chunk encoded, then decode it.
		if ( ! empty( $process['body'] ) && isset( $arrHeaders['headers']['transfer-encoding'] ) && 'chunked' == $arrHeaders['headers']['transfer-encoding'] )
			$process['body'] = HQ_Http::chunkTransferDecode($process['body']);

		if ( true === $r['decompress'] && true === HQ_Http_Encoding::should_decode($arrHeaders['headers']) )
			$process['body'] = HQ_Http_Encoding::decompress( $process['body'] );
	...
	public static function verify_ssl_certificate( $stream, $host ) {
		 ...
		 */
		$host_type = ( HQ_HTTP::is_ip_address( $host ) ? 'ip' : 'dns' );
 ...
 * All code should make use of HQ_HTTP directly through it's API.
 *
 * @see HQ_HTTP::request
 *
 * @since 0.0.1
 * @deprecated 3.7.0 Please use HQ_HTTP::request() directly
 */
class HQ_HTTP_Fsockopen extends HQ_HTTP_Streams {
...
class HQ_Http_Curl {
	...
	public function request($url, $args = array()) {
		...
		$ssl_verify = isset($r['sslverify']) && $r['sslverify'];
		if ( $is_local ) {
			/** This filter is documented in hq-includes/class-http.php */
			$ssl_verify = apply_filters( 'https_local_ssl_verify', $ssl_verify );
		} elseif ( ! $is_local ) {
			/** This filter is documented in hq-includes/class-http.php */
			$ssl_verify = apply_filters( 'https_ssl_verify', $ssl_verify );
		...
		if ( ! $r['blocking'] ) {
			...
			if ( $curl_error = curl_error( $handle ) ) {
				curl_close( $handle );
				return new HQ_Error( 'http_request_failed', $curl_error );
			}
		...
		curl_exec( $handle );
		$theHeaders = HQ_Http::processHeaders( $this->headers, $url );
		$theBody = $this->body;
			...
			if ( CURLE_WRITE_ERROR /* 23 */ == $curl_error ) {
				if ( ! $this->max_body_length || $this->max_body_length != $bytes_written_total ) {
					if ( $r['stream'] ) {
						...
						curl_close( $handle );
						return new HQ_Error( 'http_request_failed', curl_error( $handle ) );
					}
				}
			} else {
				if ( $curl_error = curl_error( $handle ) ) {
		...
		if ( true === $r['decompress'] && true === HQ_Http_Encoding::should_decode($theHeaders['headers']) )
			$theBody = HQ_Http_Encoding::decompress( $theBody );
...
/**
 * Adds Proxy support to the HiveQueen HTTP API.
 *
 * There are caveats to proxy support. It requires that defines be made in the hq-config.php file to
 ...
 * <ol>
 * <li>HQ_PROXY_HOST - Enable proxy support and host for connecting.</li>
 * <li>HQ_PROXY_PORT - Proxy port for connection. No default, must be defined.</li>
 * <li>HQ_PROXY_USERNAME - Proxy username, if it requires authentication.</li>
 * <li>HQ_PROXY_PASSWORD - Proxy password, if it requires authentication.</li>
 * <li>HQ_PROXY_BYPASS_HOSTS - Will prevent the hosts in this list from going through the proxy.
 * You do not need to have localhost and the blog host in this list, because they will not be passed
 * through the proxy. The list should be presented in a comma separated list, wildcards using * are supported, eg. *.hivequeen.org</li>
 * </ol>
 *
 * An example can be as seen below.
 *
 *     define('HQ_PROXY_HOST', '192.168.84.101');
 *     define('HQ_PROXY_PORT', '8080');
 *     define('HQ_PROXY_BYPASS_HOSTS', 'localhost, www.example.com, *.hivequeen.org');
 *
 * @since 0.0.1
 */
class HQ_HTTP_Proxy {
	...
	public function is_enabled() {
		return defined('HQ_PROXY_HOST') && defined('HQ_PROXY_PORT');
	}
	 ...
	 * @since 0.0.1
	 *
	 * @use HQ_PROXY_USERNAME
	 * @use HQ_PROXY_PASSWORD
	 *
	 * @return bool
	 */
	public function use_authentication() {
		return defined('HQ_PROXY_USERNAME') && defined('HQ_PROXY_PASSWORD');
	}
	...
	public function host() {
		if ( defined('HQ_PROXY_HOST') )
			return HQ_PROXY_HOST;
	...
	public function port() {
		if ( defined('HQ_PROXY_PORT') )
			return HQ_PROXY_PORT;
	...
	public function username() {
		if ( defined('HQ_PROXY_USERNAME') )
			return HQ_PROXY_USERNAME;
	...
	public function password() {
		if ( defined('HQ_PROXY_PASSWORD') )
			return HQ_PROXY_PASSWORD;
	...
	public function send_through_proxy( $uri ) {
		...
		if ( !defined('HQ_PROXY_BYPASS_HOSTS') )
			return true;

		static $bypass_hosts = null;
		static $wildcard_regex = array();
		if ( null === $bypass_hosts ) {
			$bypass_hosts = preg_split('|,\s*|', HQ_PROXY_BYPASS_HOSTS);

			if ( false !== strpos(HQ_PROXY_BYPASS_HOSTS, '*') ) {
				$wildcard_regex = array();
 ...
 * already a HQ_Http_Cookie() object, then they are turned into one.
 *
 * @todo The HiveQueen convention is to use underscores instead of camelCase for function and method
...
class HQ_Http_Cookie {
	...
	public function getHeaderValue() {
		 ...
		 */
		return $this->name . '=' . apply_filters( 'hq_http_cookie_value', $this->value, $this->name );
	}

	/**
	 * Retrieve cookie header for usage in the rest of the HiveQueen HTTP API.
	 *
	 * @access public
	 * @since 0.0.1
	 *
...
class HQ_Http_Encoding {
	...
	public static function accept_encoding( $url, $args ) {
		 ...
		 * @param array  $args HTTP request arguments.
		 */
		$type = apply_filters( 'hq_http_accept_encoding', $type, $url, $args );
