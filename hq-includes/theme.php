...
/**
 * Returns an array of HQ_Theme objects based on the arguments.
 *
 * Despite advances over get_themes(), this function is quite expensive, and grows
 * linearly with additional themes. Stick to hq_get_theme() if possible.
 *
 * @since 0.0.1
 *
 * @global array $hq_theme_directories
 ...
 * @return array Array of HQ_Theme objects.
 */
function hq_get_themes( $args = array() ) {
	global $hq_theme_directories;
	...
	if ( count( $hq_theme_directories ) > 1 ) {
		// Make sure the current theme wins out, in case search_theme_directories() picks the wrong
		// one in the case of a conflict. (Normally, last registered theme root wins.)
		$current_theme = get_stylesheet();
		if ( isset( $theme_directories[ $current_theme ] ) ) {
			$root_of_current_theme = get_raw_theme_root( $current_theme );
			if ( ! in_array( $root_of_current_theme, $hq_theme_directories ) )
				$root_of_current_theme = HQ_CONTENT_DIR . $root_of_current_theme;
	...
	if ( is_multisite() && null !== $args['allowed'] ) {
		...
		if ( 'network' === $allowed )
			$theme_directories = array_intersect_key( $theme_directories, HQ_Theme::get_allowed_on_network() );
		elseif ( 'site' === $allowed )
			$theme_directories = array_intersect_key( $theme_directories, HQ_Theme::get_allowed_on_site( $args['blog_id'] ) );
		elseif ( $allowed )
			$theme_directories = array_intersect_key( $theme_directories, HQ_Theme::get_allowed( $args['blog_id'] ) );
		else
			$theme_directories = array_diff_key( $theme_directories, HQ_Theme::get_allowed( $args['blog_id'] ) );
	}
	...
	foreach ( $theme_directories as $theme => $theme_root ) {
		if ( isset( $_themes[ $theme_root['theme_root'] . '/' . $theme ] ) )
			...
			$themes[ $theme ] = $_themes[ $theme_root['theme_root'] . '/' . $theme ] = new HQ_Theme( $theme, $theme_root['theme_root'] );
	}

	if ( null !== $args['errors'] ) {
		foreach ( $themes as $theme => $hq_theme ) {
			if ( $hq_theme->errors() != $args['errors'] )
				unset( $themes[ $theme ] );
		}
	}
...
/**
 * Gets a HQ_Theme object for a theme.
 *
 * @since 0.0.1
 *
 * @global array $hq_theme_directories
 ...
 * @return HQ_Theme Theme object. Be sure to check the object's exists() method if you need to confirm the theme's existence.
 */
function hq_get_theme( $stylesheet = null, $theme_root = null ) {
	global $hq_theme_directories;
	...
	if ( empty( $theme_root ) ) {
		...
		if ( false === $theme_root )
			$theme_root = HQ_CONTENT_DIR . '/themes';
		elseif ( ! in_array( $theme_root, (array) $hq_theme_directories ) )
			$theme_root = HQ_CONTENT_DIR . $theme_root;
	}

	return new HQ_Theme( $stylesheet, $theme_root );
}
 ...
 * HiveQueen workflow, then change the former. If you just have the locale in a
...
function get_locale_stylesheet_uri() {
	...
	elseif ( !empty($hq_locale->text_direction) && file_exists("$dir/{$hq_locale->text_direction}.css") )
		$stylesheet_uri = "$stylesheet_dir_uri/{$hq_locale->text_direction}.css";
	else
		$stylesheet_uri = '';
	/**
 ...
 * @global array $hq_theme_directories
 *
 * @param string $directory Either the full filesystem path to a theme folder or a folder within HQ_CONTENT_DIR
 * @return bool
 */
function register_theme_directory( $directory ) {
	...
	if ( ! file_exists( $directory ) ) {
		...
		$directory = HQ_CONTENT_DIR . '/' . $directory;
	...
	if ( ! is_array( $hq_theme_directories ) ) {
		$hq_theme_directories = array();
	}

	$untrailed = untrailingslashit( $directory );
	if ( ! empty( $untrailed ) && ! in_array( $untrailed, $hq_theme_directories ) ) {
		$hq_theme_directories[] = $untrailed;
	}
...
function search_theme_directories( $force = false ) {
	...
	$hq_theme_directories = (array) $hq_theme_directories;
	$relative_theme_roots = array();

	// Set up maybe-relative, maybe-absolute array of theme directories.
	// We always want to return absolute, but we need to cache relative
	// to use in get_theme_root().
	foreach ( $hq_theme_directories as $theme_root ) {
		if ( 0 === strpos( $theme_root, HQ_CONTENT_DIR ) )
			$relative_theme_roots[ str_replace( HQ_CONTENT_DIR, '', $theme_root ) ] = $theme_root;
		else
	 ...
	 */
	if ( $cache_expiration = apply_filters( 'hq_cache_themes_persistently', false, 'search_theme_directories' ) ) {
	...
	foreach ( $hq_theme_directories as $theme_root ) {
		...
		foreach ( $dirs as $dir ) {
			...
			if ( file_exists( $theme_root . '/' . $dir . '/style.css' ) ) {
				// hq-content/themes/a-single-theme
				// hq-content/themes is $theme_root, a-single-theme is $dir
				...
				$found_theme = false;
				// hq-content/themes/a-folder-of-themes/*
				// hq-content/themes is $theme_root, a-folder-of-themes is $dir, then themes are $sub_dirs
				...
				// Return it; HQ_Theme will catch the error.
				if ( ! $found_theme )
					$found_themes[ $dir ] = array(
						'theme_file' => $dir . '/style.css',
						'theme_root' => $theme_root,
...
function get_theme_root( $stylesheet_or_template = false ) {
	...
	if ( $stylesheet_or_template && $theme_root = get_raw_theme_root( $stylesheet_or_template ) ) {
		// Always prepend HQ_CONTENT_DIR unless the root currently registered as a theme directory.
		// This gives relative theme roots the benefit of the doubt when things go haywire.
		if ( ! in_array( $theme_root, (array) $hq_theme_directories ) )
			$theme_root = HQ_CONTENT_DIR . $theme_root;
	} else {
		$theme_root = HQ_CONTENT_DIR . '/themes';
	}
...
function get_theme_root_uri( $stylesheet_or_template = false, $theme_root = false ) {
	...
	if ( $stylesheet_or_template && $theme_root ) {
		if ( in_array( $theme_root, (array) $hq_theme_directories ) ) {
			...
			if ( 0 === strpos( $theme_root, HQ_CONTENT_DIR ) )
				$theme_root_uri = content_url( str_replace( HQ_CONTENT_DIR, '', $theme_root ) );
			elseif ( 0 === strpos( $theme_root, ABSPATH ) )
				$theme_root_uri = site_url( str_replace( ABSPATH, '', $theme_root ) );
			elseif ( 0 === strpos( $theme_root, HQ_PLUGIN_DIR ) || 0 === strpos( $theme_root, HQMU_PLUGIN_DIR ) )
	 ...
	 * @param string $siteurl                HiveQueen web address which is set in General Options.
...
function switch_theme( $stylesheet ) {
	global $hq_theme_directories, $hq_customize, $sidebars_widgets;

	$_sidebars_widgets = null;
	if ( 'hq_ajax_customize_save' === current_action() ) {
		$_sidebars_widgets = $hq_customize->post_value( $hq_customize->get_setting( 'old_sidebars_widgets_data' ) );
	...
	$old_theme  = hq_get_theme();
	$new_theme = hq_get_theme( $stylesheet );
	...
	if ( is_admin() && false === get_option( 'theme_mods_' . $stylesheet ) ) {
		 ...
		 * the Customizer when accessing hq-admin/widgets.php.
		 */
		if ( 'hq_ajax_customize_save' === current_action() ) {
	 ...
	 * @param HQ_Theme $new_theme HQ_Theme instance of the new theme.
	 */
...
function validate_current_theme() {
	 ...
	 */
	if ( defined('HQ_INSTALLING') || ! apply_filters( 'validate_current_theme', true ) )
		return true;

	if ( get_template() != HQ_DEFAULT_THEME && !file_exists(get_template_directory() . '/index.php') ) {
		switch_theme( HQ_DEFAULT_THEME );
		return false;
	}

	if ( get_stylesheet() != HQ_DEFAULT_THEME && !file_exists(get_template_directory() . '/style.css') ) {
		switch_theme( HQ_DEFAULT_THEME );
		return false;
	}
...
function get_theme_mods() {
	...
	if ( false === $mods ) {
		...
		if ( false === $theme_name )
			$theme_name = hq_get_theme()->get('Name');
...
function get_theme_mod( $name, $default = false ) {
	...
	/** This filter is documented in hq-includes/theme.php */
 ...
 * @access private
 *
 * @global array  $_hq_default_headers
 * @staticvar object $_hq_random_header
 *
 * @return object
 */
function _get_random_header_data() {
	static $_hq_random_header = null;

	if ( empty( $_hq_random_header ) ) {
		global $_hq_default_headers;
		$header_image_mod = get_theme_mod( 'header_image', '' );
		$headers = array();

		if ( 'random-uploaded-image' == $header_image_mod )
			$headers = get_uploaded_header_images();
		elseif ( ! empty( $_hq_default_headers ) ) {
			if ( 'random-default-image' == $header_image_mod ) {
				$headers = $_hq_default_headers;
			} else {
		...
		$_hq_random_header = (object) $headers[ array_rand( $headers ) ];

		$_hq_random_header->url =  sprintf( $_hq_random_header->url, get_template_directory_uri(), get_stylesheet_directory_uri() );
		$_hq_random_header->thumbnail_url =  sprintf( $_hq_random_header->thumbnail_url, get_template_directory_uri(), get_stylesheet_directory_uri() );
	}
	return $_hq_random_header;
}
...
function get_custom_header() {
	...
	if ( is_random_header_image() ) {
		...
		if ( ! $data && current_theme_supports( 'custom-header', 'default-image' ) ) {
			...
			if ( ! empty( $_hq_default_headers ) ) {
				foreach ( (array) $_hq_default_headers as $default_header ) {
	...
	return (object) hq_parse_args( $data, $default );
}
...
function register_default_headers( $headers ) {
	...
	$_hq_default_headers = array_merge( (array) $_hq_default_headers, (array) $headers );
}
...
function unregister_default_headers( $header ) {
	...
	} elseif ( isset( $_hq_default_headers[ $header ] ) ) {
		unset( $_hq_default_headers[ $header ] );
		return true;
	} else {
		return false;
	}
	...
	switch ( $feature ) {
		...
		case 'custom-background' :
			...
			$defaults = array(
				...
				'default-color'          => '',
				'hq-head-callback'       => '_custom_background_cb',
				'admin-head-callback'    => '',
				'admin-preview-callback' => '',
			);
		...
		case 'title-tag' :
			...
			if ( did_action( 'hq_loaded' ) ) {
				...
				_doing_it_wrong( "add_theme_support( 'title-tag' )", sprintf( __( 'Theme support for %1$s should be registered before the %2$s hook.' ),
					'<code>title-tag</code>', '<code>hq_loaded</code>' ), '4.1' );
...
function _custom_header_background_just_in_time() {
	...
	if ( current_theme_supports( 'custom-header' ) ) {
		...
		if ( $args[0]['hq-head-callback'] )
			add_action( 'hq_head', $args[0]['hq-head-callback'] );

		if ( is_admin() ) {
			require_once( ABSPATH . 'hq-admin/custom-header.php' );
	...
	if ( current_theme_supports( 'custom-background' ) ) {
		...
		if ( is_admin() ) {
			require_once( ABSPATH . 'hq-admin/custom-background.php' );
...
function get_theme_support( $feature ) {
	global $_hq_theme_features;
	if ( ! isset( $_hq_theme_features[ $feature ] ) )
		return false;

	if ( func_num_args() <= 1 )
		return $_hq_theme_features[ $feature ];

	$args = array_slice( func_get_args(), 1 );
	switch ( $feature ) {
		case 'custom-header' :
		case 'custom-background' :
			if ( isset( $_hq_theme_features[ $feature ][0][ $args[0] ] ) )
				return $_hq_theme_features[ $feature ][0][ $args[0] ];
			return false;
...
function _remove_theme_support( $feature ) {
	...
	switch ( $feature ) {
		case 'custom-header' :
			...
			if ( $support[0]['hq-head-callback'] )
				remove_action( 'hq_head', $support[0]['hq-head-callback'] );
	...
	unset( $_hq_theme_features[ $feature ] );
	return true;
}
...
function current_theme_supports( $feature ) {
	...
	if ( 'title-tag' == $feature ) {
		...
		if ( ! in_array( $trace[1]['function'], array( '_hq_render_title_tag', 'hq_title' ) ) ) {
			return false;
		}
	}
	...
	switch ( $feature ) {
		case 'post-thumbnails':
			...
			if ( true === $_hq_theme_features[$feature] )  // Registered for all types
				return true;
			$content_type = $args[0];
			return in_array( $content_type, $_hq_theme_features[$feature][0] );
		...
		case 'post-formats':
			...
			$type = $args[0];
			return in_array( $type, $_hq_theme_features[$feature][0] );
		...
		case 'custom-background' :
			...
			return ( isset( $_hq_theme_features[$feature][0][$header_support] ) && $_hq_theme_features[$feature][0][$header_support] );
	}
	 ...
	 */
	return apply_filters( "current_theme_supports-{$feature}", true, $args, $_hq_theme_features[$feature] );
}
...
function _delete_attachment_theme_mod( $id ) {
	$attachment_image = hq_get_attachment_url( $id );
...
/**
 * Includes and instantiates the HQ_Customize_Manager class.
 *
 * Fires when ?hq_customize=on or on hq-admin/customize.php.
 *
 * @since 0.0.1
 *
 * @global HQ_Customize_Manager $hq_customize
 */
function _hq_customize_include() {
	if ( ! ( ( isset( $_REQUEST['hq_customize'] ) && 'on' == $_REQUEST['hq_customize'] )
		|| ( is_admin() && 'customize.php' == basename( $_SERVER['PHP_SELF'] ) )
	) )
		return;

	require_once ABSPATH . HQINC . '/class-hq-customize-manager.php';
	$GLOBALS['hq_customize'] = new HQ_Customize_Manager();
}
...
function _hq_customize_loader_settings() {
	...
	$browser = array(
		'mobile' => hq_is_mobile(),
		'ios'    => hq_is_mobile() && preg_match( '/iPad|iPod|iPhone/', $_SERVER['HTTP_USER_AGENT'] ),
	);
 ...
 * @return string
 */
function hq_customize_url( $stylesheet = null ) {
 ...
 * @since 0.0.1
 */
function hq_customize_support_script() {
	$admin_origin = parse_url( admin_url() );
	$home_origin  = parse_url( home_url() );
	$cross_domain = ( strtolower( $admin_origin[ 'host' ] ) != strtolower( $home_origin[ 'host' ] ) );
 ...
 * @since 0.0.1
 *
 * @global HQ_Customize_Manager $hq_customize Customizer instance.
 *
...
function is_customize_preview() {
	...
	return ( $hq_customize instanceof HQ_Customize_Manager ) && $hq_customize->is_preview();
}
