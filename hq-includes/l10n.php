 ...
 */
function get_locale() {
        global $locale, $hq_local_package;

        if ( isset( $locale ) ) {
                /**
                 * Filter HiveQueen install's locale ID.
                 *
                 * @since 0.0.1
                 *
        ...
        //        if ( defined( 'HQ_INSTALLING' ) || ( false === $ms_locale = get_option( 'HQLANG' ) ) ) {
        //                $ms_locale = get_site_option( 'HQLANG' );
        //        }
        //
        //        if ( $ms_locale !== false ) {
        //                $locale = $ms_locale;
        ...
        /** This filter is documented in hq-includes/l10n.php */
        return apply_filters( 'locale', $locale );
}
 ...
 *
 * Loads the .mo file in HQ_LANG_DIR constant path from WordPress root.
...
function load_default_textdomain( $locale = null ) {
        ...
        if ( is_admin() || defined( 'HQ_INSTALLING' ) || ( defined( 'HQ_REPAIRING' ) && HQ_REPAIRING ) ) {
                load_textdomain( 'default', HQ_LANG_DIR . "/admin-$locale.mo" );
        }

        if ( is_network_admin() || ( defined( 'HQ_INSTALLING_NETWORK' ) && HQ_INSTALLING_NETWORK ) )
                load_textdomain( 'default', HQ_LANG_DIR . "/admin-network-$locale.mo" );
 ...
 * @param string $plugin_rel_path Optional. Relative path to HQ_PLUGIN_DIR where the .mo file resides.
 *                                Default false.
...
function load_plugin_textdomain( $domain, $deprecated = false, $plugin_rel_path = false ) {
        ...
        if ( false !== $plugin_rel_path ) {
                $path = HQ_PLUGIN_DIR . '/' . trim( $plugin_rel_path, '/' );
        ...
        $mofile = HQ_LANG_DIR . '/plugins/' . $mofile;
 ...
 *
 * @param string $domain             Text domain. Unique identifier for retrieving translated strings.
 * @param string $mu_plugin_rel_path Relative to HQMU_PLUGIN_DIR directory in which the .mo file resides.
...
function load_muplugin_textdomain( $domain, $mu_plugin_rel_path = '' ) {
        ...
        $path = trailingslashit( HQMU_PLUGIN_DIR . '/' . ltrim( $mu_plugin_rel_path, '/' ) );
...
function load_theme_textdomain( $domain, $path = false ) {
        ...
        // Otherwise, load from the languages directory
        $mofile = HQ_LANG_DIR . "/themes/{$domain}-{$locale}.mo";
        return load_textdomain( $domain, $mofile );
}
 ...
 *
 * The default directory is HQ_LANG_DIR.
 *
 * @since 0.0.1
 *
...
function get_available_languages( $dir = null ) {
        ...
        $lang_files = glob( ( is_null( $dir) ? HQ_LANG_DIR : $dir ) . '/*.mo' );
        if ( $lang_files ) {
 ...
 *
 * Looks in the hq-content/languages directory for translations of
 * plugins or themes.
 *
 * @since 0.0.1
 *
 ...
 */
function hq_get_installed_translations( $type ) {
        if ( $type !== 'themes' && $type !== 'plugins' && $type !== 'core' )
                return array();

        $dir = 'core' === $type ? '' : "/$type";

        if ( ! is_dir( HQ_LANG_DIR ) )
                return array();

        if ( $dir && ! is_dir( HQ_LANG_DIR . $dir ) )
                return array();

        $files = scandir( HQ_LANG_DIR . $dir );
        if ( ! $files )
                return array();
        ...
        foreach ( $files as $file ) {
                ...
                $language_data[ $textdomain ][ $language ] = hq_get_pomo_file_data( HQ_LANG_DIR . "$dir/$file" );
        }
 ...
 */
function hq_get_pomo_file_data( $po_file ) {
 ...
 */
function hq_dropdown_languages( $args = array() ) {
