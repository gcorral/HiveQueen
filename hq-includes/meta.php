 ...
 * Metadata API
 *
 * Functions for retrieving and manipulating metadata of various HiveQueen object types. Metadata
...
function add_metadata($meta_type, $object_id, $meta_key, $meta_value, $unique = false) {
	...
	if ( $unique && $hqdb->get_var( $hqdb->prepare(
	...
	$result = $hqdb->insert( $table, array(
		$column => $object_id,
		'meta_key' => $meta_key,
...
function update_metadata($meta_type, $object_id, $meta_key, $meta_value, $prev_value = '') {
	...
	$meta_ids = $hqdb->get_col( $hqdb->prepare( "SELECT $id_column FROM $table WHERE meta_key = %s AND $column = %d", $meta_key, $object_id ) );
	if ( empty( $meta_ids ) ) {
	...
	$result = $hqdb->update( $table, $data, $where );
	if ( ! $result )
		return false;
...
function get_metadata($meta_type, $object_id, $meta_key = '', $single = false) {
	...
	if ( null !== $check ) {
        ...
        //TODO: Goyo no cache
	//$meta_cache = hq_cache_get($object_id, $meta_type . '_meta');
...
function metadata_exists( $meta_type, $object_id, $meta_key ) {
	...
	/** This filter is documented in hq-includes/meta.php */
...
function get_metadata_by_mid( $meta_type, $meta_id ) {
	...
	$meta = $hqdb->get_row( $hqdb->prepare( "SELECT * FROM $table WHERE $id_column = %d", $meta_id ) );
...
function update_metadata_by_mid( $meta_type, $meta_id, $meta_value, $meta_key = false ) {
	...
	if ( $meta = get_metadata_by_mid( $meta_type, $meta_id ) ) {
		...
		/** This action is documented in hq-includes/meta.php */
		do_action( "update_{$meta_type}_meta", $meta_id, $object_id, $meta_key, $_meta_value );
		...
		$result = $hqdb->update( $table, $data, $where, '%s', '%d' );
		if ( ! $result )
			return false;
...
function delete_metadata_by_mid( $meta_type, $meta_id ) {
	...
	if ( $meta = get_metadata_by_mid( $meta_type, $meta_id ) ) {
		...
		$result = (bool) $hqdb->delete( $table, array( $id_column => $meta_id ) );
...
function update_meta_cache($meta_type, $object_ids) {
	...
	foreach ( $object_ids as $id ) {
                //TODO: Goyo no cache 
		//$cached_object = hq_cache_get( $id, $cache_key );
	...
	$id_list = join( ',', $ids );
	$id_column = 'user' == $meta_type ? 'umeta_id' : 'meta_id';
	$meta_list = $hqdb->get_results( "SELECT $column, meta_key, meta_value FROM $table WHERE $column IN ($id_list) ORDER BY $id_column ASC", ARRAY_A );
	...
	foreach ( $ids as $id ) {
		if ( ! isset($cache[$id]) )
			$cache[$id] = array();
                //TODO: Goyo no cache
		//hq_cache_add( $id, $cache[$id], $cache_key );
	}
...
function get_meta_sql( $meta_query, $type, $primary_table, $primary_id_column, $context = null ) {
	$meta_query_obj = new HQ_Meta_Query( $meta_query );
 ...
 *
 * `HQ_Meta_Query` is a helper that allows primary query classes, such as {@see HQ_Query} and {@see HQ_User_Query},
...
class HQ_Meta_Query {
	 ...
	 *
	 * See {@see HQ_Meta_Query::__construct()} for information on meta query arguments.
	 *
	 * @since 0.0.1
	 * @access public
	 * @var array
	...
	/**
	 * Database table that where the metadata's objects are stored (eg $hqdb->users).
	 *
	 * @since 0.0.1
	 * @access public
	 * @var string
	...
	public function parse_query_vars( $qv ) {
		...
		// HQ_Query sets 'meta_value' = '' by default.
	 ...
	 * @param string $primary_table     Database table where the object being filtered is stored (eg hq_users).
	 ...
	 *
	 * Called by the public {@see HQ_Meta_Query::get_sql()}, this method
	...
	public function get_sql_for_clause( &$clause, $parent_query, $clause_key = '' ) {
		...
		if ( false === $alias ) {
			...
			if ( 'NOT EXISTS' === $meta_compare ) {
				...
				$join .= $i ? " AS $alias" : '';
				$join .= $hqdb->prepare( " ON ($this->primary_table.$this->primary_id_column = $alias.$this->meta_id_column AND $alias.meta_key = %s )", $clause['key'] );
		...
		if ( array_key_exists( 'key', $clause ) ) {
			if ( 'NOT EXISTS' === $meta_compare ) {
				...
				$sql_chunks['where'][] = $hqdb->prepare( "$alias.meta_key = %s", trim( $clause['key'] ) );
			}
		}
		...
		if ( array_key_exists( 'value', $clause ) ) {
			...
			switch ( $meta_compare ) {
				...
				case 'NOT IN' :
					...
					$where = $hqdb->prepare( $meta_compare_string, $meta_value );
					break;

				case 'BETWEEN' :
				case 'NOT BETWEEN' :
					$meta_value = array_slice( $meta_value, 0, 2 );
					$where = $hqdb->prepare( '%s AND %s', $meta_value );
					break;

				case 'LIKE' :
				case 'NOT LIKE' :
					$meta_value = '%' . $hqdb->esc_like( $meta_value ) . '%';
					$where = $hqdb->prepare( '%s', $meta_value );
					break;
	 ...
	 * In the case of {@see HQ_Meta_Query}, this only applies to 'IN' clauses that
	...
	protected function find_compatible_table_alias( $clause, $parent_query ) {
		 ...
		 * @param object      $this         HQ_Meta_Query object.
		 */
...
function _get_meta_table($type) {
	...
	if ( empty($hqdb->$table_name) )
		return false;

	return $hqdb->$table_name;
}
