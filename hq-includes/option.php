 ...
 */
function hq_load_core_site_options( $site_id = null ) {
	global $hqdb;

	if ( !is_multisite() || hq_using_ext_object_cache() || defined( 'HQ_INSTALLING' ) )
		return;
	...
	$options = $hqdb->get_results( $hqdb->prepare("SELECT meta_key, meta_value FROM $hqdb->sitemeta WHERE meta_key IN ($core_options_in) AND site_id = %d", $site_id) );
	...
	foreach ( $options as $option ) {
		...
		hq_cache_set( $cache_key, $option->meta_value, 'site-options' );
	}
}
 ...
 * @param string|bool $autoload Optional. Whether to load the option when HiveQueen starts up. For existing options,
...
function update_option( $option, $value, $autoload = null ) {
	...
	hq_protect_special_option( $option );
	...
	$result = $hqdb->update( $hqdb->options, $update_args, array( 'option_name' => $option ) );
	if ( ! $result )
		return false;
 ...
 * aren't adding a protected HiveQueen option. Care should be taken to not name
 ...
 * @param string|bool    $autoload    Optional. Whether to load the option when HiveQueen starts up.
...
function add_option( $option, $value = '', $deprecated = '', $autoload = 'yes' ) {
	...
	$result = $hqdb->query( $hqdb->prepare( "INSERT INTO `$hqdb->options` (`option_name`, `option_value`, `autoload`) VALUES (%s, %s, %s) ON DUPLICATE KEY UPDATE `option_name` = VALUES(`option_name`), `option_value` = VALUES(`option_value`), `autoload` = VALUES(`autoload`)", $option, $serialized_value, $autoload ) );
	if ( ! $result )
		return false;
...
/**
 * Removes option by name. Prevents removal of protected HiveQueen options.
 *
 * @since 0.0.1
 *
 * @global hqdb $hqdb
...
function delete_option( $option ) {
	...
	$row = $hqdb->get_row( $hqdb->prepare( "SELECT autoload FROM $hqdb->options WHERE option_name = %s", $option ) );
	if ( is_null( $row ) )
		return false;
	...
	$result = $hqdb->delete( $hqdb->options, array( 'option_name' => $option ) );
	if ( ! defined( 'HQ_INSTALLING' ) ) {
		if ( 'yes' == $row->autoload ) {
			$alloptions = hq_load_alloptions();
			...
			hq_cache_delete( $option, 'options' );
		}
	}
	if ( $result ) {
...
function delete_transient( $transient ) {
	...
	if ( hq_using_ext_object_cache() ) {
		$result = hq_cache_delete( $transient, 'transient' );
	} else {
...
function set_transient( $transient, $value, $expiration = 0 ) {
	...
	if ( hq_using_ext_object_cache() ) {
		$result = hq_cache_set( $transient, $value, 'transient', $expiration );
	} else {
		$transient_timeout = '_transient_timeout_' . $transient;
...
function hq_user_settings() {
	...
	if ( isset( $_COOKIE['hq-settings-' . $user_id] ) ) {
		$cookie = preg_replace( '/[^A-Za-z0-9=&_]/', '', $_COOKIE['hq-settings-' . $user_id] );
		...
		$current = isset( $_COOKIE['hq-settings-time-' . $user_id]) ? preg_replace( '/[^0-9]/', '', $_COOKIE['hq-settings-time-' . $user_id] ) : 0;
	...
	setcookie( 'hq-settings-' . $user_id, $settings, time() + YEAR_IN_SECONDS, SITECOOKIEPATH, null, $secure );
	setcookie( 'hq-settings-time-' . $user_id, time(), time() + YEAR_IN_SECONDS, SITECOOKIEPATH, null, $secure );
	$_COOKIE['hq-settings-' . $user_id] = $settings;
}
...
function set_user_setting( $name, $value ) {
	...
	return hq_set_all_user_settings( $all_user_settings );
}
 ...
 * @return bool|void
 */
function hq_set_all_user_settings( $user_settings ) {
	global $_updated_user_settings;
...
function delete_all_user_settings() {
	...
	setcookie( 'hq-settings-' . $user_id, ' ', time() - YEAR_IN_SECONDS, SITECOOKIEPATH );
}
...
function get_site_option( $option, $default = false, $use_cache = true ) {
	...
	if ( true ) {
		...
		if ( $use_cache )
			$value = hq_cache_get($cache_key, 'site-options');

		if ( !isset($value) || (false === $value) ) {
			$row = $hqdb->get_row( $hqdb->prepare("SELECT meta_value FROM $hqdb->sitemeta WHERE meta_key = %s AND site_id = %d", $option, $hqdb->siteid ) );
...
function add_site_option( $option, $value ) {
	...
	if ( true ) {
		...
		$serialized_value = maybe_serialize( $value );
		$result = $hqdb->insert( $hqdb->sitemeta, array('site_id' => $hqdb->siteid, 'meta_key' => $option, 'meta_value' => $serialized_value ) );
...
function delete_site_option( $option ) {
	...
	if ( true ) {
		...
		if ( is_null( $row ) || !$row->meta_id )
			return false;
		$cache_key = "{$hqdb->siteid}:$option";
		hq_cache_delete( $cache_key, 'site-options' );

		$result = $hqdb->delete( $hqdb->sitemeta, array( 'meta_key' => $option, 'site_id' => $hqdb->siteid ) );
	}
...
function update_site_option( $option, $value ) {
	...
	if ( !is_multisite() ) {
		...
		$serialized_value = maybe_serialize( $value );
		$result = $hqdb->update( $hqdb->sitemeta, array( 'meta_value' => $serialized_value ), array( 'site_id' => $hqdb->siteid, 'meta_key' => $option ) );
...
function get_site_transient( $transient ) {
	...
	if ( hq_using_ext_object_cache() ) {
		$value = hq_cache_get( $transient, 'site-transient' );
	} else {
