<?php
/**
 * HiveQueen implementation for PHP functions either missing from older PHP versions or not included by default.
 *
 * @package PHP
 * @access private
 */
 ...
 */
function _hq_can_use_pcre_u( $set = null ) {
...
if ( !function_exists('json_encode') ) {
	function json_encode( $string ) {
		...
		if ( ! ( $hq_json instanceof Services_JSON ) ) {
			require_once( ABSPATH . HQINC . '/class-json.php' );
			$hq_json = new Services_JSON();
		}

		return $hq_json->encodeUnsafe( $string );
	}
}

if ( !function_exists('json_decode') ) {
	/**
	 * @global Services_JSON $hq_json
	 * @param string $string
...
// Defined here to prevent a notice when using it with hq_json_encode()
