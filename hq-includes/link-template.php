<?php
/**
 * HiveQueen Link Template Functions
 *
 * @package HiveQueen
 * @subpackage Template
 */
...
function user_trailingslashit($string, $type_of_url = '') {
	global $hq_rewrite;
	if ( $hq_rewrite->use_trailing_slashes )
 ...
 * @see get_permalink()
 *
 * @param int|HQ_Post $id        Optional. Post ID or post object. Default is the current post.
 ...
 * @since 0.0.1
 *
 * @param int|HQ_Post $id        Optional. Post ID or post object. Default current post.
 * @param bool        $leavename Optional. Whether to keep post name or page name. Default false.
 * @return string|false The permalink URL or false if post does not exist.
 */
function get_permalink( $id = 0, $leavename = false ) {
	 ...
	 * @param HQ_Post $post      The post in question.
	...
	if ( '' != $permalink && !in_array( $post->post_status, array( 'draft', 'pending', 'auto-draft', 'future' ) ) ) {
		...
		if ( strpos($permalink, '%category%') !== false ) {
			...
			if ( empty($category) ) {
				...
				$category = is_hq_error( $default_category ) ? '' : $default_category->slug;
			}
		}
 ...
 * @return string|HQ_Error The post permalink.
 */
function get_post_permalink( $id = 0, $leavename = false, $sample = false ) {
	...
	$post_link = $hq_rewrite->get_extra_permastruct($post->post_type);
...
function _get_page_link( $post = false, $leavename = false, $sample = false ) {
	...
	$link = $hq_rewrite->get_page_permastruct();
 ...
 *
 * This can be used in the HiveQueen Loop or outside of it.
 *
 * @since 0.0.1
 *
 * @global HQ_Rewrite $hq_rewrite
...
function get_attachment_link( $post = null, $leavename = false ) {
	...
	if ( $hq_rewrite->using_permalinks() && $parent ) {
...
function get_year_link($year) {
	...
	$yearlink = $hq_rewrite->get_year_permastruct();
	if ( !empty($yearlink) ) {
...
function get_month_link($year, $month) {
	...
	$monthlink = $hq_rewrite->get_month_permastruct();
	if ( !empty($monthlink) ) {
...
function get_day_link($year, $month, $day) {
	...
	$daylink = $hq_rewrite->get_day_permastruct();
	if ( !empty($daylink) ) {
...
function get_feed_link($feed = '') {
	...
	$permalink = $hq_rewrite->get_feed_permastruct();
	if ( '' != $permalink ) {
		if ( false !== strpos($feed, 'comments_') ) {
			$feed = str_replace('comments_', '', $feed);
			$permalink = $hq_rewrite->get_comment_feed_permastruct();
		}
...
function get_search_link( $query = '' ) {
	...
	$permastruct = $hq_rewrite->get_search_permastruct();
...
function get_post_type_archive_link( $post_type ) {
	...
	if ( get_option( 'permalink_structure' ) && is_array( $post_type_obj->rewrite ) ) {
		...
		if ( $post_type_obj->rewrite['with_front'] )
			$struct = $hq_rewrite->front . $struct;
		else
			$struct = $hq_rewrite->root . $struct;
 ...
 *
 * Can be used within the HiveQueen loop or outside of it. Can be used with
 ...
 *
 * This is meant to be attached to actions like 'hq_head'. Do not call this directly in plugins or theme templates.
 * @since 0.0.1
 *
 */
function adjacent_posts_rel_link_hq_head() {
...
function get_adjacent_post_link( $format, $link, $in_same_term = false, $excluded_terms = '', $previous = true, $taxonomy = 'category' ) {
	 ...
	 * @param string  $format   Link anchor format.
	 * @param string  $link     Link permalink format.
	 * @param HQ_Post $post     The adjacent post.
...
function get_pagenum_link($pagenum = 1, $escape = true ) {
	...
	if ( !$hq_rewrite->using_permalinks() || is_admin() ) {
		...
		$request = preg_replace( "|$hq_rewrite->pagination_base/\d+/?$|", '', $request);
		$request = preg_replace( '|^' . preg_quote( $hq_rewrite->index, '|' ) . '|i', '', $request);
		$request = ltrim($request, '/');

		$base = trailingslashit( get_bloginfo( 'url' ) );

		if ( $hq_rewrite->using_index_permalinks() && ( $pagenum > 1 || '' != $request ) )
			$base .= $hq_rewrite->index . '/';

		if ( $pagenum > 1 ) {
			$request = ( ( !empty( $request ) ) ? trailingslashit( $request ) : $request ) . user_trailingslashit( $hq_rewrite->pagination_base . "/" . $pagenum, 'paged' );
		}
...
function get_next_posts_link( $label = null, $max_page = 0 ) {
	global $paged, $hq_query;

	if ( !$max_page )
		$max_page = $hq_query->max_num_pages;
...
function get_posts_nav_link( $args = array() ) {
	...
	if ( !is_singular() ) {
		...
		$max_num_pages = $hq_query->max_num_pages;
 ...
 * @since 0.0.1
 *
 * @global HQ_Query $hq_query HiveQueen Query object.
 *
 * @param array $args {
...
function get_the_posts_navigation( $args = array() ) {
	...
	if ( $GLOBALS['hq_query']->max_num_pages > 1 ) {
		$args = hq_parse_args( $args, array(
...
function get_comments_pagenum_link( $pagenum = 1, $max_page = 0 ) {
	...
	if ( 'newest' == get_option('default_comments_page') ) {
		if ( $pagenum != $max_page ) {
			if ( $hq_rewrite->using_permalinks() )
				$result = user_trailingslashit( trailingslashit($result) . $hq_rewrite->comments_pagination_base . '-' . $pagenum, 'commentpaged');
			else
...
function get_next_comments_link( $label = '', $max_page = 0 ) {
	...
	if ( empty($max_page) )
		$max_page = $hq_query->max_num_comment_pages;
...
function paginate_comments_links($args = array()) {
	...
	if ( $hq_rewrite->using_permalinks() )
		$defaults['base'] = user_trailingslashit(trailingslashit(get_permalink()) . $hq_rewrite->comments_pagination_base . '-%#%', 'commentpaged');
...
function get_admin_url( $blog_id = null, $path = '', $scheme = 'admin' ) {
	$url = get_site_url($blog_id, 'hq-admin/', $scheme);
...
function includes_url( $path = '', $scheme = null ) {
	 ...
	 * @param string $path Path relative to the URL to the hq-includes directory. Blank string
...
function content_url($path = '') {
	$url = set_url_scheme( HQ_CONTENT_URL );
...
function network_admin_url( $path = '', $scheme = 'admin' ) {
	...
	$url = network_site_url('hq-admin/network/', $scheme);
...
function user_admin_url( $path = '', $scheme = 'admin' ) {
	$url = network_site_url('hq-admin/user/', $scheme);
...
function rel_canonical() {
	...
	global $hq_the_query;
	if ( !$id = $hq_the_query->get_queried_object_id() )
		return;
 ...
 */
function hq_get_shortlink($id = 0, $context = 'post', $allow_slugs = true) {
	/**
 ...
 *
 *  Attached to the hq_head action.
 *
 * @since 0.0.1
 */
function hq_shortlink_hq_head() {
	$shortlink = hq_get_shortlink( 0, 'query' );
 ...
 *
 * Attached to the hq action.
 *
 * @since 0.0.1
 */
function hq_shortlink_header() {
	if ( headers_sent() )
		return;
...
function the_shortlink( $text = '', $title = '', $before = '', $after = '' ) {
	...
	$shortlink = hq_get_shortlink( $post->ID );
