 ...
 *
 * Some HiveQueen functionality is based on the current user and not based on
...
function hq_notify_postauthor( $comment_id, $deprecated = null ) {
	...
	$hq_email = 'wordpress@' . preg_replace('#^www\.#', '', strtolower($_SERVER['SERVER_NAME']));
...
if ( !function_exists('hq_notify_moderator') ) :
/**
 ...
 */
function hq_notify_moderator($comment_id) {
	global $hqdb;
	...
	$comment_author_domain = @gethostbyaddr($comment->comment_author_IP);
	$comments_waiting = $hqdb->get_var("SELECT count(comment_ID) FROM $hqdb->comments WHERE comment_approved = '0'");
...
if ( !function_exists('hq_password_change_notification') ) :
/**
 ...
 */
function hq_password_change_notification(&$user) {
	...
	if ( 0 !== strcasecmp( $user->user_email, get_option( 'admin_email' ) ) ) {
		...
		// we want to reverse this for the plain text arena of emails.
		$blogname = hq_specialchars_decode(get_option('blogname'), ENT_QUOTES);
		hq_mail(get_option('admin_email'), sprintf(__('[%s] Password Lost/Changed'), $blogname), $message);
	}
}
endif;

if ( !function_exists('hq_new_user_notification') ) :
/**
...
function hq_new_user_notification( $user_id, $deprecated = null, $notify = '' ) {
	...
	$message .= '<' . network_site_url("hq-login.php?action=rp&key=$key&login=" . rawurlencode($user->user_login), 'login') . ">\r\n\r\n";
...
if ( !function_exists('hq_nonce_tick') ) :
/**
...
if ( !function_exists('hq_verify_nonce') ) :
/**
 ...
 *                   0-12 hours ago, 2 if the nonce is valid and generated between 12-24 hours ago.
 */
function hq_verify_nonce( $nonce, $action = -1 ) {
	$nonce = (string) $nonce;
	$user = hq_get_current_user();
	$uid = (int) $user->ID;
	if ( ! $uid ) {
	...
	$expected = substr( hq_hash( $i . '|' . $action . '|' . $uid . '|' . $token, 'nonce'), -12, 10 );
	if ( hash_equals( $expected, $nonce ) ) {
		return 1;
	}

	// Nonce generated 12-24 hours ago
	$expected = substr( hq_hash( ( $i - 1 ) . '|' . $action . '|' . $uid . '|' . $token, 'nonce' ), -12, 10 );
	if ( hash_equals( $expected, $nonce ) ) {
		return 2;
	}
...
if ( !function_exists('hq_create_nonce') ) :
/**
	...
	if ( in_array( $scheme, array( 'auth', 'secure_auth', 'logged_in', 'nonce' ) ) ) {
		foreach ( array( 'key', 'salt' ) as $type ) {
			...
			if ( defined( $const ) && constant( $const ) && empty( $duplicated_keys[ constant( $const ) ] ) ) {
				...
				if ( ! $values[ $type ] ) {
					$values[ $type ] = hq_generate_password( 64, true, true );
		...
		if ( ! $values['key'] ) {
			...
			if ( ! $values['key'] ) {
				$values['key'] = hq_generate_password( 64, true, true );
 ...
 */
function hq_hash($data, $scheme = 'auth') {
	$salt = hq_salt($scheme);
...
if ( !function_exists('hq_hash_password') ) :
/**
 ...
 * @since 0.0.1
 *
 * @global PasswordHash $hq_hasher PHPass object
 *
 * @param string $password Plain text user password to hash
 * @return string The hash string of the password
 */
function hq_hash_password($password) {
	global $hq_hasher;
	...
	return $hq_hasher->HashPassword( trim( $password ) );
}
endif;

if ( !function_exists('hq_check_password') ) :
/**
 ...
 * @since 0.0.1
 *
 * @global PasswordHash $hq_hasher PHPass object used for checking the password
 ...
 */
function hq_check_password($password, $hash, $user_id = '') {
	global $hq_hasher;
	...
	if ( strlen($hash) <= 32 ) {
		...
		if ( $check && $user_id ) {
			...
			hq_set_password($password, $user_id);
	...
	$check = $hq_hasher->CheckPassword($password, $hash);
...
if ( !function_exists('hq_generate_password') ) :
/**
 ...
 */
function hq_generate_password( $length = 12, $special_chars = true, $extra_special_chars = false ) {
	...
	$password = '';
	for ( $i = 0; $i < $length; $i++ ) {
		$password .= substr($chars, hq_rand(0, strlen($chars) - 1), 1);
	}
...
function hq_rand( $min = 0, $max = 0 ) {
	...
	if ( strlen($rnd_value) < 8 ) {
		...
		if ( ! defined( 'HQ_SETUP_CONFIG' ) && ! defined( 'HQ_INSTALLING' ) ) {
	...
	// Strip the first eight, leaving the remainder for the next call to hq_rand().
...
if ( !function_exists('hq_set_password') ) :
/**
...
if ( !function_exists( 'hq_text_diff' ) ) :
/**
